...
//			$SESSION['affiliate_id'] = $affiliate_id;
	...
	if ($set->introducingBrokerInterface) {
		...
		$qry = "select * from affiliates where id in ( select distinct (refer_id) as id from affiliates) and valid = 1". ($userLevel=='manager' ? " and group_id = " . $group_id : "");
		...
		while ($affiliateww = mysql_fetch_assoc($affiliateqq)) {
					...
					while($ts = mysql_fetch_assoc($traderStatsQ)){
						...
						// if (!in_array($ww['id'] . '-' .  $ts['trader_id'],$tradersProccessedForLots)) {
								...
								// echo $totalLots
									...
									// var_dump($row);
									// die();
								$a = getCommission($from, $to, 0, $group_id, $arrDealTypeDefaults, $row) / 100 * $floatAffSubCom;
						...
						$dealsForAffiliate['pnl']=1; 
								...
								/* if (!in_array($merchantID . '-' .  $arrRes['trader_id'],$tradersProccessedForPNL)) {
									...
									// die ($where);
										...
										$pnlRecordArray['affiliate_id']  = (!empty($affiliateww['id']) ? $affiliateww['id']: "");
			...
			$listMerchants .= '<tr>
                            ...
                            <td style="color: #646464;"><b><a target="_blank" href="manager/affiliates.php?act=new&id='. $affiliateww['id'] .'">'.$affiliateww['username'].'</a></b></td>
...
/* 		if ($set->introducingBrokerInterface) 
	...
	<div class="my-slider" style="align-items:center;text-align:center;">
					...
					<table width="100%"><tr>
						<td width="60%" align="left" valign="top">
							...
							'.($db['type'] == "image" || $db['type'] == "flash" ? '
								...
								<img style="width:700px;max-height:500px;max-width:100%;" src="'. $db['file'].  '" alt=""/>':$preview).'
								<br /><br /><hr /><br />
	...
	case "creative":
		...
		while ($ww=mysql_fetch_assoc($qq)) {
			...
			$allCreative .= '<tr '.($l % 2 ? 'class="trLine"' : '').'>
							...
							<td>'.$ww['title'].'</td>'
							.(strpos($ww['file'],'/tmp/')?'<td align="center" class="tooltip">'.($ww['type'] == "image" || $ww['type'] == "flash" ? '<img src="'.$set->SSLprefix.'images/wheel.gif" width=28><span class="tooltiptext" style="padding-bottom:15px">'. lang("System is checking for virus. Please refresh in a minute.") .'</span>' : '<a href="javascript:void(0);">'.$ww['title'].'</a>').'</td>':
