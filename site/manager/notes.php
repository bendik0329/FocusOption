...
switch ($act) {
	default:
		...
		$qq=function_mysql_query("SELECT * FROM ".$appTable." WHERE group_id='".$set->userInfo['group_id']."' AND ".$sortBy." BETWEEN '".$from."' AND '".$to."' ".$where." ORDER BY ".$sortBy." DESC",__FILE__);
		while ($ww=mysql_fetch_assoc($qq)) {
			...
			$noteList .= '<tr '.($l % 2 ? 'class="trLine"' : '').' '.$bgColor.'>
						<td>'.$ww['id'].'</td>
						'.($excel ? '' : '<td><a href="'.$set->SSLprefix.'manager/affiliates.php?act=new&id='.$ww['affiliate_id'].'&note_id='.$ww['id'].'#notesPlace" target="_blank">'.lang('Edit').'</a></td>').'
						<td>'.dbDate($ww['rdate']).'</td>
						<td align="center"><a href="'.$set->SSLprefix.'manager/admins.php?act=new&id='.$adminInfo['id'].'" target="_blank">'.$adminInfo['first_name'].' '.$adminInfo['last_name'].'</a></td>
		...
		<div class="normalTableTitle" style="cursor: pointer;" onclick="$(\'#tab_1\').slideToggle(\'fast\');">'.lang('Manager Notes').'</div>';
	...
	case "xml":
		...
		while ($ww=mysql_fetch_assoc($qq)) {
			...
			$csvContent[$i][] = iconv('UTF-8', 'windows-1255', lang(strtoupper($ww['status'])));
