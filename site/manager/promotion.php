...
switch ($act) {
	...
	default:
		...
		$qq = function_mysql_query("SELECT affiliates.id as group_id , mp.* FROM " . $appProm . " mp inner join affiliates on mp.affiliate_id = affiliates.id WHERE mp.valid>-1 and mp.merchant_id='" . $merchant_id . "' ".($userlevel=='manager' ? ' and affiliates.group_id = ' . $set->userInfo['group_id']  :"" ). " ORDER BY lower(mp.title) ASC",__FILE__);
		...
		$set->content .= '<form id="frmPromotion" method="post">
						...
						<table width="110%" border="0" cellpadding="0" cellspacing="5">
							...
							<tr style="height:60px!important;">
								...
								'.lang('Affiliates ID').': <input type="text" id="affiliate_id" name="db[affiliate_id]" value="'.$db['affiliate_id'].'" /> <span style="font-size: 11px;">('.lang('Leave empty for all the affiliates'). '. ' .lang('Use | for multiple affiliates').')</span></td>
							</tr><tr>
								<td width="15%"></td>
							</tr><tr>
						...
						<input type="hidden" name="act" value="deletePromotions" />
						<input type="hidden" name="merchant_id" value="'.$merchant_id.'" />
						<table><tr>
						<td align="left">'.lang('Delete by promotion name').'
						<select name="promotionName" style="margin-left:10px;margin-top:5px;width: 292px;"><option value=0">'.(lang('Choose Promotion')).'</option>'.$promotions_list.'</select>' 
						. '	</td>
...
*/
	...
	$aff = rtrim($aff,'|');
	$aff = rtrim($aff,',');
	$affiliate_ids = str_replace('|',',',$aff);
	$qry = "select id from affiliates where valid = 1 " . ($group_id>-1 ? " and group_id = " . $group_id  . " " : "" ) . " and id in (".$affiliate_ids.")";
	$rsc = function_mysql_query($qry,__FILE__);
	while ($row = mysql_fetch_assoc($rsc)) {
	$aff_id = $row['id'];
