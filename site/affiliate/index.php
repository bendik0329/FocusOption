	...
	default:
		...
		if ($noMerchants == 1) {
			...
			foreach ($merchantsArray as $merchantww) {
				...
				$qry = "select sum(Impressions) as Impressions, sum(Clicks) as Clicks,  sum(Install) as Install, sum(Leads) as Leads,  
				
				sum(Demo) as Demo,  sum(RealAccount) as RealAccount,  sum(FTD) as FTD,  sum(FTDAmount) as FTDAmount,  sum(RawFTD) as RawFTD,  
				sum(RawFTDAmount) as RawFTDAmount,  sum(Deposits) as Deposits,  sum(DepositsAmount) as DepositsAmount,  sum(Bonus) as Bonus,  
				sum(Withdrawal) as Withdrawal,  sum(ChargeBack) as ChargeBack,  sum(NetDeposit) as NetDeposit,  sum(PNL) as PNL,  sum(ActiveTrader) as 
				ActiveTrader,  sum(Commission) as Commission,  sum(PendingDeposits) as PendingDeposits,  
				sum(PendingDepositsAmount) as PendingDepositsAmount from Dashboard WHERE  MerchantID = " . $merchantww['id'] .
					" AND Date>='" . $from . "' and Date<'" . $to . "'  AND AffiliateID = " . $set->userInfo['id'];

				$dasboardDataResult = function_mysql_query($qry, __FILE__);

				$dasboardData = mysql_fetch_assoc($dasboardDataResult);

				// Bad commissions (Trader Tag)
				echo "<!-- SELECT sum(c.Commission) as badc FROM commissions c inner join traders_tag tg on c.traderID=tg.trader_id AND c.Date>='" . $from . "' AND c.Date<='" . $to . "' AND MerchantID = " . $merchantww['id'] . " WHERE c.AffiliateID = " . $set->userInfo['id'] . " -->";

				$bad_comms_result = function_mysql_query("SELECT sum(c.Commission) as badc FROM commissions c inner join traders_tag tg on c.traderID=tg.trader_id AND c.Date>='" . $from . "' AND c.Date<='" . $to . "' AND MerchantID = " . $merchantww['id'] . " WHERE c.AffiliateID = " . $set->userInfo['id']);
			...
			$set->content .= '<input type="hidden" value="' . $totalCom . '" id="commision_hidden">';
			$set->content .= '<input type="hidden" value="' . $clicksSum . '" id="clicks_hidden">';
			$set->content .= '<input type="hidden" value="' . $totalReal . '" id="signups_hidden">';
	...
	';
			...
			if (mysql_num_rows($merchantqq) > $showInnerTableForMerchantMoreThan) {
				...
				$set->content .= '<table class="tablesorter mdlReportFields_' . $merchantww['producttype'] . '" width="100%" border="0" cellpadding="4" cellspacing="0">
					<thead>
                        <tr>
                            <th><b>' . lang(ptitle('Merchant')) . ':</b></th>
                            ' . (allowView('af-impr', $deal, 'fields') ? '<th align="center">' . lang('Impression') . '</th>' : '') . '
                            ' . (allowView('af-clck', $deal, 'fields') ? '<th align="center">' . lang('Clicks') . '</th>' : '');


				if (allowView('af-instl', $deal, 'fields') && $set->deal_cpi) {
					$set->content .= '<th align="center">' . lang('Installation') . '</th>';
				}

				if (!$hideDemoAndLeads) {
					$set->content .= '
						' . (allowView('af-lead', $deal, 'fields') ? '<th align="center">' . lang(ptitle('Leads', $productType)) . '</th>' : '') . '
                        ' . (allowView('af-demo', $deal, 'fields') ? '<th align="center">' . lang(ptitle('Demo')) . '</th>' : '');
				}

				$set->content .=
					(allowView('af-real', $deal, 'fields') ? '<th align="center">' . lang(ptitle('Accounts', $productType)) . '</th>' : '')
					. ((allowView('af-frzn', $deal, 'fields') && $showCasinoFields) ? '<th>' . lang(ptitle(('Frozens'))) . '</th>' : '') . '
						
					  
                    ' . (allowView('af-ftd', $deal, 'fields') ? '<th align="center" id="ftd">' . lang(('FTD')) . '</th>' : '') . '
                    ' . (allowView('af-ftda', $deal, 'fields') ? '<th align="center" id="ftda">' . lang(('FTD Amount')) . '</th>' : '') . '
                        
						
					' . (allowView('af-tftd', $deal, 'fields') ? '<th align="center" id="tftd-f2">' . lang(('RAW FTD')) . '</th>' : '') . '
                    ' . (allowView('af-tftda', $deal, 'fields') ? '<th align="center" id="tftda">' . lang(('RAW FTD Amount')) . '</th>' : '')

					. (!allowView('af-depo', $deal, 'fields') ? '' : '<th align="center" id="depo">' . lang(('Deposits')) . '</th>')
					. (!allowView('af-depoam', $deal, 'fields') ? '' : '<th align="center" id="depoam">' . lang(('Deposit Amount')) . '</th>')

					. (allowView('af-trdvlu', $deal, 'fields') ? '<th align="center">Deposit avg</th>' : '')


					. (!allowView('af-bns', $deal, 'fields') ? '' : '<th align="center">' . lang('Bonus') . '</th>')
					. (!allowView('af-withd', $deal, 'fields') ? '' : '<th align="center">' . lang(('Withdrawal')) . '</th>')
					. (!allowView('af-chrgb', $deal, 'fields') ? '' : '<th align="center">' . lang(('ChargeBack')) . '</th>')


					. (allowView('af-ntrv', $deal, 'fields') ? '<th align="center">' . lang(('Net Deposit')) . '</th>' : '')
					. ($set->deal_pnl == 1 && allowView('af-pnl', $deal, 'fields') ? '<th>' . lang(('PNL')) . '</th> ' : '')
					. (allowView('af-qftd', $deal, 'fields') ? '<th align="center">' . lang(('QFTD')) . '</th> ' : '')
					. '<th align="center">' . lang(('Commission')) . '</th>
                    </tr>
                    </thead>
                    <tbody';
				...
				$set->content .= '
					...
					//.(allowView('af-vlm',$deal,'fields') && strtolower($productType)=='forex' ? '<th align="center">'.price($mtotalLots).'</th>':'')

					.
					(allowView('af-ntrv', $deal, 'fields') ? '<th align="center">' . price($mtotalNetRevenue) . '</th>' : '')
					. ($set->deal_pnl == 1 && allowView('af-pnl', $deal, 'fields') ? '<th>' . price($mtotalPnl) . '</th> ' : '')
					. (allowView('af-qftd', $deal, 'fields') ? '
						...
						<div class="cart-report-box">
							<div class="account-manager-top">
								...
								<div class="account-manager-list">
									...
									<p class="green-color"><a href="' . $set->SSLprefix . 'affiliate/account.php?act=commission" target="_blank">' . '$ ' . number_format($totalCom, 2, '.', '') . '</a></p>
								</div>
							</div>
