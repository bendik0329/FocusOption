...
writeToLog("Dashboard - " . json_encode($_SESSION));
if (!isLogin()) {
	if(isset($_GET['v']))
					...
					// echo $deal . '<br>';
							...
							' : '' ) ;
                                ...
                                $listMerchants .= '
                                    ...
                                    '.(allowView('af-tftda',$deal,'fields') ? '<th align="center">'.$mtotalRealFtd.'</th><th align="center">'.price($mtotalRealFtdAmount).'</th>' : '').'
                                    '.
                                    
                                    ($set->userInfo['showDeposit'] && (allowView('af-depo',$deal,'fields'))
                                    ...
                                    ($set->userInfo['showDeposit'] && (allowView('af-depoam',$deal,'fields'))
                                    ...
                                    (allowView('af-trdvlu',$deal,'fields') ? 
										...
										: '') . 
                                    ($set->userInfo['showDeposit'] && (allowView('af-bns',$deal,'fields'))
                                        ?  '<th align="center">'.price($mtotalBonus).'</th>' :''
                                    ) . 
                                    ($set->userInfo['showDeposit'] && (allowView('af-withd',$deal,'fields'))
                                        ?  '<th align="center">'.price($mtotalWithdrawal).'</th>': '').'
									'.($set->userInfo['showDeposit'] && (allowView('af-chrgb',$deal,'fields')) ?
...
// die();			
			...
			//lots 
									$sql = 'SELECT turnover AS totalTurnOver,trader_id,merchant_id,rdate,affiliate_id,profile_id,banner_id FROM data_stats  '
										 . 'WHERE affiliate_id = '.$set->userInfo['id']. ' and merchant_id = "' . $merchantww['id'] . '" AND rdate BETWEEN "' . $from . '" AND "' . $to . '" '
										 ;
   ...
   //Sub Affiliate Commission
	...
	$listMerchantsHeader = '<table class="tablesorter" width="100%" border="0" cellpadding="4" cellspacing="0">
                ...
                $listMerchantsHeader .='
                                        ...
                                        . (!allowView('af-depo',$deal,'fields') || (!$set->userInfo['showDeposit'])
                                            ? '' : '<th align="center">'.lang('Deposits').'</th>'
                                            )
                                        . (!allowView('af-depoam',$deal,'fields') || (!$set->userInfo['showDeposit'])
                                            ? '' : '<th align="center">'.lang('Deposit Amount').'</th>'
                                            )
                                        .(allowView('af-trdvlu',$deal,'fields') ? 
											'<th align="center">'.lang(ptitle('Trader Value')).'</th>' : '')
                                        . (!allowView('af-bns',$deal,'fields') || (!$set->userInfo['showDeposit'])
                                            ? '' : '<th align="center">'.lang('Bonus').'</th>'
                                            )
                                        . (!allowView('af-withd',$deal,'fields') || (!$set->userInfo['showDeposit'])
                                            ? '' : '<th align="center">'.lang('Withdrawal').'</th>')
											
											. (!allowView('af-chrgb',$deal,'fields') || (!$set->userInfo['showDeposit']) ?  '' :
...
<div class="normalTableTitle" >'.lang('Merchants Performance').'<span class="imgGear" style="float:right;"><img class="imgReportFieldsSettings_dashstat" style="padding-top:6px;width:55%;cursor:pointer;" src="'.$set->SSLprefix.'images/settings.png"/></span></div>
			...
			<table width="100%" border="0" cellpadding="4" cellspacing="5" class="dashStatFields"><tr>
				...
				'.($showCasinoFields  && allowView('af-impr',$deal,'fields') ? '			
			...
			if($displayFtdAmountTotal){
				...
				.(!$set->userInfo['showDeposit'] || !allowView('af-depoam',$deal,'fields') ?	 '' : '<td class="dashStat '.lang('Deposits Amount').'">
				...
				.(!$set->userInfo['showDeposit'] || !allowView('af-bns',$deal,'fields') ? 		 '' : '<td class="dashStat '.lang('Bonus').'">
					'.lang('Bonus').'<br />
					<span style="font-size: 18px; font-weight: bold;">'.price($totalBonus+$marketTotal_Bonus,2).'</span>
				</td>')
				.(!$set->userInfo['showDeposit'] || !allowView('af-withd',$deal,'fields') ? 		 '' : '<td class="dashStat '.lang('Withdrawal').'">
					'.lang('Withdrawal').'<br />
					<span style="font-size: 18px; font-weight: bold;">'.price($totalWithdrawal+$marketTotal_withdrawal,2).'</span>
				</td>')
				
				.(!$set->userInfo['showDeposit'] || !allowView('af-chrgb',$deal,'fields') ? 		 '' : '<td class="dashStat '.lang('ChargeBack').'">
					'.lang('ChargeBack').'<br />
	...
	';
		...
		if (mysql_num_rows($merchantqq) > $showInnerTableForMerchantMoreThan) {
                    ...
                    $set->content .='
                        ...
                        '.(allowView('af-ftda',$deal,'fields') ? '<th align="center">'.price($mftdAmount).'</th>' : '').'
                        '.(allowView('af-tftd',$deal,'fields') ? '<th align="center">'.$mtotalRealFtd.'</th><th align="center">'.price($mtotalRealFtdAmount).'</th>' : '')
                        
						.(!allowView('af-depo',$deal,'fields') 
                            ? '' : '<th align="center"><a href="'.$set->SSLprefix.'affiliate/reports.php?act=trader&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&type=deposit">'.$mtotal_deposits.'</a></th>'
                           )
