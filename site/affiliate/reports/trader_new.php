		...
		$l = 0;
			   ...
			   $deal = $LowestLevelDeal;
				...
				$loopedMerchant_ID = $int_merchant_id = $merchant_id;
                    ...
                    if ($affiliate_id) $where .= " AND rt.AffiliateID='".$affiliate_id."' ";
                    if ($group_id) $where .= " AND aff.group_id='".$group_id."' ";
                    if ($banner_id) $where .= " AND rt.CreativeID='".$banner_id."' ";
                    //if ($profile_id) $where .= " AND profile_id='".$profile_id."' ";
                    if ($trader_id) $where .= " AND rt.TraderID='".$trader_id."' ";
                    if ($param){
						$param =  trim($param);
					    $where .= " AND rt.Param='".$param."' ";
					}
                    
                    if ($param2) $where .= " AND rt.Param2='".$param2."' ";
			...
			$traders = implode(",",$arrAllTraders);
                    ...
                    if($type == 'ftd' || $type == 'totalftd'){
                        $tradersQuery = "SELECT rt.*, aff.group_id as GroupID FROM ReportTraders rt INNER JOIN affiliates aff ON rt.AffiliateID = aff.id WHERE 1=1 ".$where." ORDER BY RegistrationDate DESC, rt.TraderID ASC";
                    }else{
                        $tradersQuery = "SELECT rt.*, aff.group_id as GroupID FROM ReportTraders rt INNER JOIN affiliates aff ON rt.AffiliateID = aff.id WHERE RegistrationDate BETWEEN '".$from."' AND '".$to."' ".$where." ORDER BY RegistrationDate DESC, rt.TraderID ASC";
                    }
					...
					while ($traderItem = mysql_fetch_assoc($trader_report_resource)){
                        ...
                        $arrRes['saleStatus'] = $traderItem['SaleStatus'];
                        $arrRes['lastSaleNote'] = '';
                    ...
                    $listReport .= '
                        <tr>
							...
							'<td>' .date("d/m/Y H:i:s", strtotime($arrRes['initialftddate'])).($debugshowhide ? "20":'').'</td>'
							: '').'
