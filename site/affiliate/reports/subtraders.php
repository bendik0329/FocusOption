...
else if($userlevel == "affiliate")
	$globalWhere = "affiliate_id = '".$set->userInfo['id']."' AND ";
else
	$globalWhere = "";
				...
				if ($invalidRow['status']=='frozen') {
					$frozenTraders[] = $invalidRow['id'];
                            ...
                            $sql = "SELECT * FROM data_reg "
								...
								$sql = "SELECT * FROM data_reg "
                                    . "WHERE 1 = 1 " . $where . " AND " . $globalWhere . " rdate BETWEEN '" . $from . "' AND '" . $to . "' AND status <> 'frozen' and type='real' and sub_trader_id>0 "
			   ...
			   foreach ($arrResultSet as $arrRes) {
					...
					//lots 
						...
						$sql = 'SELECT  * FROM data_stats 
                                         WHERE merchant_id = "' . $arrMerchant['id'] . '" and  trader_id = "' . $arrRes['trader_id'] . '" and sub_trader_id =  ' . $arrRes['sub_trader_id'] ;
						...
						$qry = "SELECT tb1.merchant_id,tb1.banner_id, tb1.amount, tb1.rdate, tb1.affiliate_id, tb1.trader_id, tb1.id as id,tb1.sub_trader_id FROM data_sales  AS tb1
                                WHERE 1 = 1  AND tb1.merchant_id = " . $arrRes['merchant_id'] . " AND tb1.rdate between '" . $from ."' and '". $to ."'
				   ...
				   $sql = 'SELECT * FROM payments_details '
                    ...
                    $listReport .= '
                        <tr>
                            ...
                            <td><a href="'. $set->SSLprefix.$userlevel .'/reports.php?act=transactions&from='.date("d/m/Y", strtotime("-3 Years")).'&to='.date("d/m/Y").'&merchant_id='.$int_merchant_id.'&trader_id='.$arrRes['trader_id'].'&type=deposit">'.$total_deposits.'</a></td>
							...
							'.($set->deal_pnl==1 ? 
								(allowView('af-pnl',$deal,'fields')  ? '<td>'.price($totalPNL).'</td>':'')
...
'.($set->export ? '<div class="exportCSV" style="float:left"><a style="cursor:pointer;" onclick="$(\'#subtradersTbl\').tableExport({type:\'csvbig\',escape:\'false\',tableName:\''.  $filename .'\'});"><img border="0" src="'.$set->SSLprefix.'images/excel.png" alt="'.lang('Export to CSV').'" title="'.lang('Export to CSV').'" align="absmiddle" /> <b>'.lang('Export to CSV').'</b></a></div>':'').'
				<div class="exportCSV" style="float:left"><a style="cursor:pointer;" onclick="$(\'#subtradersTbl\').tableExport({type:\'excelbig\',escape:\'false\',tableName:\''.  $filename .'\'});"><img border="0" src="'.$set->SSLprefix.'images/excel.png" alt="'.lang('Export to XLS').'" title="'.lang('Export to XLS').'" align="absmiddle" /> <b>'.lang('Export to XLS').'</b></a>
				</div>
				'. getFavoritesHTML() .'
			...
			//width 2600
				$tableStr ='<table class="table tablesorter mdlReportFields" border="0" cellpadding="0" cellspacing="0" id="subtradersTbl">
					<thead><tr  class="table-row">
						...
						'.($set->deal_pnl==1 ? 
						(allowView('af-pnl',$deal,'fields')  ? '<th class="table-cell">'.lang(ptitle('PNL')).'</th>':'')
						:'').'
					...
					<tfoot>
						...
						'.($set->deal_pnl==1 ? 
						(allowView('af-pnl',$deal,'fields')  ? '<th style="text-align: left;">'.price($totalTotalPNL).'</th>':'')
						:'').'
								...
								submit: function(e,v,m,f){
									if(v){
										...
										if(name != ""){
											...
											saveReportToMyFav(name, \'subtraders\',user,level,type);
										}
										else{
