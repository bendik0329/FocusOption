...
if ($product_id == "" || !is_numeric($product_id)) {
    $product_id = 0;
    $ctagArray['product_id'] = $product_id;
}
...
// if ($_GET['debug']) {
	...
	// die();
// }
$productPartTag = !empty($product_id) ? 'g'.$product_id.'-' : '';
$ctag = "a". $affiliate_id . "-" . "b". $banner_id . "-" . "p". $profile_id . "-" . $productPartTag . "c". $country_id . "-" . "u". $uid . (!empty($freeParam) ? "-f". $freeParam : "" ); 
...
//-------------------------------
    ...
    $updateImpressionStat = "INSERT INTO merchants_creative_stats(Date,AffiliateID,MerchantID,BannerID,Impressions,Clicks,CountryID)VALUES('".date('Y-m-d')."','".$affiliate_id."','".$getBanner['merchant_id']."','".$banner_id."','0','1', '".$country_id."') ON DUPLICATE KEY UPDATE Clicks=(Clicks+1)";
    function_mysql_query($updateImpressionStat,__FILE__,__FUNCTION__);

    // Referral Report
    if(!empty($_SERVER['HTTP_REFERER'])){
        $updateImpressionStat = "INSERT INTO ReportReferral(Date,ReferUrlHash,ReferUrl,AffiliateID,MerchantID,ProfileID,AllTimeClicks,LastClickIp,LastClickCountry)
                VALUES('" . date('Y-m-d') . "','" . md5($_SERVER['HTTP_REFERER']) . "','" . mysql_real_escape_string($_SERVER['HTTP_REFERER']) . "','" . $affiliate_id . "','" . $getBanner['merchant_id'] . "','" . $profile_id . "','1','".$getip."','".$country_id."') ON DUPLICATE KEY UPDATE AllTimeClicks=(AllTimeClicks+1),LastClickIp='".$getip."',LastClickCountry='".$country_id."'";
        function_mysql_query($updateImpressionStat, __FILE__, __FUNCTION__);
    }
