...
$listOfAdvertisersArray = array();
function listOfAdvertisers($id){
	global $listOfAdvertisersArray;
	if (empty($listOfAdvertisersArray)){
		$qry = "select ad.* from admins ad where ad.level='advertiser' and ad.relatedMerchantID>0 and valid=1" ; 
		$qq=function_mysql_query($qry,__FILE__);
		while ($row = mysql_fetch_assoc($qq)){
			$listOfAdvertisersArray[$row['id']] = $row;
		}
	}
	
	if ($id)
		return $listOfAdvertisersArray[$id];
	else
		return $listOfAdvertisersArray;
...
switch ($act) {
	...
	case "add":
		...
		if (empty($errors)) {
			...
			if ($sendMail) {
											...
											$qq=function_mysql_query("SELECT * FROM admins WHERE valid='1' AND level='manager' and (id='".$db['advertiser_id']."' or ('".$db['advertiser_id']."'=0  and relatedMerchantID>0) )",__FILE__);
	...
	default:
		$pageTitle = lang('Messages For Advertisers');
		...
		$qq=function_mysql_query("SELECT * FROM ".$appTable." where advertiser_id>0 or (advertiser_id=0 and affiliate_id=-1 ) ORDER BY id Desc",__FILE__);
		while ($ww=mysql_fetch_assoc($qq)) {
			$l++;
			...
			$advertiser = ($ww['group_id'] ? listOfAdvertisers($ww['advertiser_id']):"");
			...
			$messagesList .= '<tr '.($l % 2 ? 'class="trLine"' : '').'>
						...
						<td align="center"><a class="inline" href="#message_logs" data-msg_id="' . $ww['id'] . '" data-advertiser_id="' . $ww['advertiser_id'] . '">'.lang('Logs').'</a></td>
					</tr>';
			}
		...
		$set->content .= '<form method="post">
						...
						<input type="hidden" name="db[affiliate_id]" value="-1" />
						...
						<table border="0" cellpadding="0" cellspacing="5" id="tab_1">
							<tr>
								...
								<td align="center">'.lang('Choose Advertiser').': </td><td><select id="advertiser" name="db[advertiser_id]" style="width: 140px;"><option value="">'.lang('All Advertisers').'</option>'.listOfAdvertisers($db['advertiser_id']).'</select></td>
					...
					<ul type="*" style="line-height: 18px;">
						...
						<li><b>{custom_field}</b> - '.lang('Custom Variable').'</li>
							...
							$set->content .= '<br><br>		
						<div class="normalTableTitle">'.lang('Advertisers Messsage List').'</div>
    ...
    <script>
        ...
        $(".inline").click(function() {
            ...
            $.get("'.$set->SSLprefix.'ajax/loadMessageApprovalLogs.php",
                { "msg_id": elem.data("msg_id"),"advertiser_id":elem.data("advertiser_id") },
                function(res) {
                    try {
                        res = JSON.parse(res);
                        var strTr = "";
                        ...
                        for (var i = 0; i < res["success"].length; i++) {
                            ...
                            strTr += "<tr><td>" + (i + 1) + "</td><td >"+res["success"][i]["advertiser_id"]+"</td><td >"+res["success"][i]["username"]+"</td><td>" + res["success"][i]["approval_date"]+"</td></tr>";
                        }
