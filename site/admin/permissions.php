...
if (!adminPermissionCheck('permissions')) _goto($lout);
...
switch ($act) {
	...
	case "delete":
			$id = $profile_id;
			$sql = "delete from permissionProfile where id=" . $id;
	...
	case "update":
			...
			$id = isset($profile_id) && ($profile_id!="")?$profile_id:$id;
			$sql = "select * from permissionProfile where id = " . $id . " limit 1";
			 //die ($sql);
			$permission_data = mysql_fetch_assoc(function_mysql_query($sql ,__FILE__));
			$inputarray = [];
			$inputarray['name'] = $permission_name;
			...
			$sql = "UPDATE permissionProfile SET " . $values .  " WHERE id = " . $id;
			function_mysql_query($sql ,__FILE__);
			_goto($set->SSLprefix.$set->basepage.'?id='.$id . '&admin_id=' . $permission_data['created_by_admin_id']);
			break;
	...
	case "add":
			...
			$sql = "INSERT into permissionProfile ($fields) values ($values)";
			function_mysql_query($sql ,__FILE__);
			_goto($set->SSLprefix.$set->basepage);
			break;
	...
	case "duplicate":
			...
			$sql = "select * from permissionProfile where id = $id";
	...
	case "valid":
			...
			$sql = "update permissionProfile  set valid = $valid where id = $id";
			...
			break;
	default:
			$pageTitle = lang('Permission Profiles');
			...
			$(document).ready(function(){
				var addbtn = "'. lang("Add").'";
				var proSettingtxt = "'. lang('Profile Settings') .'"
		  ...
		  .ui-autocomplete { 
			...
			$permissionsqq=function_mysql_query("SELECT * FROM permissionProfile",__FILE__);
			...
			while($permissionsww=mysql_fetch_assoc($permissionsqq)) {
				...
				$i++;
					...
					$set->content .= '<span><div class="btn"><a class="addrow">'.lang('Add New Profile').'</a></div>&nbsp;&nbsp;&nbsp;'
					 .($affiliate_id > 0 ?   '<div class="btn"><a href="'.$set->SSLprefix.'admin/affiliates.php?act=new&id='.$affiliate_id.'">'.lang('Back to Affiliate page').'</a></div>' : '' ). '
					 </span>
					<div class="normalTableTitle">'.lang('Profiles').'</div>
