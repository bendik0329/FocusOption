...
switch ($act) {
	...
	case "main":
		...
		commonGlobalSetTimeRange($from, $to);
                ...
                $resourceAffiliatesTierDeals = function_mysql_query($sql,__FILE__);
			...
			$netDepositTransactions = array();
				...
				if ($mType > 1) {
                                    $listMerchants    .= '</tbody><tfoot>
                                            <tr>
                                                <th><b>'.lang('Total 345').':</b></th>
                                                <th align="center">'.$mviewsSum.'</th>
                                                <th align="center">'.$mclicksSum.'</th>
                                                '.($set->deal_cpi?'<th align="center">'.$mviewCPI.'</th>':'')
                                                ...
                                                <th align="center"><a href="/admin/reports.php?act=transactions&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&type=deposit">'.price($mtotalBonus).'</a></th>
 ...
 */
                       ...
                       output_memory('','B8','');
                        ...
                        if (!$needToSkipMerchant) {
							...
							foreach ($arrFtds as $arrFtd) {
                                ...
                                // echo '11<br>';
                                    // $arrFtd['isFTD'] = true;
									//$a = getCommission($arrFtd['rdate'], $arrFtd['rdate'], 0, -1, $arrDealTypeDefaults, $arrFtd);
									//$totalCom += $a;
									//$ftdCom += $a;
                                }
                            }
                       ...
                       output_memory('','B8','');
                         ...
                         $arrFtds  = getTotalFtds($from, $to, 0, $merchantww['id'], $merchantww['wallet_id'],$selected_group_id,0,0,"",$FILTERbyTrader,"",false,1);
                        ...
                        if (!$needToSkipMerchant) {
							...
							foreach ($arrFtds as $arrFtd) {
                                ...
                                getFtdByDealType($merchantww['id'], $arrFtd, $arrDealTypeDefaults, $ftdUsersQualified, $irrelevant['amount'], $new_qualified_ftd,false);
                        ...
                        if (strtolower($merchantww['producttype']) == 'sportsbetting' || strtolower($merchantww['producttype']) == 'casino') {
                            ...
                            while ($arrAff = mysql_fetch_assoc($resourceAffiliates)) {
                                ...
                                if ($boolTierRevShare) {
									...
									$a = getCommission($from, $to, 1, -1, $arrDealTypeDefaults, $row);
...
output_memory('After Forex','C2','');
								...
								/* $pnlRecordArray=array(
										['affiliate_id']  => (!empty($affiliate_id) ? $affiliate_id: ""),
										['merchant_id']  => $merchantww['id'],
										['trader_id']  => (isset($trader_id) ? $trader_id: ""),
										['banner_id']  => $banner_id,
										['profile_id']  => $profile_id,
										['group_id']  => $group_id,
										['searchInSql']  => $searchInSql,
										['fromdate']  => $from,
										['todate']  => $to
									); */
...
output_memory('PNL','C3','');
		...
		output_memory('Aftr PNL','C4','');
			...
			$listMerchants .= '<tr>
                            ...
                            ' . ($set->deal_cpi?'<td align="center">'.($viewCPI ? $viewCPI : '0').'</td>':'').'
...
// $totalChargeback $totalChargeBack+=$chargeback;
		...
		// List Merchants
			...
			if ($set->hidePendingProcessHighAmountDeposit ==0 ) {
			//pending deposits
		...
		$tableArr = Array(
			...
			array_push($tableArr,
				...
				(object) array(
				  'id' => 'cpi',
				  'str' => '<td class="dashStat '.lang('CPI').'">
								'.lang('CPI').'<br />
								<span style="font-size: 18px; font-weight: bold;">'.($viewCPI ? number_format($viewCPI) : '0').'</a></span>
							</td>'
				)
			);
			}
 ...
 */	
 ...
 array_push($tableArr,
		...
		if($showCasinoFields){
			...
			($set->hidePendingProcessHighAmountDeposit==0 ? 
			array_push($tableArr,(object) array(
			  'id' => 'PendingDeposits',
	...
	if(strtolower($merchantww['producttype']) == 'forex'){
		...
		$set->content .='  <link rel="stylesheet" href="js/unslider/dist/css/unslider.css">
...
' :'') .'
		<div class="normalTableTitle" >'.lang('Merchants Performance').'<span class="imgGear" style="float:right;"><img class="imgReportFieldsSettings_dashstat" style="padding-top:6px;width:55%;cursor:pointer;" src="../images/settings.png"/></span></div>	
...
FROM  `affiliates_traffic`  GROUP BY refer_url, merchant_id, affiliate_id ORDER BY rdate DESC LIMIT 8";
		...
		*/
		...
		$set->content .= '
		<div class="normalTableTitle" >'.lang('Products Place').'<span style="float:right"><img class="imgReportFieldsSettings" style="padding-top:6px;width:55%;cursor:pointer;" src="../images/settings.png"/></span></div>
