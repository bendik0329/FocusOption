...
$pageTitle = lang('Creative Categories');
...
switch ($act) {
	...
	case "add":
		...
		if ($db['id']=='') { 
			...
			$qq=function_mysql_query("SELECT * FROM merchants where valid = 1",__FILE__);
		   ...
		   $qry = "INSERT INTO merchants_creative_categories ( `merchant_id`, `categoryname`, `valid`) VALUES
			...
			if ($db['id']>0)
			$qry = "update merchants_creative_categories set categoryname='" . $categoryname . "' where id = " . $db['id'];
	...
	default:
			...
			$row = '';
			$qq=function_mysql_query("SELECT * from ". $appTable,__FILE__);
			$dataRows .= '';
