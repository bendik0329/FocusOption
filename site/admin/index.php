...
$password = strip_tags($password);		
	...
	case "main":
		...
		while ($merchantww = mysql_fetch_assoc($merchantqq)){
		    ...
		    $dasboardDataResult = function_mysql_query("select sum(Impressions) as Impressions, sum(Clicks) as Clicks,  sum(Install) as Install, sum(Leads) as Leads,  sum(Demo) as Demo,  sum(RealAccount) as RealAccount,  sum(FTD) as FTD,  sum(FTDAmount) as FTDAmount,  sum(RawFTD) as RawFTD,  sum(RawFTDAmount) as RawFTDAmount,  sum(Deposits) as Deposits,  sum(DepositsAmount) as DepositsAmount,  sum(Bonus) as Bonus,  sum(Withdrawal) as Withdrawal,  sum(ChargeBack) as ChargeBack,  sum(NetDeposit) as NetDeposit,  sum(PNL) as PNL,  sum(ActiveTrader) as ActiveTrader,  sum(Commission) as Commission,  sum(PendingDeposits) as PendingDeposits,  sum(PendingDepositsAmount) as PendingDepositsAmount from Dashboard WHERE MerchantID = ".$merchantww['id']." AND Date>='".$from."' AND Date<'".$to."'");
		    $dasboardData = mysql_fetch_assoc($dasboardDataResult);
		    
		    
		    // Bad commissions (Trader Tag)
		    $bad_comms_result = function_mysql_query("SELECT sum(c.Commission) as badc FROM commissions c inner join traders_tag tg on c.traderID=tg.trader_id AND c.Date>='".$from."' AND c.Date<='".$to."'");
		    $bad_comms = mysql_fetch_assoc($bad_comms_result);
		    if(!empty($bad_comms['badc'])){
		        $dasboardData['Commission'] = $dasboardData['Commission'] - $bad_comms['badc'];
		    }


	    $dasboardData['NetDeposit'] = ($dasboardData['DepositsAmount'] - ($dasboardData['Withdrawal'] + $dasboardData['ChargeBack'] + $dasboardData['Bonus']));

            $listMerchants .= '<tr>
                            <td style="color: #646464;" id="'.($merchantww['name']). '"><b>'.lang($merchantww['name']).'</b></td>
                            <td align="center"><a href="/admin/reports.php?act=clicks&type=views&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&merchant_id='.$merchantww['id'].'">'.$dasboardData['Impressions'].'</a></td>
                            <td align="center"><a href="/admin/reports.php?act=clicks&type=clicks&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&merchant_id='.$merchantww['id'].'">'.$dasboardData['Clicks'].'</a></td>
                            ' . ($set->deal_cpi?'<td align="center"><a href="/admin/reports.php?act=install&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&merchant_id='.$merchantww['id'].'">'.$dasboardData['Install'].'</a></td>':'').'
                            ' . ($hideDemoAndLeads ? '' : '<td align="center"><a href="/admin/reports.php?act=trader&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&merchant_id='.$merchantww['id'].'&type=lead">'.number_format($dasboardData['Leads'],0).'</a></td>
                            <td align="center"><a href="/admin/reports.php?act=trader&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&merchant_id='.$merchantww['id'].'&type=demo">'.number_format($dasboardData['Demo'],0).'</a></td>') . '
                            <td align="center"><a href="/admin/reports.php?act=trader&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&merchant_id='.$merchantww['id'].'&type=real">'.number_format($dasboardData['RealAccount'],0).'</a></td>
                            ...
                            <td align="center"><a href="/admin/reports.php?act=trader&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&merchant_id='.$merchantww['id'].'&type=ftd">'.$dasboardData['FTD'].'</a></td>
                            <td align="center">'.price($dasboardData['FTDAmount']).'</td>
                            <td align="center"><a href="/admin/reports.php?act=trader&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&merchant_id='.$merchantww['id'].'&type=totalftd">'.($dasboardData['RawFTD']).'</a></td>
                            <td align="center">'.price($dasboardData['RawFTDAmount']).'</td>
                            <td align="center"><a href="/admin/reports.php?act=transactions&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&merchant_id='.$merchantww['id'].'&type=deposit">'.($dasboardData['Deposits']).'</a></td>
                            <td align="center">'.price(($dasboardData['DepositsAmount'])).'</td>
                            <td align="center">'.price(@round(($dasboardData['DepositsAmount']/$dasboardData['FTD']),2)).'</td>
                            <td align="center">'.price(($dasboardData['Bonus'])).'</td>
                            <td align="center">'.price(($dasboardData['Withdrawal'])).'</td>
                            <td align="center">'.price(($dasboardData['ChargeBack'])).'</td>
							<td align="center">'.price($lots).'</td>
                            <td align="center"><a href="/admin/reports.php?act=stats&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&merchant_id='.$merchantww['id'].'">'.price(($dasboardData['NetDeposit'])).'</a></td>'
							.($set->deal_pnl==1 ? '
							<td align="center">'.price($dasboardData['PNL']).'</td> ' : '').
							'<td align="center">'.($dasboardData['ActiveTrader']).'</td>'.
                            '<td align="center">'.price(($dasboardData['Commission'])).'</td>
                        </tr>';
  
  
		    
		    // Dashboard Data START
		    $viewsSum += $dasboardData['Impressions'];
		    $clicksSum += $dasboardData['Clicks'];
		    $mTotalCPI += $dasboardData['Install'];
		    $totalLeads += $dasboardData['Leads'];
		    $totalDemo += $dasboardData['Demo'];
		    $totalReal += $dasboardData['RealAccount'];
		    $newFTD += $dasboardData['FTD'];
		    $ftdAmount += $dasboardData['FTDAmount'];
		    $totalRealFtd += $dasboardData['RawFTD'];
		    $totalRealFtdAmount += $dasboardData['RawFTDAmount'];
		    $total_deposits += $dasboardData['Deposits'];
		    $total_depositsAmount += $dasboardData['DepositsAmount'];
		    $totalBonus += $dasboardData['Bonus']; 
		    $totalWithdrawal += $dasboardData['Withdrawal'];
		    $totalChargeback += $dasboardData['ChargeBack'];
		    $totalnetDeposit += $dasboardData['NetDeposit'];
		    $totalTotalPnls += $dasboardData['PNL'];
		    $activeTraders += $dasboardData['ActiveTrader'];
		    $totalComs += $dasboardData['Commission'];
		    $pendingDeposits += $dasboardData['PendingDeposits'];
		    $sumPendingDeposits += $dasboardData['PendingDepositsAmount'];
		    $totalLots += 0;
...
' :'') .'
		...
		if (mysql_num_rows($merchantqq) > 1) {
                        ...
                        $set->content .= '<table class="tablesorter mdlReportFields_'. $merchantww['producttype'] .'" width="100%" border="0" cellpadding="4" cellspacing="0">
						<thead>
							<tr>
								...
								. ($hideDemoAndLeads ? '' : '
                                                                <th align="center">'.lang(ptitle('Leads',$productType)).'</th>
								<th align="center">'.lang(ptitle('Demo')).'</th>') . '
								<th align="center">'.lang(ptitle('Accounts',$productType)).'</th>
                        ...
                        $set->content .= $listMerchants . '</tbody><tfoot>
                            <tr>
                                ...
                                ' . ($hideDemoAndLeads ? '' : '<th align="center"><a href="/admin/reports.php?act=trader&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&type=lead">'.number_format($totalLeads,0).'</a></th>
                                <th align="center"><a href="/admin/reports.php?act=trader&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&type=demo">'.number_format($totalDemo,0).'</a></th>') . '
                                <th align="center"><a href="/admin/reports.php?act=trader&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&type=real">'.number_format($totalReal,0).'</a></th>
                                ' . ($showCasinoFields ? '<th align="center"><a href="/admin/reports.php?act=trader&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&type=frozen">'.number_format($totalFrozens, 0).'</a></th>' : '') . '
                                <th align="center"><a href="/admin/reports.php?act=trader&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&type=ftd">'.($newFTD).'</a></th>
                                <th align="center">'.price(($ftdAmount)).'</th>
                                <th align="center"><a href="/admin/reports.php?act=trader&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&merchant_id='.$merchantww['id'].'&type=totalftd">'.($totalRealFtd).'</a></th>
                                <th align="center">'.price(($totalRealFtdAmount)).'</th>
                                <th align="center"><a href="/admin/reports.php?act=trader&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&type=deposit">'.($total_deposits).'</a></th>
								...
								<th align="center">'.price($totalLots).'</th>
								
                                <th align="center"><a href="/admin/reports.php?act=stats&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'">'
                                . price(($totalnetDeposit)).'</a></th>'
								.($set->deal_pnl==1 ? '<th align="center">'.price(($totalPnl)).'</th>':'').
                                '<th align="center"><a href="/admin/reports.php?act=trader&from='.date("d/m/Y", strtotime($from)).'&to='.date("d/m/Y", strtotime($to)).'&type=activeTrader">'.($activeTraders).'</a></th>'.
