...
switch ($act) {
	...
	case "deletecreative":
			...
			break;
				...
				$merchant_id = isset($_GET['merchant_id']) ? $_GET['merchant_id'] : "1";
				if ($id) {
				mysql_query("update merchants_creative set valid = 0 where id = " .$id);
				}	
				
				if ($merchant_id>0)
					_goto($set->basepage.'?act=trash&merchant_id='.$merchant_id);
				else  {
					// die ('gerger');
					_goto($set->basepage.'?act=trash');
				}
	...
	case "del_banner":
		...
		mysql_query("DELETE FROM ".$appTable." WHERE id='".$ww['id']."'");
		die('<a href="'.$set->basepage.'?merchant_id='.$ww['merchant_id'].'">'.lang('Go Back').'!</a>');
		break;
	...
	case "save_banner":
		...
		if (chkUpload('file')) {
			...
			$db['file'] = UploadFile('file', '5120000', 'jpg,gif,swf,jpeg,png', ($oldFileName[0] ? $oldFileName[0] : ''), 'files/banners/');
			$exp        = explode(".", $db['file']);
		...
		_goto($set->basepage . '?act=edit_banner&id=' . $db['id'] . '&ty=1');
		break;
	...
	case "edit_banner":
		...
		$tag = 'a' . $affiliate_id . '-b' . $db['id'] . '-p' . $profile_id . $freeParam;  // Creat CTag.
		...
		if ($db['type'] == "link") {
			...
			$link = $set->webAddress.'view.php?ctag='.$tag;
			$code = '<!-- '.$set->webTitle.' Affiliate Code -->
					<iframe frameborder="0" src="'.$set->webAddress.'view.php?ctag='.$tag.'" style="width: '.$db['width'].'px; height: '.$db['height'].'px;" scrolling="no"></iframe>
					<!-- // '.$set->webTitle.' Affiliate Code -->';
			...
			$preview = '<iframe src="' . $set->basepage . '?act=' . $actType . '&id=' . $db['id'] . '&ctag=' . $tag 
						. '" width="100%" height="500" frameborder="1" scrolling="no" zoom="50%">
						</iframe>
						<br />
						<a href="' . $set->basepage . '?act=' . $actType . '&id=' . $db['id'] . '&ctag=' . $tag . '" target="_blank" style="font-size: 20px; font-weight: bold; color: green;">' 
...
// All Affiliates
		...
		$set->content .= '<div align="left">
						<table width="99%"><tr>
							<td width="50%" align="left" valign="top">
								...
								<tr><td class="blueText">'.lang('Promotion').':</td><td><select name="db[promotion_id]" style="width: 263px;"><option value="">'.lang('General').'</option>'.listPromotions($db['promotion_id'],$db['merchant_id'],"","",1).'</select></td></tr>
								<tr><td class="blueText">'.lang('Creative Name').':</td><td><input type="text" name="db[title]" value="'.$db['title'].'" style="width: 250px;" /></td></tr>
								<tr><td class="blueText">'.lang('Creative URL').':</td><td><input type="text" name="db[url]" value="'.str_replace('"','&quot;',($db['url'])).'" style="width: 250px;" /></td></tr>
								...
								'.($db['type'] == "flash" || $db['type'] == "image" || $db['type'] == "mobilesplash" || $db['type'] == "mobileleader" ? '
									<tr><td class="blueText">'.lang('Creative File').':</td><td>'.fileField('file',$db['file']).'</td></tr>
								...
								'.($db['type'] == "image" || $db['type'] == "flash" || $db['type'] == "mobileleader" || $db['type'] == "mobilesplash" ? '
									...
									<div class="normalTableTitle">'.lang('Get Tracking Code For Affiliate').'</div>
                                                                        ...
                                                                        .custom-combobox {
                                                                          ...
                                                                          .ui-autocomplete { 
                                                                            ...
                                                                            <form method="get" id="getTrackingCodeForAffiliate">
                                                                                    ...
                                                                                    <tr><td>'.lang('Dynamic Parameter').':</td><td><input type="text" name="fParam" value="'.$_GET['fParam'].'" style="width: 200px;" /></td></tr>
										...
										if ($affiliate_id && !($db['type'] == "mail" || $db['type'] == "content")) {
                                                                                    $set->content .= 'Javascript:&nbsp;&nbsp;&nbsp;&nbsp;<textarea cols="56" rows="10" onclick="this.focus(); this.select();" style="overflow: auto; font-size: 11px;">'.$code.'</textarea>'.$codelink ;
								...
								if($set->facebookshare  and(($db['type'] == "link" or $db['type']=='image') && $db['file'] && $affiliate_id)){
									...
									//die ($fullurl);
										...
										$set->content .= '<div style="vertical-align:top">'.lang('Facebook Share').': <textarea cols="45" rows="8">'.$set->webAddress.'facebookshare.php?brand='.rawurlencode($merchantww['name']).'&name='.rawurlencode($db['title']).'&image='.rawurlencode($db['file']).'&url='.rawurlencode(str_replace("ad.g","click.php",$link)).'&date='.rawurlencode($db['last_update']).'&pta='.rawurlencode(time()).'</textarea></div>';
										} else {
										$set->content .= '<div style="vertical-align:top">'.lang('Facebook Share').': <textarea cols="45" rows="2">'.$fullurl.'</textarea></div>';
									}
									}
									...
									$base64 = 'data:image/PNG;base64,' . base64_encode($qrCode );
										...
										$set->content .= '<br><div style="vertical-align:top">'.lang('QR Code').':&nbsp;<img src='.$base64.' /></div>';
									}
	...
	case "valid":
		...
		echo '<a onclick="ajax(\''.$set->basepage.'?act=valid&id='.$db['id'].'\',\'bnr_'.$db['id'].'\');" style="cursor: pointer;">'.xvPic($valid).'</a>';
		die();
		break;
			...
			/*for ($i = 1; $i <= $max_upload; $i++) {
				...
				if ($form_type == '1') {
                                    if (chkUpload('file1_' . $i)) {
                                        ...
                                        $db['file'] = UploadFile('file1_' . $i, '5120000', 'jpg,jpeg,swf,bmp,gif,png,ico,pdf,pps', '', 'files/banners/');
                                        
                                        if (empty($db['file'])) {
                                            if (!empty($errors)) {
                                                _goto($set->basepage . '?merchant_id=' . $merchant_id . '&errors=' . json_encode($errors));
                                            } else {
                                                _goto($set->basepage . '?merchant_id=' . $merchant_id);
                                            }
                                        }
                                        ...
                                        for ($intCnt = 0; $intCnt < $intNumberOfInserts; $intCnt++) {
                                            ...
                                            $arrData['title']        = !empty($_POST['name1_' . $i]) ? $_POST['name1_' . $i] : $creative_name . '_' . $i;
					...
					if (isset($db['title']) && !empty($db['title'])) { 
					    ...
					    $insert     = 1; 
                                    ...
                                    if (chkUpload('file4_' . $i)) {
                                        $db['title'] = $_POST['name4_' . $i];
                                        $db['file']  = UploadFile('file4_' . $i, '5120000', 'jpg,jpeg,swf,bmp,gif,png,ico,pdf,pps', '', 'files/banners/');
					...
					if (chkUpload('file5_' . $i)) {
                                            $db['title'] = $_POST['name5_' . $i];
                                            $db['file']  = UploadFile('file5_' . $i, '5120000', 'jpg,jpeg,swf,bmp,gif,png,ico,pdf,pps', '', 'files/banners/');
	...
	case "trash":
		$set->pageTitle = lang('Creative Materials'). ' ' .lang('Recycle Bin');
		// $merchant_id = '1';
		$numqq = mysql_query("SELECT id FROM merchants");
		if (!$merchant_id) {
			$numww = mysql_fetch_assoc($numqq);
			
			if (mysql_num_rows($numqq) <= 1) { 
				_goto($set->basepage . '?act=trash&merchant_id=' . $numww['id']);
			}
			...
			$set->content .= '<div align="center"><b>' . lang('Choose merchant to manage his creative material') 
						  . ':</b><br /><br /><select onchange="location.href=\'' . $set->basepage . '?'.($isNew==1 ? 'new=1&' : '' ) .'act=trash&' . 'merchant_id=\'+this.value + hash;"><option value="">' 
          ...
          // die ('grgtr');      
		$qq = mysql_query("SELECT * FROM " . $appProm . " WHERE merchant_id='" . $merchant_id . "' ORDER BY title ASC");
		...
		while ($ww = mysql_fetch_assoc($qq)) {
			...
			$PromList .= '<tr ' . ($l % 2 ? 'class="trLine"' : '') . '>
						...
						<td align="center"><a href="' . $set->basepage . '?merchant_id=' . $merchant_id . '&prom_id=' . $ww['id'] . '">' . lang('Edit') . '</a></td>
						<td align="center" id="promlng_' . $ww['id'] . '"><a onclick="ajax(\'' . $set->basepage . '?act=prom_valid&id=' . $ww['id'] . '\',\'promlng_' 
		...
		if (mysql_num_rows($numqq) > 1) { 
			$set->content .= '<div align="right" style="padding-bottom: 10px;"><b>' . lang('Switch Merchant') . ':</b> <select onchange="location.href=\'' 
						  . $set->basepage . '?act=trash&merchant_id=\'+this.value;"><option value="">'.lang('Please Choose Merchant').'</option>' . listMerchants($merchant_id,0,0,1) . '</select></div>';
		}
...
//		if ($categoryname) $where .= " AND categoryid='".$categoryid."'";
		...
		while ($ww=mysql_fetch_assoc($qq)) {
			...
			$totalTraffic = mysql_fetch_assoc(mysql_query("SELECT SUM(views) AS totalViews, SUM(clicks) AS totalClicks FROM stats_banners WHERE banner_id='".$ww['id']."' AND merchant_id='".$ww['merchant_id']."'"));
			$allCreative .= '<tr '.($l % 2 ? 'class="trLine"' : '').'>
							<td>'.$ww['id'].'</td>
							<td><a href="'.$set->basepage.'?act=restore&merchant_id='.$ww['merchant_id'].'&id='.$ww['id'].'">'.lang('Restore').'</a></td>
							<td align="left">'.$ww['title'].'</td>
							<td align="center">'.($ww['type'] == "image" || $ww['type'] == "flash" || $ww['type'] == "mobilesplash" || $ww['type'] == "mobileleader" ? getBanner($ww['file'],'25') : '<a href="javascript:void(0);">'.$ww['title'].'</a>').'</td>
...
*/
	...
	default:
		$set->pageTitle = lang('Creative Materials for');
		// $merchant_id = '1';
		$numqq = mysql_query("SELECT id FROM merchants");
		...
		if (!$merchant_id) {
			...
			if (mysql_num_rows($numqq) <= 1) { 
				_goto($set->basepage . '?merchant_id=' . $numww['id']. ($isNew==1 ? "&new=1" : "" ));
			}
                        ...
                        //$set->content  = '<script>var hash = window.location.hash ? "#tab_3" : "";</script>';
			$set->content .= '<div align="center"><b>' . lang('Choose merchant to manage his creative material') 
						  . ':</b><br /><br /><select onchange="location.href=\'' . $set->basepage . '?'.($isNew==1 ? 'new=1&': '') . 'merchant_id=\'+this.value + hash;"><option value="">' 
...
//		if ($categoryname) $where .= " AND categoryid='".$categoryid."'";
		...
		while ($ww=mysql_fetch_assoc($qq)) {
			...
			$allCreative .= '<tr '.($l % 2 ? 'class="trLine"' : '').'>
							<td>'.$ww['id'].'</td>
							<td><a href="'.$set->webAddress. ltrim($set->basepage, '/').'?act=edit_banner&id='.$ww['id'].'" class="inline">'.lang('Edit').'</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="'.$set->basepage.'?act=deletecreative&merchant_id='.$ww['merchant_id'].'&id='.$ww['id'].'">'.lang('Delete').'</a></td>
							<!--td><a href="javascript:void(0);" onclick="NewWin(\''.$set->basepage.'?act=edit_banner&id='.$ww['id'].'\',\'editbanner_'.$ww['id'].'\',\'1000\',\'800\',\'1\');">'.lang('Edit').'</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="'.$set->basepage.'?act=deletecreative&merchant_id='.$ww['merchant_id'].'&id='.$ww['id'].'">'.lang('Delete').'</a></td-->
							<td align="left">'.$ww['title'].'</td>
							<td align="center">'.($ww['type'] == "image" || $ww['type'] == "flash" || $ww['type'] == "mobilesplash" || $ww['type'] == "mobileleader" ? getBanner($ww['file'],'25') : '<a href="javascript:void(0);">'.$ww['title'].'</a>').'</td>
							<td align="center">'.ucwords($ww['type']).'</td>
							<td align="center">'. ($ww['type'] == "link" ? '' : $ww['width'].'x'.$ww['height']).'</td>
							...
							<td align="center">'.number_format($totalTraffic['totalClicks'],0).'</td>
							<td align="center" id="bnr_'.$ww['id'].'">
								<a onclick="ajax(\''.$set->basepage.'?act=valid&id='.$ww['id'].'\',\'bnr_'.$ww['id'].'\');" style="cursor: pointer;">'.xvPic($ww['valid']).'</a>
								'.($delete ? '<a href="'.$set->basepage.'?act=del_banner&id='.$ww['id'].'">.</a>' : '').'
							</td>
						</tr>';
			}
		// var_dump ($fileArrays);
		...
		$set->content .= '<form method="post" enctype="multipart/form-data">
						...
						'<div align="left" style="padding-top: 10px; padding-bottom: 10px;">
							<table><tr>
								...
								<td align="left">
								<input type="text" name="creative_url" value="http://" style="width: 280px;" /'.
						...
						<div class="tabs">
							<div class="tab selected" rel="1" name="banner_image_flash">'.lang('Banner Image / Flash').'</div>
						...
						<div class="tab_open" id="tab_1" style="display: block;">
							<table class="normal" width="100%" border="0" cellpadding="2" cellspacing="0">
								<thead>
								<tr>
									<td width="50">#</td>
						...
						'.($set->creative_email ? '<div class="tab_open" id="tab_6">
							...
							<p></p>
							<span>'.lang('Create tracking url by adding') . ' <u>'.'http://'.$_SERVER[HTTP_HOST] . '/click.php?ctag={ctag}&subid={subid}</u> ' . lang('in the html code').'.</span>
							<p></p>
						</div>' : '').'
						...
						<div style="padding: 10px;"><input id="submit_creative_material" type="submit" value="'.lang('Save').'" /></div>
						</div>
						</form>
			...
			$set->content .= '<td align="left" class="blueText">'.lang('Creative Type').':</td>
						...
						if (mysql_num_rows($numqq) > 1) {
									$set->content .= '	
											<td><select onchange="location.href=\''  . $set->basepage . '?merchant_id=\'+this.value;"><option value="">Please Choose Merchant</option>' . listMerchants($merchant_id) . '</select></td>
...
*/
	...
	case "prom_valid":
		...
		echo '<a onclick="ajax(\''.$set->basepage.'?act=prom_valid&id='.$db['id'].'\',\'promlng_'.$db['id'].'\');" style="cursor: pointer;">'.xvPic($valid).'</a>';
		die();
		break;
