...
if($act == "valid"){
		...
		if ($db['id']>0 &&  ($valid==1))
		{
			...
			if($set->activateLogs){
				...
				$fields['country'] = '';
				$fields['location'] = 'Affiliate List';
				$fields['userType'] = $set->userInfo['level'];
				$fields['_file_'] = __FILE__;
				$fields['_function_'] = __FUNCTION__;
			...
			if($set->activateLogs){
				...
				$fields['ip'] = $set->userInfo['ip'];
				$fields['user_id'] = $set->userInfo['id'];
				$fields['theChange'] = ($valid == 1?'activate':'deactivate') . " affiliate";
				$fields['country'] = '';
				$fields['location'] = 'Affiliate List';
				$fields['userType'] = $set->userInfo['level'];
				$fields['_file_'] = __FILE__;
...
$set->content.='
	<script type="text/javascript">
		...
		$show_account = isset($_POST['show_account']) ? $_POST['show_account'] :(isset($_GET['show_account']) ? $_GET['show_account'] : "1");		
		...
		while ($ww = mysql_fetch_assoc($qq)) {
			...
			$affList .= '<tr>
					<td>'.$ww['id'].'</td>';
						...
						if($ww['valid']==1):
							if($ww['password'] == ""):							
								$affList .= ($excel ? '' : '<td align="center" style="min-width:85px"><a href="'.$set->SSLprefix.'admin/affiliates.php?act=new&id='.$ww['id'].'" style="float:left;padding:0 6px;">'.lang('Edit').'</a>'. ( $show_account==1 || $show_account=="-3" && $act!="pending" ? '<span style="float:left;"> | </span><div class="test'. $ww['id'] .'" style="float:left;padding:0 6px;"><div class="tooltip"><a href="javascript:void(0);" oncontextmenu="return false">'.lang('Login').'</a><span class="tooltiptext">'. lang("Please set password to login.") .'</span></div>':'').'</td>');
							else:
								$affList .= ($excel ? '' : '<td align="center" style="min-width:85px"><a href="'.$set->SSLprefix.'admin/affiliates.php?act=new&id='.$ww['id'].'" style="float:left;padding:0 6px;">'.lang('Edit').'</a>'. ( $show_account==1 || $show_account=="-3" && $act!="pending" ? '<span style="float:left;"> | </span><div class="test'. $ww['id'] .'" style="float:left;padding:0 6px;"><a href="'.$set->SSLprefix.'/?act=login&username='.$ww['username'].'&password='.$ww['password'].'&admin='.$set->userInfo['id'].'" target="_blank" oncontextmenu="return false">'.lang('Login').'</a></div>':'').'</td>');
							endif;
						else:
			...
			$affList .='<div class="active_text'. $ww['id'] .'" style="display:none"><a href="/?act=login&username='.$ww['username'].'&password='.$ww['password'].'&admin='.$set->userInfo['id'].'" target="_blank">'.lang('Login').'</a></div>';
			...
			<td align="center"><a href="mailto:'.$ww['mail'].'">'.$ww['mail'].'</a></td>
					...
					<td align="center" class="valid" data-active="'. $ww['valid'] .'" id="lng_'.$ww['id'].'">'.($excel ? ($ww['valid'] ? 'Yes' : ($ww['valid']==-1 ? 'Deleted' : 'No')) : '<a onclick="ajax(\''.$set->SSLprefix.$set->basepage.'?act=valid&id='.$ww['id'].'\',\'lng_'.$ww['id'].'\');setStatus('. $ww['id'] .','. $ww['valid'] .','. $password .');send_email('. $ww['id'].')" style="cursor: pointer;">'.xvPic($ww['valid']).'</a>').'</td>
				</tr>';
			}
		$set->sortTable = 1;
		...
		<form method="get">
					...
					<table width="98%" border="0" cellpadding="0" cellspacing="5">
						...
						<tr>
							...
							<!--td width="100"><select name="group_id" style="width: 100px;"><option value="">'.lang('All Groups').'</option><option value="0" '.($group_id == "0" ? 'selected="selected"' : '').'>'.lang('General').'</option>'. $affiliateGroups[$group_id].'</select></td>
							<td width="100"><select name="status_id" style="width: 100px;"><option value="">'.lang('All Categories').'</option><option value="0" '.($status_id == "0" ? 'selected="selected"' : '').'>'.lang('General').'</option>'.$affiliateStatus[$status_id].'</select></td-->
							...
							<option value="-3"'. ($show_account=="-3" ?"selected":"") .'>'.lang('Show All Accounts').'</option></select></td>
							...
							<td align="left" width="60%">
								'.lang('Total Online').': <a href="'.$set->SSLprefix.$set->basepage.'?act=search&logged=1"><b>'.$online.'</b></a> / '.lang('Total Affiliates').': <b>'.$totalAffiliates.'</b>
							</td>
						</tr>
					</table>
					...
					'.($set->export ? '<div class="exportCSV" style="float:left"><a style="cursor:pointer;" onclick="javascript:saveLogActivity(\'csv\');$(\'#affiliatesData\').tableExport({type:\'csvbig\',escape:\'false\',ignoreColumn:[1],tableName:\''.  $filename .'\'});"><img border="0" src="'.$set->SSLprefix.'images/excel.png" alt="'.lang('Export to CSV').'" title="'.lang('Export to CSV').'" align="absmiddle" /> <b>'.lang('Export to CSV').'</b></a></div>':'').'
					<div class="exportCSV" style="float:left"><a style="cursor:pointer;" onclick="javascript:saveLogActivity(\'excel\');$(\'#affiliatesData\').tableExport({type:\'excelbig\',escape:\'false\',ignoreColumn:[1],tableName:\''.  $filename .'\'});"><img border="0" src="'.$set->SSLprefix.'images/excel.png" alt="'.lang('Export to XLS').'" title="'.lang('Export to XLS').'" align="absmiddle" /> <b>'.lang('Export to XLS').'</b></a>
					...
					<script>
							...
							function saveLogActivity(type){
								
								
								$.ajax({
									type: "POST",
									url: "'. $set->SSLprefix .'ajax/saveLogActivity.php",
									data: {
										
										ip : "' . $set->userInfo['ip'] .'",
										user_id : ' .  $set->userInfo['id'] . ',
										theChange  : '. json_encode($_GET) .',
										country  : "",
										location :  "Export" + type,
										userType: "'. $set->userInfo['level'] .'",
										_file_: "'. __FILE__ .'",
										_function_: "'. __FUNCTION__ .'"
					...
					$tableStr = '<table class="table tablesorter mdlReportFields" width="100%" border="0" cellpadding="0" cellspacing="0" id="affiliatesList">
						<thead>
						<tr  class="table-row">
							<th class="table-cell">'.lang('Affiliate ID').'</th>
							'.($excel ? '' : '<th class="table-cell" style="min-width:85px">'.lang('Actions').'</th>').'
				...
				$(document).ready(function(){
					...
					//MODAL
					$fields = getReportsHiddenCols("affiliateListReport","admin",$set->userInfo['id']);
					if($fields){
