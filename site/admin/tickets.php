...
switch ($act) {
	...
	case "add":
		...
		$db['affiliate_id'] = retrieveAffiliateId($affiliate_id);
		...
		if ($errors) {
			...
			$mailqq=function_mysql_query("SELECT id,email FROM admins WHERE valid='1' AND group_id='".$set->userInfo['group_id']."'",__FILE__);
			$allEmails = Array();
			while ($mailww=mysql_fetch_assoc($mailqq)){ 
				$allEmails[] = $mailww['email'];
				//sendTemplate('NewTicketFromAffiliate',$mailww['id'],0,'',$lastID);
			}
			...
			//send email to affiliate if admin has selected - ADDED BY SHALINI
			if(!empty($db['affiliate_id'])){
				sendTemplate('NewTicketForAffiliate',$db['affiliate_id'],0,'',$lastID);
			}
	...
	case "new":
		...
		// All Affiliates.
			...
			if (!(isset($affiliate_id) && !empty($affiliate_id))) {
				$listOfAffiliates = '<option selected value="0">'.lang('Choose Affiliate').'</option>';
			}
		...
		$set->content .= '<div class="normalTableTitle" style="width: 100%;">'.lang('Open New Ticket').'</div>
			<div style="background: #F8F8F8; padding: 10px;">
				<form action="'.$set->SSLprefix.$set->basepage.'" method="post" '.($set->isNetwork ? 'onSubmit= "return checkMerchant()"' : '').'>
				<input type="hidden" name="act" value="add" />
				<table>
					'.($set->isNetwork  || 1>0 ? '
					<tr>
						<td '.err('subject').'>'.lang('Affiliate').':</td>
						<td><!--select id="affiliateIDSBBN" name="affiliate_id_byName" style="width: 360px;" >'.$affiliateOptions.'</select-->
						
						<span class="ui-widget" style="padding-right:20px">'
								. '<!-- name="affiliate_id" -->'
						...
						<!--&nbsp&nbsp ID: <select id="affiliateIDSB" name="db[affiliate_id]" style="width: 220px;" >'.$affiliateOptions2.'</select>--></td>
...
//			die();
		...
		$affiliates = function_mysql_query('SELECT id,username FROM affiliates WHERE valid=1 ORDER BY username ASC',__FILE__);
		$affiliates2 = function_mysql_query('SELECT id,username FROM affiliates WHERE valid=1 ORDER BY id ASC',__FILE__);
		$affiliateOptions = '<option value=-1>'.lang('Select Affiliate').'</option>';
...
case "resend":	
		...
		sendTemplate('ResendNotification',$ww['affiliate_id'],0,'',$id,$ww['merchantID']);
	...
	default:
		...
		$ticketqq=function_mysql_query("SELECT * FROM ".$appTable." WHERE ticket_id='0' ".$where." ORDER BY id DESC",__FILE__);
		while ($ticketww=mysql_fetch_assoc($ticketqq)) {
			...
			$allTickets .= '<tr style="background: #'.$bg.';">
							...
							<td align="center"><a href="'.$set->SSLprefix.'admin/affiliates.php?act=new&id='.$adminInfo['id'].'" target="_blank">'.$adminInfo['username'].'</a></td>
...
//	if($set->isNetwork){
			$newTicketBtn = '<input type="button" style="position:absolute; margin-left:20px; margin-top:-5px; padding:10px; background:LIGHT GREY; color:#000; border:0px" value="New ticket" onClick="window.location=\''.$set->SSLprefix.'admin/tickets.php?act=new\';"/>';
	//	}else{
	//		$newTicketBtn = '';
	//	}
