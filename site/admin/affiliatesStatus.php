...
switch ($act) {
	...
	default:
			...
			$row = '';
			$sql ="SELECT afs.*,ad.username FROM ".$appTable." afs JOIN admins ad on ad.id = afs.created_by_admin_id ORDER BY id ASC";
		...
		while ($ww=mysql_fetch_assoc($qq)) {
			...
			$langList .= '<tr '.($l % 2 ? 'class="trLine"' : '').'>
						<td>'.$ww['id'].'</td>
						<td align="center"><a href="'.$set->SSLprefix.'admin/affiliatesStatus.php?act=add&status_id='.$ww['id'].'">'.lang('Edit').'</a> | <a href="'.$set->SSLprefix.'admin/affiliatesStatus.php?act=delete&status_id='.$ww['id'].'">'.lang('Delete').'</a></td>
						<td>'.$ww['title'].'</td>
						<td align="center">'.dbDate($ww['rdate']).'</td>
						<td align="center"><a href="'.$set->SSLprefix.'admin/affiliates_list.php?status_id='.$ww['id'].'">'.$totalAffiliates.'</a></td>
						<td align="center"><a href="'.$set->SSLprefix.'admin/admins.php?act=new&id='.$ww['created_by_admin_id'].'">'.$ww['username'].'</a></td>
		...
		$set->content = '<form method="post" >
						...
						<table class="normal" width="100%" border="0" cellpadding="2" cellspacing="0">
							...
							<tr>
								...
								<td align="center">'.lang('Created By').'</td>
