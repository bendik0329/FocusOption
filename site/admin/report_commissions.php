...
if($_GET['back'] == 1){
    $getLastDateCommissionResult = function_mysql_query("SELECT Date FROM commissions ORDER BY Date ASC LIMIT 1", __FILE__);    
    if(!empty($getLastDateCommissionResult)){
        $getLastDateCommission = mysql_fetch_assoc($getLastDateCommissionResult);
        if(!empty($getLastDateCommission['Date'])){
            $from = date("d/m/Y",strtotime($getLastDateCommission['Date'].' -2 days'));
            $to = date("d/m/Y 23:59:59",strtotime($getLastDateCommission['Date']));
        }
    }
}elseif(!empty($_GET['m_date'])){
	$from =  date("d/m/Y",strtotime($_GET['m_date']));
    $to =  date("d/m/Y 23:59:59",strtotime($_GET['m_date']));    
}else{
...
//$from =  date("d/m/Y",strtotime('-130 day'));
//$to =  date("d/m/Y",strtotime('-120 day')) . " 23:59:59";
                        ...
                        foreach ($arrRanges as $arrRange) {
							...
							$strAffDealType = getAffiliateTierDeal($regww['merchant_id'],$regww['affiliate_id']);
                                ...
                                $boolTierCplCount = !is_null($strAffDealType) && 'cpl_count' == $strAffDealType;
								    ...
								    switch ($regww['type']) {
                                        case 'lead': $commissionArray[] = addCommissionForProductActions($ww['id'], $ww['name'], $regww['affiliate_id'], $regww['rdate'], $regww['trader_id'], $regww['product_id'], 'cpllead');
                                            break;
                                        case 'real': $commissionArray[] = addCommissionForProductActions($ww['id'], $ww['name'], $regww['affiliate_id'], $regww['rdate'], $regww['trader_id'], $regww['product_id'], 'cplaccount');
                                    }
                                        ...
                                        if (!$boolTierCplCount) {
											...
											if(isset($commission_type) && $commission_type == "All" || $commission_type == 'CPL'){
												if(!empty($a)){
													...
													$regww['affiliate_id'] = $regww['affiliate_id'];
							   ...
							   echo " Commission after Tier " . $tierCom . "<br/>";
								...
								while ($salesww = mysql_fetch_assoc($salesqq)) {
									...
									if ($salesww['data_sales_type'] == 'deposit') {   // NEW.
                                        ...
                                        // cpa type in that case like a commission on sale
                                        $commissionArray[] = addCommissionForProductActions($ww['id'], $ww['name'], $salesww['affiliate_id'], $salesww['rdate'], $salesww['trader_id'], $salesww['product_id'], 'cpa');
                                    ...
                                    if ($salesww['data_sales_type'] == "withdrawal" || $salesww['data_sales_type'] == "chargeback"){
										$tranrow['id'] = $salesww['id'];
										$tranrow['affiliate_id'] = $salesww['affiliate_id'];
										$tranrow['tranz_id'] = $salesww['tranz_id'];
										$tranrow['trader_id'] = $salesww['trader_id'];
									...
									$b = getCommission($arrFtd['FTDqualificationDate'], $arrFtd['FTDqualificationDate'], 0, $selected_group_id, $arrDealTypeDefaults, $arrFtd);
											...
											if(isset($commission_type) && $commission_type == "All" || $commission_type == 'CPA'){
                                                                            ...
                                                                            if(!empty($b)){
                                                                                ...
                                                                                // CPA By Countries Group
                                                                                $resource_my_cpageo = function_mysql_query('SELECT cgd.* FROM cpa_group_delas cgd right join cpa_countries_groups ccg ON ccg.id = cgd.group_id WHERE cgd.rdate <= "'.$arrFtd['FTDqualificationDate'].'" AND cgd.merchant_id = "' . $arrFtd['merchant_id'] . '" AND cgd.affiliate_id = "' . $arrFtd['affiliate_id'] . '" ORDER BY rdate DESC', __FILE__, __FUNCTION__);
                                                                                $rowno_my_cpageo = mysql_num_rows($resource_my_cpageo);
                                                                                if($rowno_my_cpageo > 0){
                                                                                    
                                                                                        if(!empty($_GET['geocpa'])){
                                                                                            echo "GEO CPA\n";
                                                                                        }
                                                                                        
                                                                                        $arraygeocpadeals = [];
                                                                                        while ($item = mysql_fetch_assoc($resource_my_cpageo)) {
                                                                                            if(empty($arraygeocpadeals[$item['group_id']])){
                                                                                                $arraygeocpadeals[$item['group_id']] = [
                                                                                                    'date' => $item['rdate'],
                                                                                                    'value' => $item['value']
                                                                                                ];
                                                                                            }else{
                                                                                                
                                                                                                if(strtodate($arraygeocpadeals[$item['group_id']]['date']) < strtodate($item['rdate'])){
                                                                                                    $arraygeocpadeals[$item['group_id']] = [
                                                                                                    'date' => $item['rdate'],
                                                                                                    'value' => $item['value']
                                                                                                ];
                                                                                        ...
                                                                                        if(!empty($_GET['geocpa'])){
                                                                                            echo "VALUES\n";
                                                                                            print_r($arraygeocpadeals);
                                                                                        }
                                                                                        
                                                                                        if(array_sum(array_column($arraygeocpadeals,'value')) > 0){
                                                                                            $AffDealsArry = extractDealTypes(['affiliate_id' => $arrFtd['affiliate_id'], 'merchant_id' => $ww['id'], 'rdate' => $arrFtd['rdate']], $arrDealTypeDefaults,5);
                                                                                            if(empty($AffDealsArry['tier']['amount'])){
                                                                                                $b_country = getAffilliateCpaCountryGroupCommission($arrFtd['country'], $arrFtd['affiliate_id'], $arrFtd['merchant_id'], $arrFtd['FTDqualificationDate']);
                                                                                                $b = $b_country;
                                                                                                
                                                                                                if(!empty($_GET['geocpa'])){
                                                                                                    echo "commission - ".$b_country."\n";
                                                                                                }
                                                                                            }
                                                                                        }
                                                                                }
...
// die ($sql);											
				  ...
				  if ($set->deal_pnl == 1) {
					...
					//	$qry = "select id from affiliates where id in ( select distinct (refer_id) as id from affiliates) and sub_com>0 and valid = 1". ($userlevel=='manager' ? " and group_id = " . $group_id : "");
						...
						while ($row = mysql_fetch_assoc($rsc)) {
                            ...
                            // TODO: Temporary pending (because need trader id for write commission, but it missing in traffic now (can't be)
                            // click commission
                            /*  $clickCommissionsSQL = "SELECT * FROM traffic WHERE product_id = ".(int)$productId." AND clicks = 1 AND type = 'traffic' AND affiliate_id = ".(int)$affiliateId." AND rdate".$searchInSql;
                              $clickCommissionsResource = function_mysql_query($clickCommissionsSQL,__FILE__);

                                  while ($commissionsClick = mysql_fetch_assoc($clickCommissionsResource)) {
                                      $commissionArray[] = addCommissionForProductActions($ww['id'], $ww['name'], $commissionsClick['affiliate_id'], $commissionsClick['rdate'], $commissionsClick['trader_id'], $commissionsClick['product_id'], 'cpllead');
                                  }
                */
                            // install commission
                            $installCommissionsSQL = "SELECT * FROM data_install WHERE merchant_id = ".(int)$ww['id']."  AND rdate".$searchInSql;
                            $installCommissionsResource = function_mysql_query($installCommissionsSQL,__FILE__);
                              while ($installClick = mysql_fetch_assoc($installCommissionsResource)) {
                                  $commissionArray[] = addCommissionForProductActions($ww['id'], $ww['name'], $installClick['affiliate_id'], $installClick['rdate'], $installClick['trader_id'], $installClick['product_id'], 'cpi');
                              }
		...
		foreach($commissionArray as $key=>$com){
			...
			// ADD TO DB
			function_mysql_query("INSERT IGNORE INTO `commissions` (`merchantID`, `affiliateID`, `traderID`, `transactionID`, `Date`, `Type`, `Amount`, `DealType`, `Commission`, `DealTypeCondition`) VALUES ('".$com['merchant_id']."', '".$com['affiliate_id']."', '". $com['trader_id'] ."', '".$com['id']."', '".date("Y-m-d H:i:s", strtotime($com['rdate'])) ."', '". ucwords($com['type']) ."', '".$com['amount']."', '".$com['location']."', '".$com['commission']."', '');",__FILE__,__FUNCTION__);
