...
if($_GET['back'] == 1){
    $getLastDateTraderReportResult = function_mysql_query("SELECT Date FROM ReportTraders2 ORDER BY Date ASC LIMIT 1", __FILE__);    
...
*/            
    ...
    $qry = "SELECT dr1.* FROM data_reg dr1 WHERE dr1.merchant_id = 1 and dr1.rdate>='".$from."' and dr1.rdate<='".$to."' GROUP BY dr1.merchant_id, dr1.trader_id 
        union all 
    SELECT dr2.* FROM data_reg dr2 inner join data_sales ds on ds.trader_id=dr2.trader_id WHERE dr2.merchant_id = 1 and dr2.rdate<'".$from."' and ds.rdate>='".$from."' and ds.rdate<='".$to."' GROUP BY dr2.merchant_id, dr2.trader_id";
    ...
    if ($set->deal_pnl == 1) {
        ...
        FROM
            data_reg dr1
        WHERE
            dr1.merchant_id = 1 AND dr1.rdate >= '".$from."' AND dr1.rdate <= '".$to."'
        GROUP BY
            dr1.merchant_id,
            dr1.trader_id
        UNION ALL
    ...
    WHERE
        dr2.merchant_id = 1 AND dr2.rdate < '".$from."' AND ds1.rdate >= '".$from."' AND ds1.rdate <= '".$to."'
    GROUP BY
        dr2.merchant_id,
        dr2.trader_id";
    ...
    while ($traderInfo = mysql_fetch_assoc($trader_report_resource)) {
        ...
        FROM
            data_reg dr1
        WHERE
            dr1.merchant_id = 1 AND dr1.rdate >= '".$from."' AND dr1.rdate < '".$to."' AND dr1.initialftddate > dr1.rdate - INTERVAL 1 DAY
        GROUP BY
            dr1.merchant_id,
            dr1.trader_id
        UNION ALL
    ...
    INNER JOIN data_sales ds1 ON
        ds1.trader_id = dr2.trader_id
    WHERE
        dr2.merchant_id = 1 AND dr2.rdate < '".$from."' AND ds1.rdate >= '".$from."' AND ds1.rdate < '".$to."' AND dr2.initialftddate > dr2.rdate - INTERVAL 1 DAY
    GROUP BY
        dr2.merchant_id,
        dr2.trader_id
									) dr 
...
$replace_query = "REPLACE INTO `ReportTraders2` 
