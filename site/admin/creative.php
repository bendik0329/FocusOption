...
switch ($act) {
	...
	case "deletecreative":
				...
				$fields['country'] = '';
				$fields['location'] = 'Creatives - Delete Creatives';
				$fields['userType'] = $set->userInfo['level'];
				$fields['_file_'] = __FILE__;
				$fields['_function_'] = 'Delete Creatives';
	...
	case "featured":
		...
		if ($db['featured']) $featured='0'; else $featured='1';
		updateUnit($appTable,"featured='".$featured."'","id='".$db['id']."'");
		if($set->activateLogs){
		//activity logs
		$fields =array();
		$fields['ip'] = $set->userInfo['ip'];
		...
		$fields['country'] = '';
		$fields['location'] = 'Creatives - Featured Banner';
		$fields['userType'] = $set->userInfo['level'];
		$fields['_file_'] = __FILE__;
		$fields['_function_'] = 'Featured Banner';
		...
		echo '<a onclick="ajax(\''.$set->SSLprefix.$set->basepage.'?act=featured&id='.$db['id'].'\',\'fbnr_'.$db['id'].'\');" style="cursor: pointer;">'.xvPic($featured).'</a>';
		die();
		break;
	...
	case "valid":
		...
		$fields['theChange'] = json_encode($db);
		$fields['country'] = '';
		$fields['location'] = 'Creatives - Activate/Deactivate Banner';
		$fields['userType'] = $set->userInfo['level'];
		$fields['_file_'] = __FILE__;
		$fields['_function_'] = 'Activate/Deactivate Banner';
 ...
 */
		...
		while ($ww=mysql_fetch_assoc($qq)) {
			...
			$allCreative .= '<tr '.($l % 2 ? 'class="trLine"' : '').'>
							...
							<td align="center">'.$promInfo['title'].'</td>
	...
	default:
		...
		if (!$merchant_id) {
			...
			if( empty($numww) ){
				...
				$set->content .="<div class='noMerchantsDiv'>".  lang("There are no active merchants in the system.") . "<a href='admin/index.php'>". lang("Click here") . "</a>" . lang(" to got to dashboard  ") ."</div>";
				theme();
				die();
			}
			if (mysql_num_rows($numqq) <= 1) { 
		...
		if ($type) $where .= " AND type='".$type."'";
		if ($lang) $where .= " AND language_id='".$lang."'";
		if ($promotion || ($promotion == 0 && $promotion != '') ) $where .= " AND promotion_id='".$promotion."'";
		if ($category_id) $where .= " AND category_id='".$category_id."'";
		if ($width) $where .= " AND width='".$width."'";
		if ($height) $where .= " AND height='".$height."'";
		if ($valid==1) {
...
//		if ($categoryname) $where .= " AND categoryid='".$categoryid."'";
        ...
        $isCSVSession = false;
                if (isset($_GET['downloadcsv']) && strlen($_GET['downloadcsv'])>0) {
                    $isCSVSession = true;
		}
		...
		$pgg=$_GET['pg'] * $getPos;
		if ($isDownloadSession || $isCSVSession) 
		...
		while ($ww=mysql_fetch_assoc($qq)) {
			...
			//list($banner_width,$banner_height) = @getimagesize($ww["file"]);
			$promoInfo = getPromotion($ww['promotion_id']);
			$set->content .="
						<style>
					
											</style>";

			$totalTraffic['totalViews'] = $totalTraffic['totalClicks'] = 0;
			//$totalTraffic = mysql_fetch_assoc(function_mysql_query("SELECT SUM(views) AS totalViews, SUM(clicks) AS totalClicks FROM traffic WHERE  rdate> DATE_SUB(curdate(), INTERVAL 1 WEEK) and banner_id='".$ww['id']."' AND merchant_id='".$ww['merchant_id']."'",__FILE__));
			...
			$allCreative .= '<tr '.($l % 2 ? 'class="trLine"' : '').'>
							...
							<td><!--span class="dimantion-wrap" style="display:none">'.$banner_width .'x'. $banner_height .'</span-->'.$ww['id'].'</td>
							<td><a href="'.$set->webAddress. ltrim($set->basepage, '/').'?act=edit_banner&id='.$ww['id'].'" class="inline">'.lang('Edit').'</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="'.$set->SSLprefix.$set->basepage.'?act=deletecreative&merchant_id='.$ww['merchant_id'].'&id='.$ww['id'].$qrys.'">'.lang('Delete').'</a></td>
							<!--td><a href="javascript:void(0);" onclick="NewWin(\''.$set->SSLprefix.$set->basepage.'?act=edit_banner&id='.$ww['id'].'\',\'editbanner_'.$ww['id'].'\',\'1000\',\'800\',\'1\');">'.lang('Edit').'</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="'.$set->SSLprefix.$set->basepage.'?act=deletecreative&merchant_id='.$ww['merchant_id'].'&id='.$ww['id'].'">'.lang('Delete').'</a></td-->
							<td align="left">'.$ww['title'].'</td>'
							.(strpos($ww['file'],"/tmp")?
							...
							'<td align="center" class="img-wrap">'.($ww['type'] == "image" || $ww['type'] == "flash" || $ww['type'] == "mobilesplash" || $ww['type'] == "mobileleader" ?  '<img src="'.$ww['file'].'" width=50 height =50>' : '<a href="javascript:void(0);">'.$ww['title'].'</a>').'</td>') .
							'<td align="center">'.ucwords($ww['type']).'</td>
							<td align="center" class="dimantion-wrap">'. ($ww['type'] == "link" ? '' : $ww['width'].'x'.$ww['height']).'</td>
							<td align="center">'.listLangs($ww['language_id'],1).'</td>
							<td align="center">'.$promoInfo['title'].'</td>
							<td align="center">'.listCategory($ww['category_id'],$category_id,1,1).'</td>
							...
							<td align="center"><a href="'.$ww['url'].'" target="_blank" title="'.$ww['url'].'">'.$ww['url'].'</a></td>
							<td align="center">'.$ww['alt'].'</td>
							<td align="center" id="fbnr_'.$ww['id'].'"><a onclick="ajax(\''.$set->SSLprefix.$set->basepage.'?act=featured&id='.$ww['id'].'\',\'fbnr_'.$ww['id'].'\');" style="cursor: pointer;">'.xvPic($ww['featured']).'</a></td>
							<td align="center">'.number_format($totalTraffic['totalViews'],0).'</td>
							<td align="center">'.number_format($totalTraffic['totalClicks'],0).'</td>
							<td align="center" id="bnr_'.$ww['id'].'">
							...
							<td><a href="'.$set->SSLprefix.'admin/reports.php?act=ActiveCreatiesStats&banner_id='.$ww['id'].'">'.lang('See Stats').'</a></td>
						</tr>';
						
						$allCreativeCSV .= $ww['id'].';'.$ww['title'].';'.ucwords($ww['type']).';'.($ww['type'] == "link" ? '' : $ww['width'].'x'.$ww['height']).';'.listLangs($ww['language_id'],1).';'.$promoInfo['title'].';'.listCategory($ww['category_id'],$category_id,1,1).';'.$ww['url'].';'.$ww['alt'].';'.($ww['featured']?'Featured':'').';'.number_format($totalTraffic['totalViews'],0).';'.number_format($totalTraffic['totalClicks'],0).';'.($ww['valid']?'On':'Off').';'."\n";
			}
                ...
                if ($isCSVSession) {
                        ...
                        header('Content-Disposition: attachment; filename=creatives.csv');
                        header('Pragma: no-cache');
                    
                        $allCreativeCSV = 'id;title;type;size;language;title;category;url;alt;featured;totalViews;totalClicks;valid;'."\n".$allCreativeCSV;
...
$combolist.='</select>';
					...
					include("common/creatives/add_banner_form.php");
						...
						$set->content .='<br>
                                                ...
                                                <div class="normalTableTitle" style="cursor: pointer;" onclick="$(\'#tab_41\').slideToggle(\'fast\');">'.lang('Creatives List for').': <b>'.listMerchants($merchant_id,1,0,1).'</b></div>
                                                    <br>
						<div id="tab_41" ' .(!$isNew==1 ? '>' : 'style="display:none!important;">' );
			...
			$set->content .= '<td align="left" class="blueText">'.lang('Creative Type').':</td>
								<td align="left" class="blueText">'.lang('Category').':</td>
								<td align="left" class="blueText"><span class="langHover">'.lang('Language').':</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img class="imgSettingsLng" style="display:none;width:15%;cursor:pointer;" src="../images/settings.png" title="'.lang('Add and change languages list').'"onclick="javascript:window.open(\'../admin/languages.php\')"/></td>
									<td style="width:20px;"></td>
										<td align="left" class="blueText">'.lang('Width').':</td>
								<td align="left" class="blueText">'.lang('Height').':</td>
								<td align="left" width="110px" ><span class="blueText">'.lang('Choose Size').':</span><br><span style="font-size:10px">'.lang('Width'). ' X ' . lang('Height') . ' ( '.lang('Count').' )</span></td>
		...
		$set->content .= '
								...
								<td align="left"><select name="category_id" style="width: 100px;"><option value="">'.lang('All').'</option>'.listCreativeCategoryByMerchant($category_id,$merchant_id).'</select></td>
								...
								<td align="left" width="130">'.$combolist.'</td>
								<td style="width:20px";></td>
								<td align="left"><select name="promotion" style="width: 140px;"><option value="">'.lang('All').'</option><option value="0">'.lang('General').'</option>'.listPromotions($promotion,$merchant_id).'</select></td>
								...
								<td align="left"><input type="text" name="q" value="'.$q.'" style="width: 120px;" /></td>
								<td style="width:20px";></td>
								<td><input type="submit" value="'.lang('Search').'" /></td><td>&nbsp;&nbsp;</td><td><input name="download" type="submit" value="'.lang('Download').'" /> <input name="downloadcsv" type="submit" value="'.lang('Download CSV').'" /></td>
...
*/
		...
		$('.langHover,#lang').parent('td').on('mouseenter',function(){
			$('.imgSettingsLng').show();
		});/* .on('mouseleave',function(){
			$('.imgSettingsLng').hide();
		}) */
		$('.langHover,#lang').parent('td').on('mouseleave',function(){
			$('.imgSettingsLng').hide();
		});
		</script>";
		theme();
