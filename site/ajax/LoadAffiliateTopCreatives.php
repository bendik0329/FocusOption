...
if (!empty($affwhere)) {
		...
		//feaured creatives
		$haveFeatured = false;
	if($set->ShowOnlyFeaturedCreativesWhenGotSome == 1 || true){
	$qry = "SELECT * from merchants_creative mrc 
		...
		while ($bnrww = mysql_fetch_assoc($bnrqq)) {
					...
					$haveFeatured = true;
					$getBanner = $bnrww;
                    if (isset($merchantsArr[$bnrww['merchant_id']])){
							$merchantInfo = $merchantsArr[$merchantInfo['id']];
					}
					else{
					$merchantInfo = dbGet($bnrww['merchant_id'],"merchants");
						$merchantsArr[$merchantInfo['id']] = $merchantInfo;
					}
 ...
 */
		...
		/*$qry = "
						select creatives.banner_id, creatives.merchant_id from 
(select sum(tr.clicks)  , tr.banner_id , tr.merchant_id , tr.affiliate_id ,tr.rdate from traffic tr where tr.rdate > '".$dateBeforeMonthsAgo."' and tr.merchant_id in (".$affwhere.") and tr.affiliate_id = ".$affiliate_id."
group by tr.merchant_id , tr.banner_id
) affiliate

inner join 
(
select mc.id as banner_id, mc.merchant_id  from merchants_creative mc where 1=1 AND ( mc.promotion_id = 0 
              OR mc.promotion_id IN (SELECT id 
                                      FROM   merchants_promotions 
                                      WHERE  merchant_id IN ( ".$affwhere." ) 
                                             AND affiliate_id IN (".$affiliate_id.", 0 )) )
     
    and mc.valid=1   
    AND mc.merchant_id IN ( 1 ) 
       AND mc.file NOT LIKE '%tmp%' 
       AND mc.affiliateready = 1 
	      order by mc.promotion_id  desc) 
 creatives
 
 on affiliate.merchant_id = creatives.merchant_id and affiliate.banner_id = creatives.banner_id
 
 limit 2;
 ";*/

		$qry = "SELECT MerchantID as merchant_id, BannerID as banner_id FROM merchants_creative_stats WHERE Date > '".$dateBeforeMonthsAgo."' and MerchantID in (".$affwhere.") and AffiliateID = ".$affiliate_id." goup by banner_id limit 2";
...
*/
          $qry = "SELECT MerchantID as merchant_id, BannerID as banner_id FROM merchants_creative_stats mcs LEFT JOIN merchants_creative mrc ON mcs.BannerID = mrc.id 
          WHERE mrc.valid=1 and (mrc.promotion_id =0 or mrc.promotion_id in (
                                select id from merchants_promotions where merchant_id in (".$affwhere .") and affiliate_id='".$affiliate_id."'
                                )  )
                        and  mrc.file NOT LIKE '%tmp%' and  mrc.affiliateReady=1 and   stb.merchant_id in (".$affwhere .") GROUP BY banner_id ORDER BY RAND() LIMIT \" . (5 - $bannersCount); ";
