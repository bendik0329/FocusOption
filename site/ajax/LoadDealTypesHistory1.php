...
if (
	...
	while ($arrRow = mysql_fetch_assoc($resource)) {
		//if (is_numeric($arrRow['amount'])) {
			...
			//if($rv == 0){
				if($arrRow['amount'] > 0){
					$tooltip[$timelineDt][]= lang('NetDeposit');
				}
				else if($arrRow['amount'] == 0){
					$tooltip[$timelineDt][]= lang('NetDeposit') . "-<span class='red'>" . lang('INACTIVE')."</span>";
				}
			//}
			$rv++;
		//}
...
$sql = "SELECT IFNULL(amount, '" . lang('Merchant Default') . "') AS amount, rdate AS rdate, id AS id FROM `affiliates_deals` 
			WHERE affiliate_id = " . $_GET['affiliate_id'] . " AND dealType = 'cpi'  AND merchant_id = " . $_GET['merchant_id'] . "  and  amount>-1
			GROUP BY rdate ORDER BY rdate DESC;";
	$resource = function_mysql_query($sql,__FILE__,__FUNCTION__);
	$cm = 0 ;
	while ($arrRow = mysql_fetch_assoc($resource)) {
		//if (is_numeric($arrRow['amount'])) {
			...
			$arrTimeLine[$timelineDt][] = array('amount' => $arrRow['amount'], 'rdate' => $arrRow['rdate'], 'id' => $arrRow['id'],'deal_type'=>'cpi');
			//$arrDealType_cpm[] = array('amount' => $arrRow['amount'], 'rdate' => $arrRow['rdate'], 'id' => $arrRow['id']);
			if($cm==0)
			$arrDealType_cpi[$timelineDt][] = array('amount' => $arrRow['amount'], 'rdate' => $arrRow['rdate'], 'id' => $arrRow['id'], "current"=>1);
			else
			$arrDealType_cpi[$timelineDt][] = array('amount' => $arrRow['amount'], 'rdate' => $arrRow['rdate'], 'id' => $arrRow['id']);
			//if($cm == 0){
				if($arrRow['amount'] > 0){
					$tooltip[$timelineDt][]= lang('CPI');
				}
				else if($arrRow['amount'] == 0){
					$tooltip[$timelineDt][]= lang('CPI') . "-<span class='red'>" . lang('INACTIVE')."</span>";
				}
			//}
			$cm++;
		//}
	...
	$intLargestResultSetCount = $intLargestResultSetCount > count($arrDealType_cpi) ? $intLargestResultSetCount : count($arrDealType_cpi);
	...
	while ($arrRow = mysql_fetch_assoc($resource)) {
		...
		//if (is_numeric($arrRow['amount'])) {
			...
			if($tr == 0)
			$arrDealType_tier[$timelineDt][] = array('amount' => $arrRow['valid']?$arrRow['amount']:0, 'rdate' => $arrRow['rdate'], 'id' => $arrRow['id'], 'tier_amount' => $arrRow['tier_amount'],'pcpa' => $arrRow['valid']?$arrRow['pcpa']:0, "current"=>1);
			else
			$arrDealType_tier[$timelineDt][] = array('amount' => $arrRow['valid']?$arrRow['amount']:0, 'rdate' => $arrRow['rdate'], 'id' => $arrRow['id'], 'tier_amount' => $arrRow['tier_amount'],'pcpa' => $arrRow['valid']?$arrRow['pcpa']:0);
	...
	$timelineDt = $regTimelineDt;
			...
			if(!empty($arrDealTypeDefaults['revenue_amount'])){
				...
				if($arrDealTypeDefaults['revenue_amount']> 0){
					$tooltip[$timelineDt][]= lang('Merchant Default') . " - ".lang('NetDeposit');
				}
				else if($arrDealTypeDefaults['revenue_amount'] == 0){
					$tooltip[$timelineDt][]=lang('Merchant Default') . " - ". lang('NetDeposit') . "-<span class='red'>" . lang('INACTIVE')."</span>";
				}
			}
	...
	//echo json_encode($arrTimeLine);die;
   ...
   position: absolute;
	...
	<div class="events-content" style="overflow-x:scroll;min-height:200px">
		<ol>';
		$i=0;
		...
		foreach($arrTimeLine as $key=>$timeline){
			...
			$timelineTxt .='<li '.  ($i==0?' class="selected"':'') .' data-date="'. date('d/m/Y',strtotime($key)) .'">';
								...
								foreach($arrDealType_revenue[$key] as $k=>$row)
								{
									if($revenue==0){
										if(!empty($arrDealType_revenue[$key])){
											$timelineTxt .='<td valign="top" class="mainTD"><table class="subTable"><thead>';
											if(isset($row['default']) && $row['default']==true){
												$timelineTxt.='<tr><td colspan="2" class="heading">'. lang('Merchant Default Deal'). " - " . strtoupper(lang('NetDeposit')) .'</td></tr>';
											}
											else{
												$timelineTxt.='<tr><td colspan="2" class="heading">'. strtoupper(lang('NetDeposit')) .'</td></tr>';
											}
												$timelineTxt .= '<tr><td style="color:white;">' . strtoupper(lang('DATE')) . '</td><td style="color:white;">' . lang('COMMISSION')  . '</td></tr></thead><tbody>';
										}
									}
								...
								foreach($arrDealType_tier[$key] as $k=>$row)
								{
									if($tier==0){
										if(!empty($arrDealType_tier[$key])){
											$timelineTxt .='<td valign="top" class="mainTD"><table class="subTable"><thead>';
											if(isset($row['default']) && $row['default']==true){
												$timelineTxt.='<tr><td colspan="4" class="heading">'. lang('Merchant Default Deal'). " - " . strtoupper(lang('TIER')) .'</td></tr>';
												}
											else{
												$timelineTxt.='<tr><td colspan="4" class="heading">'. strtoupper(lang('TIER')) .'</td></tr>';
											}
												$timelineTxt .= '<tr><td style="color:white;">' . strtoupper(lang('DATE')) . '</td><td style="color:white;">' . lang('TIER AMOUNTS')  . '</td><td style="color:white;">' . lang('CPA')  . '</td><td style="color:white;">' . lang('PCPA')  . '</td></tr></thead><tbody>';
										}
									}
									if(isset($row['default']) && $row['default']==true){
										$boolDefaultDeal = true;
									...
									$timelineTxt .= '<tr><td><input class = "deals_rdate" type="text" ' .(isset($row['current'])?$isRowMarkedAsActive:'') . ' data-rdate_id="' . $row['id'] . '" value="' . $row['rdate'] . '" readonly/></td>
											<td style="min-width:147px"><input type="text" ' . (isset($row['current'])?$isRowMarkedAsActive:'') . ' data-tier_amount_id="' . $row['id'] . '" value="' . $row['tier_amount'] . '" '. $cls .'/>$</td>
											<td style="min-width:147px"><input type="text" '. (isset($row['current'])?$isRowMarkedAsActive:'') .' data-amount_id="' . $row['id'] . '" value="' . $row['amount'] . '"'. $cls .' />$</td>
											<td style="min-width:147px"><input type="text" '. (isset($row['current'])?$isRowMarkedAsActive:'') .' data-pcpa_id="' . $row['id'] . '" value="' . $row['pcpa'] . '"'. $cls .' />$</td></tr>';
											if(isset($row['current'])){
													$boolCurrentDeal = true;
											}
									}
					...
					if($i===0){
						...
						$cpi= 0;
						if(isset($arrDealType_cpi[$key])){
								/* if(!empty($arrDealType_cpl[$key])){
									$timelineTxt .='<td valign="top"><table class="subTable"><thead>
									<tr><td colspan="2" class="heading">'. strtoupper(lang('CPL')) .'</td></tr>
									<tr><td style="color:white;">' . strtoupper(lang('DATE')) . '</td><td style="color:white;">' . lang('DEPOSIT')  . '</td></tr></thead><tbody>';
								} */
								$isRowMarkedAsActive      =  $cpi === 0 ? 'style="color: green; font-weight: bold; width: 140px;"' : 'style="width: 140px;"';
								foreach($arrDealType_cpi[$key] as $k=>$row)
								{
									if($cpi==0){
										if(!empty($arrDealType_cpi[$key])){
											$timelineTxt .='<td valign="top" class="mainTD"><table class="subTable"><thead>';
											if(isset($row['default']) && $row['default']==true){
												$timelineTxt.='<tr><td colspan="2" class="heading">'. lang('Merchant Default Deal'). " - " . strtoupper(lang('CPI')) .'</td></tr>';
											}
											else{
												$timelineTxt.='<tr><td colspan="2" class="heading">'. strtoupper(lang('CPI')) .'</td></tr>';
											}
											$timelineTxt .= '<tr><td style="color:white;">' . strtoupper(lang('DATE')) . '</td><td style="color:white;">' . lang('COMMISSION')  . '</td></tr></thead><tbody>';
										}
									}
						...
						if(isset($arrDealType_cpc[$key])){
								...
								$isRowMarkedAsActive      =  $cpc === 0 ? 'style="color: green; font-weight: bold; width: 140px;"' : 'style="width: 140px;"';
								foreach($arrDealType_cpc[$key] as $k=>$row)
								{
									if($cpc==0){
										if(!empty($arrDealType_cpc[$key])){
			...
			$timelineTxt .= $boolDefaultDeal  ? '' : ($boolAtLeastOneRowExists
									  ? '<tr><td><input type="submit" class="historySubmitBtn" onclick="return false;" value="' . lang('Save changes') . '"/></td></tr>'
									  : '');
...
$(document).ready(function(){
	...
	$timelineTxt .= !$isAdmin ? '' : '<script>
										...
										$("[data-pcpa_id]").change(function() {
											var id     = $.trim($(this).attr("data-pcpa_id"));
                                                                                        var amount = $(this).val();
