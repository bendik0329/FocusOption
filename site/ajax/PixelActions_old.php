...
if (
    ...
    if (isset($arrAccountPixelParams[$_POST['param']])) {
		array_push($arrAccountPixelParams,$_POST['param']);
        //$arrAccountPixelParams[$_POST['param']]['value'] = empty($arrAccountPixelParams[$_POST['param']]['value']) ? 1 : 0;
    }
    
    $sql = "UPDATE `affiliates` "
            . "SET `accounts_pixel_params_replacing` = '" . mysql_real_escape_string(json_encode($arrAccountPixelParams)) . "' "
            . "WHERE `id` = " . mysql_real_escape_string($_POST['affiliate_id']) . ";";
    //echo $sql;
    ...
    $arrAffiliatePixelParams = mysql_fetch_assoc(function_mysql_query($sql,__FILE__,__FUNCTION__));
    $arrSalePixelParams      = json_decode($arrAffiliatePixelParams['sales_pixel_params_replacing'], true);
    ...
    $sql = "UPDATE `affiliates` "
            . "SET `sales_pixel_params_replacing` = '" . mysql_real_escape_string(json_encode($arrSalePixelParams)) . "' "
            . "WHERE `id` = " . mysql_real_escape_string($_POST['affiliate_id']) . ";";
