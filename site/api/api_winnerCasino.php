...
$casinoName 		= "winnercasino";
$casinoName 		= "magicRedCasino";
$defualtStartLimit = "&start=0&limit=2500";
$forceAddNonExistingAccount = true;
$productParam = '&product=';

$allPosibleMerchants = '1,17,18';
...
if ($api_url == '') {
	...
	echo ' >>> API_url found for: ' . $merchant['name'] . ' !<br>';
}
...
if ($justStats==0) { 
		...
		//	for ($camsID=0; $camsID < count($campaignIDs); $camsID++) {  // Irrelevant.
			...
			//	}
				...
				$url = $api_url . '?api_username=' . $api_user . '&api_password=' . $api_pass . '&module=playerssignups&command=getallbydate&startdate=' 
								. $scanDateFrom . '&enddate=' . $scanDateTo . $productParam.$casinoName.$defualtStartLimit;
				//die ($url);
			...
			echo '<br>url: ' . $url.'<br>';
				...
				foreach($xml[1] as $xml_line) {
					...
					$clientType  =  getTag('<clienttype>','<\/clienttype>',$xml_line) ; //$arrUsernameMatches[0][0];
					$db['merchant_id']      = getMerchantByType($clientType);
					...
					$chkDouble = mysql_fetch_assoc(mysql_query("SELECT id,type FROM data_reg WHERE merchant_id= '".$db['merchant_id']."' and trader_id='".$db['trader_id']."'"));
					...
					if (count($db) > 1) {
						if (!$exist && !$db['id']) {
							mysql_query(
								"INSERT INTO data_reg (couponName,merchant_id,rdate,ctag,affiliate_id,group_id,market_id,banner_id,profile_id,country,trader_id,trader_alias,type,freeParam,platform,status) 
								VALUES
								('".$db['couponName']."','".$db['merchant_id']."','".$db['rdate']."','".$db['ctag']."','".$db['affiliate_id']."','".$db['group_id']."','".$db['market_id']."','".$db['banner_id']."',
								'".$db['profile_id']."','".$db['country']."','".$db['trader_id']."','".$db['trader_alias']."','".$db['type']."','".$db['freeParam']."','".$db['platform']."',
								'".($frozen==0 ? '' : 'frozen')."')"
							) or die(mysql_error());
							...
							$subid =  GetOverrideDynamicTracker ($db['affiliate_id'], $db['uid']);
							...
							mysql_query("UPDATE data_reg SET type='".$db['type']."' WHERE id='".$db['id']."' and merchant_id= '".$db['merchant_id']."' ");
		...
		//	for ($camsID=0; $camsID < count($campaignIDs); $camsID++) {  // Irrelevant.
				{
				echo '<hr /><b>Connecting to Players Changes Database (Campaign ID:'.$campaignIDs[$camsID].') Page: <u>'.$page.'</u>...</b><br />';
				
			//	if (!$campaignIDs[$camsID]  ) {
			//		continue;
			//	}
				
				$url = $api_url . '?api_username=' . $api_user . '&api_password=' . $api_pass . '&module=playerschanges&command=getallbydate&startdate=' 
								. $scanDateFrom . '&enddate=' . $scanDateTo . $productParam.$casinoName.$defualtStartLimit;
				//die ($url);
				$url 		  = encodeDateTimeWithinUrl($url);
				$url 		  = convertToHttpsPostRequest($url);
			echo '<br>url: ' . $url.'<br>';
				...
				foreach($xml[1] as $xml_line) {
					...
					$db['trader_id'] = getTag('<playercode>','<\/playercode>',$xml_line);
					$db['rdate']     = date("Y-m-d", strtotime(getTag('<effectiveDate>','<\/effectiveDate>',$xml_line)));
					$db['status'] =  getTag('<freezingStatus>','<\/freezingStatus>',$xml_line);
					$db['product']      = getTag('<product>','<\/product>',$xml_line);
					...
					echo 'Frozen Changed for trader '.$db['trader_id'].' to '.$db['status'].'!<br>';
					$intInserted++;
					$qry = "UPDATE data_reg SET status='".$db['status']."' WHERE trader_id='".$db['trader_id']."' and merchant_id in (".$allPosibleMerchants . ")";
		...
		foreach ($xml[1] as $xml_line) {
			...
			$qqrry = "SELECT id,ctag FROM data_reg WHERE merchant_id= '".$db['merchant_id']."' and trader_id='".$db['trader_id']."' LIMIT 1";
			$chkTrader       = mysql_fetch_assoc(mysql_query($qqrry));
			$intProcessed++;
			
			if (!$chkTrader['id'] && $forceAddNonExistingAccount) {

				ForceAddAccount($xml_line,$db['merchant_id'],$frozen);
			}
			...
			$chkExist = mysql_fetch_assoc(mysql_query(
											"SELECT id,type,tranz_id FROM data_sales 
											WHERE merchant_id= '".$db['merchant_id']."' and  trader_id='".$db['trader_id']."' AND tranz_id='".$db['tranz_id']."'"
											));
			...
			$traderInfo = mysql_fetch_assoc(mysql_query("SELECT ctag,ftdamount,initialftdtranzid,merchant_id,trader_id FROM data_reg WHERE  merchant_id = ".$db['merchant_id'] ." and trader_id='".$db['trader_id']."'"));
			...
			if (count($db) > 1 AND !$tranzExist) {
				...
				mysql_query("INSERT INTO data_sales (merchant_id, rdate,ctag,affiliate_id,group_id,market_id,banner_id,profile_id,country,tranz_id,trader_id,trader_alias,type,amount,freeParam) 
							VALUES
							('".$db['merchant_id']."','".$db['rdate']."','".$db['ctag']."','".$db['affiliate_id']."','".$db['group_id']."','".$db['market_id']."',
		...
		if ($traderInfo['ftdamount']==0 && $db['type']=='deposit' && $traderInfo['initialftdtranzid']=='') {
				...
				if (!in_array($db['affiliate_id'], $sentAffiliates) && $db['type'] == "deposit") {
					$new_ftd = mysql_num_rows(mysql_query(
									"SELECT id FROM data_sales WHERE merchant_id= '".$db['merchant_id']."' and affiliate_id='".$db['affiliate_id']."' 
...
$url = $api_url . '?api_username=' . $api_user . '&api_password=' . $api_pass . '&module=playerstats&command=getallbydate&startdate=' 
	    . $scanDateFrom . '&enddate=' . $scanDateTo . $productParam.$casinoName.'&start='.($page*$RowsPerPage) . '&limit=' . ($RowsPerPage+ $page*$RowsPerPage);
...
// $url = $api_url . '?api_username=' . $api_user . '&api_password=' . $api_pass . '&module=playerstats&command=getallbydate&startdate=' 
			    // . $scanDateFrom . '&enddate=' . $scanDateTo . $productParam.$casinoName;
...
if ($justStats ==0) {
		...
		foreach($xml[1] as $xml_line) {
			...
			$intProcessed++;
			$chkTrader = mysql_fetch_assoc(mysql_query("SELECT id,ctag FROM data_reg WHERE merchant_id= '".$db['merchant_id']."'  and trader_id='".$db['trader_id']."' LIMIT 1"));
			...
			$chkExist = mysql_fetch_assoc(mysql_query(
				"SELECT id,type,tranz_id FROM data_sales 
				WHERE merchant_id= '".$db['merchant_id']."'  and trader_id='".$db['trader_id']."' AND type='".$db['type']."' AND tranz_id='".$db['tranz_id']."'"
			));
			...
			// Check cTag From Trader
			$traderInfo = mysql_fetch_assoc(mysql_query("SELECT ctag FROM data_reg WHERE merchant_id= '".$db['merchant_id']."'  and trader_id='".$db['trader_id']."'"));
