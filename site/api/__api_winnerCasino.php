...
$defualtStartLimit = "&start=0&limit=2000";
		...
		foreach($xml[1] as $xml_line) {
			...
			if (count($db) > 1) {
				if (!$exist && !$db['id']) {
					...
					$pixelurl = 'http://' . $_SERVER['HTTP_HOST'] . '/pixel.php?act=account&ctag='.$db['ctag'].'&trader_id='.$db['trader_id'].'&trader_alias='.$db['trader_alias'];
					//die ($pixelurl);
...
if ($traderInfo['ftdamount']==0 && $db['type']=='deposit' && $traderInfo['initialftdtranzid']=='') {
		...
		$pixelurl = 'http://' . $_SERVER['HTTP_HOST'] . '/pixel.php?act=deposit&ctag='.$db['ctag'].'&trader_id='.$db['trader_id'].'&tranz='.$db['tranz_id'].'&type='.$db['type'].'&currency='.$db['coin'].'&amount='.$db['amount'];
					//die ($pixelurl);
					echo file_get_contents($pixelurl);
