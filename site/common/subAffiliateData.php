...
/**
 *  Getting data for affiliate which have several sub-affiliates for selected period
 * @param $from
 * @param $to
 * @param $affiliateId
 * @param $isSubAffiliate
 * @return array
 */
function getDashboardStatsByAffiliateToPeriodReferrals($from, $to, $affiliateId, $isSubAffiliate = false)
{

    $sql = "SELECT id FROM affiliates WHERE refer_id = '".$affiliateId."'";

    $sqlResource = function_mysql_query($sql,__FILE__);
    $subAffiliates = [];
    while ($subAffiliatesItem = mysql_fetch_assoc($sqlResource)) {
        $subAffiliates[] = $subAffiliatesItem['id'];
    }
    $subAffiliates = implode(',', $subAffiliates);
    $sql = "SELECT 
          SUM(Clicks) as clicks, 
          SUM(Impressions) as impressions, 
          SUM(Leads) as leads,
          SUM(Demo) as demo,
          SUM(RealAccount) as real_account,
          SUM(FTD) as ftd,
          SUM(FTDAmount) as ftd_amount,
          SUM(Deposits) as deposits,
          SUM(DepositsAmount) as deposits_amount,
          SUM(Bonus) as bonus,
          SUM(Withdrawal) as withdrawal,
          SUM(ChargeBack) as charge_back,
          SUM(PNL) as pnl,
          SUM(Install) as cpi,
    ...
    if (!$isSubAffiliate) {
        $sql .= " FROM Dashboard WHERE AffiliateID IN (".$subAffiliates.") AND Date BETWEEN '".$from."' AND '".$to."'  ";
    } else {
        $sql .= " FROM Dashboard WHERE AffiliateID = ".(int)$affiliateId." AND Date BETWEEN '".$from."' AND '".$to."'  ";
    }

    return mysql_fetch_assoc(function_mysql_query($sql,__FILE__));
}

/**
 * Updated getting stats for sub-affiliates (if isSubAffiliate is true, that meant that $affiliate_id is sub-affiliate id instead of affiliate)
 * @param $from
 * @param $to
 * @param $affiliate_id
 * @param $isSubAffiliate // if true, affiliate_id value contain sub-affiliate id
 * @return array|int
 */
function _getSubAffiliateData($from, $to, $affiliate_id, $isSubAffiliate = false)
{
    global $set;
    $arrResult = [];

    // Configurable option in admin config for hide commission
    if ($set->hideSubAffiliation == 1){
        $arrResult['commission'] = 0;
        return $arrResult['commission'];
    }

    //lots (only for forex need to be)
    ...
    while ($lot = mysql_fetch_assoc($sqlResource)) {
        $totalLots += $lot['totalTurnOver'];
    }

    // Dashboard stats for affiliate
    $dashboardStats = getDashboardStatsByAffiliateToPeriodReferrals($from, $to, $affiliate_id, $isSubAffiliate);
    $arrResult['clicks'] = !empty($dashboardStats['clicks']) ? $dashboardStats['clicks'] : 0;
    $arrResult['views'] = !empty($dashboardStats['impressions']) ? $dashboardStats['impressions'] : 0;
    $arrResult['leads'] = !empty($dashboardStats['leads']) ? $dashboardStats['leads'] : 0;
    $arrResult['demo'] = !empty($dashboardStats['demo']) ? $dashboardStats['demo'] : 0;
    $arrResult['real'] = !empty($dashboardStats['real_account']) ? $dashboardStats['real_account'] : 0;
    $arrResult['ftd'] = !empty($dashboardStats['ftd']) ? $dashboardStats['ftd'] : 0;
    $arrResult['ftd_amount'] = !empty($dashboardStats['ftd_amount']) ? $dashboardStats['ftd_amount'] : 0;
    $arrResult['deposits'] = !empty($dashboardStats['deposits']) ? $dashboardStats['deposits'] : 0;
    $arrResult['deposits_amount'] = !empty($dashboardStats['deposits_amount']) ? $dashboardStats['deposits_amount'] : 0;
    $arrResult['bonus'] = !empty($dashboardStats['bonus']) ? $dashboardStats['bonus'] : 0;
    $arrResult['withdrawal'] = !empty($dashboardStats['withdrawal']) ? $dashboardStats['withdrawal'] : 0;
    $arrResult['chargeback'] = !empty($dashboardStats['charge_back']) ? $dashboardStats['charge_back'] : 0;
    $arrResult['volume'] = !empty($dashboardStats['volume']) ? $dashboardStats['volume'] : 0;

    // commissions for sub affiliates
    if (!$isSubAffiliate) {
        $sql = 'SELECT SUM(Commission) as commission FROM commissions  '
            . 'WHERE TYPE = "AFF" AND affiliateID = '.(int)$affiliate_id.' AND Date BETWEEN "' . $from . '" AND "' . $to . '" ';
    } else {
        $sql = 'SELECT SUM(Commission) as commission FROM commissions  '
            . 'WHERE TYPE = "AFF" AND subAffiliateID = '.(int)$affiliate_id.' AND Date BETWEEN "' . $from . '" AND "' . $to . '" ';
    }


    $arrResult['commission'] = mysql_fetch_assoc(function_mysql_query($sql,__FILE__))['commission'];

    $arrResult['pnl'] = $dashboardStats['pnl'];
    $arrResult['cpi'] = !empty($dashboardStats['cpi']) ? $dashboardStats['cpi'] : 0;

    $arrResult['lots'] = $totalLots;
            ...
            if (!empty($netDepositTransactions)) {
                foreach($netDepositTransactions as $trans) {
                    ...
                    if (floatval($amount<>0)  && !empty($trans[0]['rdate'])) {
                        ...
                        echo ($_GET['com']==2) ? 'com6  :  ' . $affiliate_id . " : "  . $thisComis.'<br>' : '';
