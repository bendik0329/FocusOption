...
//			$SESSION['affiliate_id'] = $affiliate_id;
	...
	if ($set->introducingBrokerInterface) {
		...
		$pageTitle = lang('Sub Affiliates') .' ' . lang('Report')   . ($affiliate_id>0 ? " " . lang('For') . ' #' . $affiliate_id : '') ;
			...
			<ul class="breadcrumb">
				<li><a href="'.($userlevel!=''?$userlevel."/":'/').'">'.lang('Dashboard').'</a></li>
				<li><a href="' . $set->uri . '" class="arrow-left">'.$pageTitle.'</a></li>
		...
		if($userlevel == "admin")
		$qry = "select * from affiliates where id in ( select distinct (id) as id from affiliates) and valid = 1";
		elseif($userlevel == "manager")
		$qry = "select * from affiliates where id in ( select distinct (id) as id from affiliates) and valid = 1". ($userlevel=='manager' ? " and group_id = " . $group_id : "");
		else 
			$qry = "";
		if($qry!==""){
				...
				$hasResults=true;
						...
						if($userlevel == ""){
							...
							// while ($merchantww = mysql_fetch_assoc($merchantqq)) {
								...
								$data = getSubAffiliateData($from,$to,$affiliateww['id'],$affiliate_id,'full',$userlevel);
								else
								$data = getSubAffiliateData($from,$to,$set->userInfo['id'],$affiliate_id,'full',"");
...
$query = "SELECT COUNT(id) FROM data_reg where merchant_id ='" .($ww['id'])."' and ".$where." type='real' AND rdate BETWEEN '".$month[$i]."' AND '".date("Y-m-d",strtotime("-1 Day",strtotime("+1 Month",strtotime($month[$i]))))." 23:59:59'";
		...
		$set->content .='<script src="js/highcharts.js"></script>
	...
	<script src="../js/unslider/dist/js/unslider.js"></script>
		...
		$set->content .= '<form style="margin-top:10px;" method="get">
			...
			'.($set->export ? '<td><div class="exportCSV" style="float:left"><a href="'.$set->uri.(strpos($set->uri,'?') ? '&' : '?').'excel=csv"><img border="0" src="images/excel.png" alt="'.lang('Export to CSV').'" title="'.lang('Export to CSV').'" align="absmiddle" /> <b>'.lang('Export to CSV').'</b></a></div>':'').'
		...
		$tableContentheader= '<div class="specialTableTitle" style="margin-top: 10px!important;">'.($userlevel == ""?lang('Sub Affiliates Preformance'):lang('Merchant Preformance')).'</div>';
	...
	case "del_banner":
		...
		die('<a href="'.$set->basepage.'?merchant_id='.$ww['merchant_id'].'">Go Back!</a>');
		break;
...
// All Affiliates
	...
	//$set->pageTitle = lang('Get Creative Code');
		$set->breadcrumb_title =  lang('Get Creative Code');
			...
			<ul class="breadcrumb">
				...
				<li><a href="'. $set->uri .'" class="arrow-left">'.lang('Get Creative Code').'</a></li>
				<li><a style="background:none !Important;"></a></li>
			</ul>';
	...
	case "add":
		...
		_goto($set->basepage.'?act=creative');
		break;
