...
$where = '';
    ...
    if ($browser_name) {
        $where .= " AND dr.browser_name='" . $browser_name . "' ";
    }
    
    if ($os_name) {
        $where .= " AND dr.os_name='" . $os_name . "' ";
    }
...
if($type == 'ftd' || $type == 'totalftd'){
    $tradersQuery = "SELECT rt.*, dr.ip, dr.os_ver, dr.os_name, dr.browser_ver, dr.browser_name, dr.device_name, dr.saleStatus as SaleStatusOriginal, dr.freeParam5 as freeParam5, aff.group_id as GroupID, td.status as tag_status, td.notes as tag_notes FROM ReportTraders rt INNER JOIN affiliates aff ON rt.AffiliateID = aff.id INNER JOIN data_reg dr ON dr.trader_id = rt.TraderID left JOIN traders_tag td ON td.trader_id = rt.TraderID AND td.valid = 1 WHERE 1=1 ".$where." ORDER BY RegistrationDate DESC";
}else{
    $tradersQuery = "SELECT rt.*, dr.ip, dr.os_ver, dr.os_name, dr.browser_ver, dr.browser_name, dr.device_name, dr.saleStatus as SaleStatusOriginal, dr.freeParam5 as freeParam5, aff.group_id as GroupID, td.status as tag_status, td.notes as tag_notes FROM ReportTraders rt INNER JOIN affiliates aff ON rt.AffiliateID = aff.id INNER JOIN data_reg dr ON dr.trader_id = rt.TraderID left JOIN traders_tag td ON td.trader_id = rt.TraderID AND td.valid = 1 WHERE RegistrationDate BETWEEN '".$from."' AND '".$to."' ".$where." ORDER BY RegistrationDate DESC";
}
...
$os_name_list_string = '';
$browser_name_list = [];
$browser_name_list_string = '';

$device_filter = function_mysql_query("SELECT os_name, browser_name  FROM data_reg WHERE rdate >= '".date('Y-m-d',(time()-(60*60*24*90)))."' AND os_name != '' AND browser_name != '' GROUP BY os_name, browser_name", __FILE__);
while ($device_filter_data = mysql_fetch_assoc($device_filter)) {
    $os_name_list[$device_filter_data['os_name']] = $device_filter_data['os_name'];
    $browser_name_list[$device_filter_data['browser_name']] = $device_filter_data['browser_name'];    
}

foreach($os_name_list as $key => $value){
    $os_name_list_string .= '<option value="'.$key.'" '.((!empty($os_name) && $os_name == $key)?'selected':'').' >'.$value.'</option>';
}

foreach($browser_name_list as $key => $value){
    $browser_name_list_string .= '<option value="'.$key.'" '.((!empty($browser_name) && $browser_name == $key)?'selected':'').' >'.$value.'</option>';
}
...
while ($traderItem = mysql_fetch_assoc($tradersData)) {
    ...
    $ftdUsers = '';
        ...
        if(!empty($traderItem['tag_status']) || !empty($traderItem['tag_notes'])){
            $traderItem['Commission'] = 0;
            $traderItem['AdminNotes'] = $traderItem['tag_status'];
            if(!empty($traderItem['tag_notes'])){
                $traderItem['AdminNotes'] .= ' - '.$traderItem['tag_notes'];    
            }
        ...
        $tradersDataQuery = function_mysql_query("select c.merchantID as merchant_id, c.affiliateID as affiliate_id, SUM(c.Commission) as commissions from commissions c where c.affiliateID = '".$traderItem['AffiliateID']."' AND c.merchantID = '".$traderItem['MerchantID']."' AND c.traderID = '".$traderItem['TraderID']."'", __FILE__);
        $tradersDataComms = mysql_fetch_assoc($tradersDataQuery);
        
        if(!empty($tradersDataComms['commissions'])){
            $traderItem['Commission'] = $tradersDataComms['commissions'];
        }

        $listReport .= '<td class="trader-alias" nowrap>' . $traderItem['TraderAlias'] . '</td>
                            ' . ($set->ShowEmailsOnTraderReportForAdmin ? '<td>' . $traderItem['Email'] . '</td>' : '' ) . '
                            ' . ($set->ShowPhonesOnTraderReportForAdmin ? '<td>' . $traderItem['Phone'] . '</td>' : '' ) . '
                            <td title="' . (date("d/m/Y", strtotime($traderItem['RegistrationDate']))) . '">' . (date("d/m/Y", strtotime($traderItem['RegistrationDate']))) . '</td>
                            ...
                            <td>' . $traderItem['ip'] . '</td>
                            <td nowrap>' . $traderItem['os_ver'].'</td>
                            <td nowrap>' . $traderItem['browser_ver'].'</td>
                            <td nowrap>' . $traderItem['device_name'].'</td>
                            ' . ($set->showDynamicFilters == 1 ?'<td></td>' : '' ) . '
                            <td><a href="/' . $userlevel . '/affiliates.php?act=new&id=' . $traderItem['AffiliateID'] . '" target="_blank">' . $traderItem['AffiliateID'] . '</a></td>
                            ...
                            <td>' . (!empty($traderItem['FTDAmount']) ? price($ftdAmount) : "" ) . '</td>
                            ...
                            <td>' . $traderItem['SaleStatusOriginal'] . '</td>
...
$set->content .= '<div class="normalTableTitle" style="width: 100%;">' . lang('Report Search') . '</div>
        ...
        ($set->showDynamicFilters == 1 ? '<td>' . lang($set->dynamicFilterTitle) . '</td>' : '') . '
						...
						<td><select name="country_id" style="width: 150px;"><option value="">' . lang('All') . '</option>' . getCountries($country_id) . '</select></td>
                                                <td><select name="os_name" style="width: 100px;"><option value="">' . lang('All') . '</option>' . $os_name_list_string . '</select></td>
                                                <td><select name="browser_name" style="width: 100px;"><option value="">' . lang('All') . '</option>' . $browser_name_list_string . '</select></td>
...
$tableStr = '<table class="table tablesorter mdlReportFields" border="0" cellpadding="0" cellspacing="0" id="traderTbl">
        ...
        . ( $displayForex == 1 ? '
						...
						<th  class="table-cell">' . lang('Country') . '</th>
                                                <th  class="table-cell">' . lang('IP') . '</th>
                                                <th  class="table-cell">' . lang('OS') . '</th>
                                                <th  class="table-cell">' . lang('Browser') . '</th>
                                                <th  class="table-cell">' . lang('Device') . '</th>
						' . ($set->showDynamicFilters == 1 ?
