...
$set->breadcrumb_title =  lang($pageTitle);
		...
		if ($merchant_id) {
            		...
            		$whereDashboard .= " AND MerchantID = '".$merchant_id."' ";
                }
                
		if ($affiliate_id) {
                    $where .= " AND AffiliateID = '".$affiliate_id."' ";
            		$whereDashboard .= " AND AffiliateID = '".$affiliate_id."' ";
                }
                
		if ($country_id) {
                    $where  .= " AND country_id='".$country_id."' ";
            		$whereDashboard .= " AND CountryID = '".$country_id."' ";
                }
	...
	/*	$sqlDashboard = "SELECT SUM(Clicks) as clicks, SUM(Impressions) as impressions, SUM(Install) as install, AffiliateID, MerchantID FROM Dashboard WHERE Date >= '".$from."' AND Date <= '".$to."' AND ".$whereDashboard." GROUP BY AffiliateID, MerchantID";
		$DashboardData = function_mysql_query($sqlDashboard,__FILE__);

		$dashboardAllData = [];

		while ($item = mysql_fetch_assoc($DashboardData)) {
            $dashboardAllData[$item['affiliateid'].'-'.$item['merchantid']]['clicks'] += $item['clicks'];
            $dashboardAllData[$item['affiliateid'].'-'.$item['merchantid']]['impressions'] += $item['impressions'];
            $dashboardAllData[$item['affiliateid'].'-'.$item['merchantid']]['install'] += $item['install'];
		}
	...
	*/
		...
		$sqlReportTraders = "SELECT AffiliateID, MerchantID, country, SUM(ReportTraders.Volume) as Volume, SUM(ReportTraders.WithdrawalAmount) as WithdrawalAmount,
        ...
        /*    $countryArray[$item['country']]['clicks'] = $dashboardAllData[$item['affiliateid'].'-'.$item['merchantid']]['clicks'];
            $countryArray[$item['country']]['views'] = $dashboardAllData[$item['affiliateid'].'-'.$item['merchantid']]['impressions'];*/

		/*	$countryArray[$item['country']]['clicks'] = $statsAllData[$item['country']]['clicks'];
			$countryArray[$item['country']]['views'] = $statsAllData[$item['country']]['impressions'];*/

        //    $countryArray[$item['country']]['cpi'] = $dashboardAllData[$item['affiliateid'].'-'.$item['merchantid']]['install'];
            $countryArray[$item['country']]['cpi'] = $InstallationsDataItems[$item['country']]['installations'];

            $countryArray[$item['country']]['merchant'] = $MerchantsDataItems[$item['MerchantID']];
