...
<a href="/'. $userlevel .'/reports.php?act=transactions&from='.date("d/m/Y", strtotime("-3 Years")).'&to='.date("d/m/Y").'&merchant_id='.$loopedMerchant_ID.'&trader_id='.$arrRes['trader_id'].'&type=deposit">'.$total_deposits.'</a>
                            ...
                            <td>'.(!empty($reason) ? $reason : ($hidePendingReason ? '' : lang('Pending'))).'</td>
                        </tr>';
			...
			<div style="background: #F8F8F8;">';
				$tableStr ='<table width="2600" class="tablesorter" border="0" cellpadding="0" cellspacing="0" id="traderTbl">
					<thead><tr>
						...
						<th>'.lang('Self Deposit').'</th>
						<th>'.lang('Total Next Deposits').'</th>
						<th>'.lang('Next Deposits').'</th>
						<th>'.lang('Total Deposits').'</th>
						<th>'.($type == "deposit" ? lang('Deposit Amount') : lang('Deposit Amount')).'</th>
						<th>'.lang('Volume').'</th>
