...
//			$SESSION['affiliate_id'] = $affiliate_id;
	...
	if ($set->introducingBrokerInterface) {
		...
		if($userlevel == "admin")
		$qry = "select * from affiliates where id in ( select distinct (refer_id) as id from affiliates where not refer_id =0) and valid = 1";
		elseif($userlevel == "manager")
		$qry = "select * from affiliates where id in ( select distinct (refer_id) as id from affiliates where not refer_id =0) and valid = 1". ($userlevel=='manager' ? " and group_id = " . $group_id : "");
		else 
			$qry = "";
		...
		$totalCPI=0;
			...
			/*	if($userlevel == "admin")
				...
				else if($userlevel == "managar")
					$sql = "SELECT id,username FROM affiliates WHERE valid = 1 AND refer_id = " . $affiliate_id  .  ($userlevel=='manager' ? " AND  group_id = " . $group_id : "");
				else {
        ...
        function getSubAffiliateTier($aff_id,$merchant_sub_aff_levels_array){
            $result = [];
            $cur_aff_tier = $merchant_sub_aff_levels_array[0][$aff_id];

            if(!empty($cur_aff_tier)){
                foreach ($cur_aff_tier as $key => $value) {
                    $t_val = end($value);

                    if($t_val > 0){
                       $result[$key] = 'L' . $key . ' - ' . round($t_val,4) . '%';
                    }
			...
			$affiliateData = function_mysql_query($sql,__FILE__);
			 $hasResults = false;
        if ($affiliate_id > 0 || ( $set->userInfo['id'] && empty($set->userInfo['userLevel']) )) {
		while ($affiliateww = mysql_fetch_assoc($affiliateData)) {
						...
						if($userlevel == ""){
							...
							// while ($merchantww = mysql_fetch_assoc($merchantqq)) {
								...
								if($userlevel == "admin" || $userlevel=="manager")
								    $data = _getSubAffiliateData($from, $to, $affiliateww['id']);
								else{
								    $data = _getSubAffiliateData($from, $to, $affiliateww['id'], true);
								}
                ...
                //var_dump($data);
								...
								if($userlevel == "admin" || $userlevel == "managar"){
								$listMerchants .= '<tr>
												...
												<td align="center">' . implode(' | ',getSubAffiliateTier($affiliateww['id'],$merchant_sub_aff_levels_array)) . '</td>
												<td align="center">'.$data['clicks'].'</td>
												'.($set->deal_cpi?'<td align="center">'.$data['cpi'].'</td>':'').'
												...
												<td align="center">'.price($data['chargeback']).'</td>
                                                                                                <td align="center">' . price($data["volume"]) . '</td>
        ...
        for ($fid = 0; $fid <= count($chartAffiliates) - 1; $fid++)
            $whereAffiliates[] = "affiliate_id='" . $chartAffiliates[$fid] . "'";
        if (count($whereAffiliates) > 0)
            $where = "(" . implode(" OR ", $whereAffiliates) . ") AND";
...
$query = "SELECT COUNT(id) FROM data_reg where merchant_id ='" .($ww['id'])."' and ".$where." type='real' AND rdate BETWEEN '".$month[$i]."' AND '".date("Y-m-d",strtotime("-1 Day",strtotime("+1 Month",strtotime($month[$i]))))." 23:59:59'";
	...
	<script src="'.$set->SSLprefix.'js/unslider/dist/js/unslider.js"></script>
		...
		$set->content .= '
			<table width="100%" border="0" cellpadding="4" cellspacing="5"><tr>
				...
				'.($set->deal_cpi?'<td class="dashStat">
					'.lang('Installation').'<br />
					<span style="font-size: 18px; font-weight: bold;">'.($totalCPI ? $totalCPI: '0').'</span>
				</td>':'').'
				<td class="dashStat">
					'.lang('Leads').'<br />
				...
				<td class="dashStat">
					' . lang('volume') . '<br />
					<span style="font-size: 18px; font-weight: bold;">' . price($totalVolume, 2) . '</span>
				</td>
				<td class="dashStat">
		...
		<thead>';
							...
							$tableContent .='<tr  class="table-row">
								<th class="table-cell">'.($set->introducingBrokerInterface ? lang('Introduce Broker ID') : lang('Affiliate ID')).'</th>
								<th class="table-cell">'.($set->introducingBrokerInterface ? lang('IB Username') : lang('Affiliate Username')).'</th>
								<th align="center" class="table-cell">' . lang('Tier Level') . '</th>
								<th align="center" class="table-cell">'.lang('Clicks').'</th>
								'.($set->deal_cpi?'<th align="center" class="table-cell">'.lang('Installation').'</th>':'').'
								<th align="center" class="table-cell">'.lang('Leads').'</th>
								<th align="center" class="table-cell">'.lang('Demo').'</th>
								<th align="center" class="table-cell">'.lang(ptitle('Accounts')).'</th>
								<th align="center" class="table-cell">'.lang('FTD').'</th>
								<th align="center" class="table-cell">'.lang('FTD Amount').'</th>
								<th align="center" class="table-cell">'.lang('Deposits').'</th>
								<th align="center" class="table-cell">'.lang('Deposits Amount').'</th>
								<th align="center" class="table-cell">'.lang('Bonus').'</th>
								<th align="center" class="table-cell">'.lang('Withdrawal').'</th>
								<th align="center" class="table-cell">'.lang('Chargeback').'</th>
                                                                <th align="center" class="table-cell"> ' . lang('Volume') . '</th>								
								<th align="center" class="table-cell">% '.lang('Your commission').'</th>
							</tr>';
							$tableContent .='
						</thead>
						<tfoot>
							<tr>
								...
								'.($set->deal_cpi?'<th align="center">'.$totalCPI.'</th>':'').'
								...
								<th align="center">' . price($totalVolume) . '</th>
						...
						'.$listMerchants.'
								...
								'. (allowView('af-impr',$deal,'fields') ? '
								<th align="center">' . lang('Tier Level') . '</th>
								': '').'
								'. (allowView('af-clck',$deal,'fields') ? '
								<th align="center">'.lang('Clicks').'</th>
								': '').'
								'. (allowView('af-instl',$deal,'fields') && $set->deal_cpi ? '
								<th align="center">'.lang('Installation').'</th>
								': '').'
								'. (allowView('af-lead',$deal,'fields') ? '
								...
								': '').'
                                                                <th align="center"> ' . lang('Volume') . '</th>
	...
	case "save_banner":
		...
		if (chkUpload('file')) {
			...
			$db['file'] = UploadFile('file','5120000','jpg,gif,swf,jpeg,png','','files/banners/');
			$exp=explode(".",$db['file']);
			$ext = strtolower($exp[count($exp)-1]);
            if ($ext == "swf")
                $db['type'] = "flash";
            else if ($ext == "jpg" OR $ext == "jpeg" OR $ext == "gif" OR $ext == "png")
                $db['type'] = "image";
            else
                $db['type'] = "link";
...
$preview = getBanner($db['file'],80);
        ...
        if ($db['type'] == "image")
            $srcFile = '<img border="0" src="' . $set->webAddress . $db['file'] . '" alt="" />';
$codelink = '<br />
<table>
	<tr>
...
$affiliateqq=function_mysql_query("SELECT id,username,first_name,last_name FROM affiliates WHERE valid='1' ORDER BY id ASC",__FILE__);
        while ($affiliateww = mysql_fetch_assoc($affiliateqq))
            $allAffiliates .= '<option value="' . $affiliateww['id'] . '" ' . ($affiliateww['id'] == $affiliate_id ? 'selected' : '') . '>[' . $affiliateww['id'] . '] ' . $affiliateww['username'] . ' (' . $affiliateww['first_name'] . ' ' . $affiliateww['last_name'] . ')</option>';
...
if ($affiliate_id) {
	$profileqq=function_mysql_query("SELECT id,name,url FROM affiliates_profiles WHERE affiliate_id='".$affiliate_id."' AND valid='1' ORDER BY id ASC",__FILE__);
            while ($profileww = mysql_fetch_assoc($profileqq))
                $allProfiles .= '<option value="' . $profileww['id'] . '" ' . ($profileww['id'] == $profile_id ? 'selected' : '') . '>[' . $profileww['name'] . '] ' . $profileww['url'] . ' (Site ID: ' . $profileww['id'] . ')</option>';
	}
...
// All Affiliates
					...
					<table width="100%"><tr>
						<td width="60%" align="left" valign="top">
							...
							'.($db['type'] == "image" || $db['type'] == "flash" ? '
								...
								<img style="max-width:100%;max-height:500px;" src="'. $db['file'].  '" alt=""/>':$preview).'
								<br /><br /><hr /><br />
								...
								<form method="get">
									...
									<table>
									<tr><td>'.lang('Choose Affiliate').':</td><td><select name="affiliate_id" onchange="form.submit();"><option value="">'.lang('Choose Affiliate').'</option>'.$allAffiliates.'</select></td></tr>
									<tr><td>'.lang('Choose Profile').':</td><td><select name="profile_id" onchange="form.submit();"><option value="">'.lang('Choose Profile').'</option>'.$allProfiles.'</select></td></tr>
									</table>
								</form>
	...
	case "add":
		...
		if (chkUpload('file')) {
			$db['file'] = UploadFile('file','5120000','jpg,gif,swf,jpeg,png','','files/sub_banners/');
			$exp=explode(".",$db['file']);
			$ext = strtolower($exp[count($exp)-1]);
            if ($ext == "swf")
                $db['type'] = "flash";
	...
	case "creative":
		...
		while ($ww=mysql_fetch_assoc($qq)) {
			...
			$allCreative .= '<tr '.($l % 2 ? 'class="trLine"' : '').'>
							<td>'.$ww['id'].'</td>
							<!--td><a href="javascript:void(0);" onclick="NewWin(\''.$set->basepage.'?act=edit_banner&id='.$ww['id'].'\',\'editbanner\',\'1000\',\'800\',\'1\');">Edit</a></td-->
		...
		$set->content .= '<form method="post" enctype="multipart/form-data">
						...
						<div align="left" style="padding-top: 10px; padding-bottom: 10px;">
							<table>
							<tr><td align="left" class="blueText"><b>' . lang('Choose Brand') 
						  . ':</td><td><select onchange="location.href=\'' . $set->basepage . '?act=creative&merchant_id=\'+this.value + hash;"><option value="">' 
