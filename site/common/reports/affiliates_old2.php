...
*/////
$StaticDataFetch = false;

if($StaticDataFetch){
$twoMonthsAgoDate = date("Y-m-d" ,strtotime("-2 months"));	
$twoMonthsAgoDateTM = strtotime(date("Y-m-d" ,strtotime("-2 months")));	

$fromTM = strtotime($from);
$toTM = strtotime($to);


if($fromTM <= $twoMonthsAgoDateTM and $toTM <= $twoMonthsAgoDateTM){
...
if($fromTM <= $twoMonthsAgoDateTM and $toTM >= $twoMonthsAgoDateTM){
	...
	$static_from = $from;
	$static_to = $twoMonthsAgoDate;
	
	$from  = date("Y-m-d" , strtotime('+1 day', $twoMonthsAgoDateTM));
	
	$to = $to;
	
}

$static_from .= " 00:00:00"; 
$static_to .= " 23:59:59"; 

//$from .= " 00:00:00"; 

echo " Static Data Dates : ". $static_from . " - " . $static_to . "<br/>";
echo " Live Data Dates : ". $from . " - " . $to . "<br/>";

include('common/AffiliatesStaticData.php');
}
...
//load static data
				...
				//$sql = "select * from affiliates_static_data where rdate between '" .   $static_from  . "' and '" . $static_to . "' and affiliate_id=" . $ww['id'] . " "
				...
				$sql = "SELECT affiliates.*, acr.campID FROM affiliates LEFT JOIN"
                    ...
                    while ($ww = mysql_fetch_assoc($qq)) {
                        ...
                        // while ($merchantww = mysql_fetch_assoc($merchantqq)) {
							...
							$totalTraffic  = [];
								if($StaticDataFetch){
								if(!empty($static_from) && !empty ($static_to)){
								$listData = getAllAffiliateStaticData($static_from, $static_to, $merchantww['id'], $ww['id']);
								...
								if(!empty($listData))
								{
									...
									foreach($listData as $key=>$row){
										...
										if ($groupMerchantsPerAffiliate == 0) {
											...
											$totalCPI = $row['cpi'];
											$totalLeadsAccounts = $row['lead'];
											$totalDemoAccounts = $row['demo'];
											$totalRealAccounts = $row['real'];
											$totalFTD = $row['ftd'];
											$totalQFTD = $row['qftd'];
											$totalDeposits = $row['deposite_accounts'];
											$totalFTDAmount = $row['ftd_amount'];
											$totalDepositAmount = $row['deposits'];
											...
											$totalImpressions += $row['views'];
											$totalClicks += $row['clicks'];
											//echo "Row Clicks : " . $totalClicks . "<br/>"; 
											...
											$totalFTD += $row['ftd'];
											$totalQFTD += $row['qftd'];
											$totalDeposits += $row['deposit_accounts'];
											$totalFTDAmount += $row['ftd_amount'];
											$totalDepositAmount += $row['deposites'];
											$totalVolume += $row['volume'];
											$totalBonus += $row['bonus'];
											$totalWithdrawal += $row['withdrawal'];
											$totalChargeBack += $row['chargeback'];
                            ...
                            $arrClicksAndImpressions     = getClicksAndImpressions($arrRange['from'], $arrRange['to'], $merchantww['id'], $ww['id']);
								...
								if (empty($allAffiliatesTraffic[$merchantww['id']]))
								
								$allAffiliatesTraffic[$merchantww['id']]     = getClicksAndImpressions($arrRange['from'], $arrRange['to'], $merchantww['id'], -1);
...
//								SELECT sb.affiliate_id, IFNULL(SUM(sb.views), 0) AS impressions, IFNULL(SUM(sb.clicks), 0) AS clicks FROM traffic AS sb INNER JOIN affiliates AS aff ON aff.id = sb.affiliate_id AND aff.valid = 1 INNER JOIN merchants AS merchants ON merchants.id = sb.merchant_id AND merchants.valid = 1 WHERE sb.type="traffic"  AND sb.merchant_id = 1 AND sb.rdate BETWEEN '2017-01-01' AND '2017-04-03 23:59:59' group by sb.affiliate_id
                        ...
                        if (!$needToSkipMerchant) {
                            ...
                            /* 
									...
									$sql = "select * from (SELECT tb1.status ,tb1.amount ,tb1.tranz_id , tb1.id, data_reg.group_id,data_reg.trader_id,data_reg.affiliate_id,data_reg.merchant_id,data_reg.banner_id,data_reg.profile_id, data_reg.initialftddate, tb1.type AS data_sales_type, tb1.rdate AS data_sales_rdate  FROM data_sales AS tb1 "
											. "INNER JOIN (select * from data_reg where  data_reg.type <> 'demo' and merchant_id = " . $merchantww['id'] . " AND affiliate_id=".$ww['id']."  ) AS data_reg ON tb1.merchant_id = data_reg.merchant_id AND tb1.trader_id = data_reg.trader_id "
			...
			//Sub Affiliate Commission
						...
						if($_GET['com'])	{
                                  ...
                                  /*   $sql = "SELECT id FROM payments_details WHERE status = 'canceled' AND affiliate_id = '" . $ww['id'] . "' ";
									...
									//echo "2. Static Text : " .  $totalClicksM . "<br/>";
							   ...
							   $showWebsite = !empty($ww['website']) && $ww['website']!='http://' && $ww['website']!='http://www.' ? true : false;
                                ...
                                $listReport .=  
                                    ...
                                    <td style="text-align: center;">' . $listGroups[$ww['group_id']] . '</td>
									  ...
									  if($_GET['com'])
							   {
								   echo " Commission 1: totalComs " . $totalComs . "<br/>";
							   } 
						...
						$totalImpressionsM += $totalImpressions;
						$totalClicksM += $totalClicks;
			...
			<table><tr>
				...
				'<script type="text/javascript">
					$(document).ready(function(){
						...
						$("input[name=\'affiliate_id\']").keyup(function(){
							var affid = $(this).val();
		...
		<div class="normalTableTitle" class="table">'.lang('Affiliate Report').'<span style="float:right"><img class="imgReportFieldsSettings" style="padding-top:6px;width:55%;cursor:pointer;" src="'.$set->SSLprefix.'images/settings.png"/></span></div>
			<div style="background: #F8F8F8;">';
				...
				$tableStr='<table class="table tablesorter mdlReportFields" border="0" cellpadding="0" cellspacing="0"  id="affiliates">
