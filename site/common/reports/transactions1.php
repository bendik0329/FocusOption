		...
		// $resourceMerchants = $resourceMerchanrs;
			...
			// } elseif ($type == 'deposit' || $type == 'withdrawal' || $type == 'bonus' || $type == 'alltransactions') {
				...
				if(strtolower($arrMerchantRow['producttype']) == "casino" || strtolower($arrMerchantRow['producttype']) == "sportbet" || strtolower($arrMerchantRow['producttype']) == "forex")
				{
							...
							$sql = "	SELECT ds.*"."	FROM data_stats AS ds WHERE 5=5 and ds.merchant_id = '" . $int_merchant_id."' and ".$globalWhere." rdate BETWEEN '".$from."' AND '".$to."' ".$where .
							...
							$sql = "	SELECT ds.*"."	FROM data_sales AS ds WHERE 5=5 and 1=1 and ds.type='volume' and ds.merchant_id = '" . $int_merchant_id."' and ".$globalWhere." rdate BETWEEN '".$from."' AND '".$to."' ".$where .
							"ORDER BY ds.trader_id ASC";
				}
			}
			else{
 ...
 $arrRes['amount'] =  strtolower($arrRes['salesType'])=='deposit' ? $arrRes['amount'] : $arrRes['amount']*-1;
                    ...
                    $listReport .= '
                        <tr>
							...
							<td>'.price($ww['turnover']).'</td>
							<td>'.price($ww['spread']).'</td>
							<td>'.price($ww['pnl']).'</td>
                        </tr>';
					...
					<tr>
						...
						<td>
							<select name="type" style="width: 110px;">
								...
								<option value="positions" '.($type == "positions" ? 'selected' : '').'>'.lang('Positions').'</option>
							</select>
						</td>
