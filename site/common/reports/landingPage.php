...
$pageTitle = lang('Landing Pages Report');
	$set->breadcrumb_title =  lang($pageTitle);
		...
		if ($merchant_id) {
                    ...
                    $where_banners .= " AND MerchantID='".$merchant_id."' ";
                }
	  ...
	  $where    .= empty($group_id) ? '' : ' AND group_id = ' . $group_id . ' ';
          $where_banners_join    .= empty($group_id) ? '' : ' AND aff.group_id = ' . $group_id . ' ';
	...
	// die ($sql);
		...
		if ($affiliate_id) {
                    ...
                    $where_banners .= " AND AffiliateID='".$affiliate_id."' ";
        }
		// clicks and impressions
		
		$sql = "select mcs.BannerID as banner_id, SUM(mcs.Clicks) as total_clicks,sum(mcs.Impressions) as total_views from merchants_creative_stats mcs RIGHT JOIN affiliates aff ON aff.id = mcs.AffiliateID " . $where_banners_join . " where " . $where_banners . " AND mcs.MerchantID > 0 and mcs.Date BETWEEN '" . $from . "' AND '" . $to . "' GROUP BY mcs.BannerID";
 ...
 //die ($sql);		
	...
	// Qualified
		...
		$newUrlArray = array();		 
			foreach($creativeArray as $data){
					if (empty($newUrlArray[$data['url']])) {
						...
						if ($set->deal_cpi) $newUrlArray[$data['url']]['cpi'] = 0;
						...
						if ($set->deal_cpi) $newUrlArray[$data['url']]['cpi'] += $data['cpi'];
		...
		$set->content   .= '
			...
			'.($set->export ? '<div class="exportCSV" style="float:left"><a style="cursor:pointer" onclick="$(\'#landingPageData\').tableExport({type:\'csvbig\',escape:\'false\',tableName:\''.  $filename .'\'});"><img border="0" src="'.$set->SSLprefix.'images/excel.png" alt="'.lang('Export to CSV').'" title="'.lang('Export to CSV').'" align="absmiddle" /> <b>'.lang('Export to CSV').'</b></a></div>':'').'
				<div class="exportCSV" style="float:left"><a style="cursor:pointer" onclick="$(\'#landingPageData\').tableExport({type:\'excelbig\',escape:\'false\',tableName:\''.  $filename .'\'});"><img border="0" src="'.$set->SSLprefix.'images/excel.png" alt="'.lang('Export to XLS').'" title="'.lang('Export to XLS').'" align="absmiddle" /> <b>'.lang('Export to XLS').'</b></a>
				</div>
				'. getFavoritesHTML() .'
			...
			//width 2400
				$tableStr='<table class="table tablesorter mdlReportFields" border="0" cellpadding="0" cellspacing="0" id="landingPageTbl">
					<thead><tr  class="table-row">
						<th style="text-align: left;"  class="table-cell">'.lang('URL').'</th>
								...
								submit: function(e,v,m,f){
									if(v){
										...
										if(name != ""){
											...
											saveReportToMyFav(name, \'Landing Page\',user,level,type);
										}
										else{
