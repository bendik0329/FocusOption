...
$bankdb = (isset($bankdb)?$bankdb:$_POST['bankdb']);
$paypaldb = (isset($paypaldb)?$paypaldb:$_POST['paypaldb']);
$webmoneyldb = (isset($webmoneyldb)?$webmoneyldb:$_POST['webmoneyldb']);
$Netellerdb = (isset($Netellerdb)?$Netellerdb:$_POST['Netellerdb']);
$Skrilldb = (isset($Skrilldb)?$Skrilldb:$_POST['Skrilldb']);
$Chinaunionpaydb = (isset($Chinaunionpaydb)?$Chinaunionpaydb:$_POST['Chinaunionpaydb']);
$epaymentsdb = (isset($epaymentsdb)?$epaymentsdb:$_POST['epaymentsdb']);
...
/* if ($paymentMethod == "bank") {
				...
				$db = $Chinaunionpaydb;
				 ...
				 else if (strtolower($paymentMethod) == "epayments") {
				if (!$epaymentsdb['pay_firstname']) $errors['pay_firstname'] = 1;
				if (!$epaymentsdb['pay_lastname']) $errors['pay_lastname'] = 1;
				if (!$epaymentsdb['pay_address1']) $errors['pay_address1'] = 1;
				// if (!$Skrilldb['pay_address2']) $errors['pay_address2'] = 1;
				if (!$epaymentsdb['pay_city']) $errors['pay_city'] = 1;
				// if (!$Skrilldb['pay_zip']) $errors['pay_zip'] = 1;
				if (!$epaymentsdb['pay_country']) $errors['pay_country'] = 1;
				if (!$epaymentsdb['pay_email']) $errors['pay_email'] = 1;
				if (!$epaymentsdb['preferredCurrency']) $errors['preferredCurrency'] = 1;
				// if (!$Skrilldb['pay_account']) $errors['pay_account'] = 1;
		...
		/*if ($errors) {
				...
				echo true;
                        ...
                        unset($db['username']);
                        $db['id'] = $_POST['affiliate'];
                        $db['ip'] = $_SERVER['REMOTE_ADDR'];
                        $db['paymentMethod'] = $paymentMethod;
                        $lastID = dbAdd($db, 'affiliates');
						echo true . "-" . $paymentMethod;
