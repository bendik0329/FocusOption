...
switch ($act) {
	...
	case "save_qualification": 
		...
		$qualify_amount = isset($_POST['db']['qualify_amount']) ? $_POST['db']['qualify_amount'] : 0 ; 
                ...
                $max_pnl_monthly_amount = isset($_POST['db']['max_pnl_monthly_amount']) ? $_POST['db']['max_pnl_monthly_amount'] : 0 ; 
                
                if($max_pnl_monthly_amount != getTraderDealsValue($api_affiliate_id,'max_pnl_monthly')){
			function_mysql_query ("INSERT INTO `traders_deals` "
                                . "(`rdate`, `admin_id`,`merchant_id`,`affiliate_id`,`trader_id`,`dealType`,`amount`,`valid`) VALUES "
                                . "('".date('Y-m-d H:i:s')."', '".$set->userInfo['id']."','0','".$api_affiliate_id."','0','max_pnl_monthly',".$max_pnl_monthly_amount.",'1')",__FILE__);
                }
                
                if($userLevel =='admin'){
                    $pnl_lower = isset($_POST['db']['pnl_lower']) ? $_POST['db']['pnl_lower'] : 0 ; 
                    if($pnl_lower != getTraderDealsValue($api_affiliate_id,'pnl_lower')){
                            function_mysql_query ("INSERT INTO `traders_deals` "
                                    . "(`rdate`, `admin_id`,`merchant_id`,`affiliate_id`,`trader_id`,`dealType`,`amount`,`valid`) VALUES "
                                    . "('".date('Y-m-d H:i:s')."', '".$set->userInfo['id']."','0','".$api_affiliate_id."','0','pnl_lower',".$pnl_lower.",'1')",__FILE__);
                    }
                }
	...
	_goto($set->SSLprefix.$set->basepage.'?act=new&id='.$api_affiliate_id.'&toggleTo=deal_type');
...
// die (1);
		...
		case "activate":
			$db=dbGet($id,$appTable);
			if ($valid=='' &&  $db['id']>0 && $_GET['method']=='delete')
				$valid = -1;
			
			if ($db['valid']=='-1') $valid=1; else $valid='-1';
			
			if (($db['valid']!=-1 && $valid==-1 && $db['id']>0) || $_GET['method']=='delete' && $db['id']>0){
				
				$mailCode = mysql_fetch_assoc(function_mysql_query('SELECT mailCode,title FROM mail_templates WHERE mailCode="AffiliateAccountIsNowDeleted" and valid=1',__FILE__,__FUNCTION__));
				sendTemplate($mailCode['mailCode'],$db['id']);
		   }
...
// die ('greger');		
	...
	case "add":
		...
		$qq = "SELECT id,valid, manager_private_note FROM ".$appTable." WHERE lower(username)='".$user."' AND id != '".$db['id']."'";
		// die ($qq);
		...
		if (isMustField('phone') AND !$db['phone']) $errors['phone'] = lang('Please fill out your phone');
                ...
                unset($_SESSION['new_affiliate_data']);
                
		if ($errors) {
                    
                        $_SESSION['new_affiliate_data'] = $db;
		...
		if(!empty($db['id'])){
            ...
            if(!empty($db['id']) && !empty($db['sub_com_level'])){

                foreach ($db['sub_com_level'] as $level => $amount) {

                    $merchant_sub_com_level = mysql_fetch_assoc(function_mysql_query("select * from merchants_affiliate_level where affiliate_id = " .$db['id']." AND level = ".(int)$level."  AND merchant_id = 0 ORDER BY rdate DESC LIMIT 1", __FILE__));

                    if($merchant_sub_com_level['amount'] != $amount){
                        $level_item = [
		...
		$qq = "select id,valid,group_id,manager_private_note from affiliates where id = " . $db['id'] . " limit 1;";			
		...
		if (!is_numeric($db['valid']) && $db['valid']!=''){
			...
			if ($currentAff['valid']==1 && $valid==0 && $db['id']>0){
		
				$mailCode = mysql_fetch_assoc(function_mysql_query('SELECT mailCode,title FROM mail_templates WHERE mailCode="AffiliateAccountIsNowPending" and valid=1',__FILE__,__FUNCTION__));
				sendTemplate($mailCode['mailCode'],$db['id']);
			}
	...
	if ($userLevel=='manager')
			...
			$db['group_id'] = isset($db['group_id']) && is_numeric($db['group_id'])  ? $db['group_id'] : $defaultGroupID;
			...
			if(empty($currentAff)){
				...
				sendTemplate($mailCode['mailCode'],$db['id']);
                        ...
                        if(!empty(($db['manager_private_note']) && !empty($db['id'])) || (empty($db['manager_private_note']) && !empty($db['id']) && !empty($currentAff['manager_private_note']))){
                            
                            $count_affiliate_private_notes = mysql_result(function_mysql_query("SELECT COUNT(id) FROM affiliate_private_notes WHERE affiliate_id='".$db['id']."'",__FILE__,__FUNCTION__),0);

                            $affiliate_private_notes = [];
                            $affiliate_private_notes['admin_id'] = $set->userInfo['id'];
                            $affiliate_private_notes['affiliate_id'] = $db['id'];                            
                            
                            if((int)$count_affiliate_private_notes === 0 && !empty($currentAff['manager_private_note'])){
                                $affiliate_private_notes['message'] = $currentAff['manager_private_note'];
                                $affiliate_private_notes_result = dbInsert($affiliate_private_notes,'affiliate_private_notes');
                                sleep(1);
                            }
                            
                            
                            if(!empty($db['manager_private_note'])){
                                $affiliate_private_notes['message'] = $db['manager_private_note'];
                                $affiliate_private_notes_result = dbInsert($affiliate_private_notes,'affiliate_private_notes');
                            }
			}
			...
			$newGroupID = $_POST['db']['group_id'];
			$db['group_id'] = $newGroupID!='' && is_numeric($newGroupID) ? $newGroupID : $db['group_id'];
...
echo '<br>'; */
			...
			if (!empty($db['id']) && isset($currentAff) && !empty($currentAff) && isset($db)){
/* 			
echo '<br>';
echo '<br>';
var_dump($lastID);
	...
	case "new":
		$set->content .='<style>
			...
			$errDesc = ucwords(str_replace('%20',' ',$explodedArray[1]));
                ...
                if (isset($_GET['error']) && !empty($_GET['error'])) {
                        if(!empty($_SESSION['new_affiliate_data'])){
                            $db = $_SESSION['new_affiliate_data'];
                        }
                       unset($_SESSION['new_affiliate_data']);
                       if(empty($id)){
                           unset($id);
...
// echo 		($etqry).'<br>';
	...
	var check_ids = new Array("username","password","first_name","last_name","mail","website");
	var names = new Array("'.lang('Username').'","'.lang('Password').'","'.lang('First Name').'","'.lang('Last Name').'","'.lang('E-mail').'","Website");
	...
	$set->content.='</script>';
        ...
        $merchant_sub_aff_levels = function_mysql_query("SELECT lv1.*  FROM merchants_affiliate_level lv1 INNER JOIN ( SELECT max(rdate) MaxDate, level FROM merchants_affiliate_level WHERE affiliate_id = ".$db['id']." AND merchant_id = 0 GROUP BY level ) lv2 ON lv1.level = lv2.level AND lv1.rdate = lv2.MaxDate WHERE lv1.affiliate_id = ".$db['id']." AND lv1.merchant_id = 0 order by lv1.rdate desc",__FILE__);
        ...
        $manager_private_note_recent ='';
        $manager_private_note_list = '<hr>';
        $manager_private_note_query = function_mysql_query("select an.*, adm.level AS level, concat(adm.first_name,' ',adm.last_name) AS name from affiliate_private_notes an LEFT join admins adm on adm.id = an.admin_id where an.affiliate_id =  ".(int)$db['id']." order by an.rdate desc");
        while($manager_private_note_item = mysql_fetch_assoc($manager_private_note_query)){
            if(empty($manager_private_note_recent)){
                $manager_private_note_recent = '<div style="border: 1px; background-color: #e5e5e5; padding: 10px;"><b style="font-size:16px;">'.lang('Recent Manager Private Note').'</b><div style="margin: 5px 0;">'.date('d-m-Y H:i:s',strtotime($manager_private_note_item['rdate'])).' by '.$manager_private_note_item['name'].' ('.$manager_private_note_item['level'].')</div><span style="font-size:14px;">'.$manager_private_note_item['message'].'</span><br><a href="#" id="show-modal-manager-notes" style="display: block; text-align: right;">'.lang('All notes').'<a></div>';
            }
            $manager_private_note_list .= '<br><div style="margin: 5px 0; text-align: left;">'.date('d-m-Y H:i:s',strtotime($manager_private_note_item['rdate'])).' by '.$manager_private_note_item['name'].' ('.$manager_private_note_item['level'].')</div><div style="font-size:14px; text-align: left;">'.$manager_private_note_item['message'].'</div><br><br><hr>';
        }
        
        if(empty($manager_private_note_recent)){
            $manager_private_note_recent = '<div style="border: 1px; background-color: #e5e5e5; padding: 10px;"><b style="font-size:16px;">'.lang('Recent Manager Private Note').'</b><div style="margin: 5px 0;"><span style="font-size:14px;">'.$db['manager_private_note'].'</span></div></div>';
        }
											...
											$set->content .='<td align="left">
												<div id="dd" class="wrapper-dropdown-3" tabindex="1">
													...
													<ul class="dropdown_new">
														...
														<li><a href="#" data-valid = 0>'. lang('Pending') .'</a></li>
...
$set->content .= '				
											...
											<input type="radio" name="db[valid]" '.($db['valid']==0? 'value=0 checked' : 'value=0').' />'.lang('Pending').'</td>-->
	...
	$set->content .='
											...
											<td align="left"><input type="text" id="website" name="db[website]" value="'.$db['website'].'" style="width: 280px;" />'.(isMustField('website') ? ' <span class="required">*</span>' : '').'</td>
										</tr><tr>
										<td colspan="3" height="5"></td>
										</tr>
...
';
								...
								<td width="48%" align="left" valign="top">
									...
									if($id){
										...
										$set->content.='</tr><tr>
											...
											<td colspan="2" align="left"><div class="exportCSV" style="margin: 0;"><a href="javascript:void(0);" onclick="simpleConfirmation(\''.lang('Are you sure you want to reset the pasword?').'\',\''.$set->basepage.'?act=send_password&id='.$db['id'].'\');">'.lang('Reset Password').'</a></div></td>
										</tr><tr>
											<td colspan="2" height="5"></td>
										</tr><tr>
									...
									$set->content.='<tr>
										...
										<tr>
											<td align="left" class="blueText">
											<div style="float:left;padding-top:5px;padding-right:10px;min-width:100px">'.lang('Terms & Condition Accepted').'</div></td>
											<td><label class="switch"><input type="checkbox" name="terms" class="terms"  value=1 checked disabled><div class="slider round"></div></label></td>
										</tr>
										<tr>
											<td align="left" class="blueText">
										...
										<tr>
											...
											'.($userLevel=='admin' ? '<td align="left"><select name="db[group_id]" style="width: 242px;"><option value="0">'.lang('General').'</option>'.listGroups($db['group_id']).'</select>&nbsp;&nbsp;<a href="admin/groups.php" target="_blank" class="blueText" style="text-decoration:underline;">'. lang("Add New") .'</a>
											' : '<td align="left">' . listGroups($db['group_id'],1) ).'
											</td>
										...
										$set->content .= ($db['id'] ? '<tr>
											...
											<td align="left">
												<select style="width: 242px;" onchange="simpleConfirmation(\''.lang('Are you sure you want to send this mail?').'\',\''.$set->SSLprefix.$set->basepage.'?act=send_mail&affiliate_id='.$db['id'].'&mailCode=\'+this.value);">
										...
										$set->content .= '<tr>
											...
											<td align="left">
                                                                                            '.$manager_private_note_recent.'
                                                                                            <textarea  cols="80" rows="10" name="db[manager_private_note]"  style="width:97%;height:100px; text-align: left;"></textarea>
                                                                                            
                                                                                            <div data-remodal-id="modal-manager-notes">
                                                                                                <button data-remodal-action="close" class="remodal-close"></button>
                                                                                                <h1>'.lang('Manager Private Note').'</h1>
                                                                                                '.$manager_private_note_list.'
                                                                                                <button data-remodal-action="cancel" class="remodal-cancel">'.lang('Close').'</button>
                                                                                            </div>                                                                                            
											...
											$.prompt("'.lang('Are you sure you want to').' "+ type +" '. lang('this account?').'", {
												...
												submit: function(e,v,m,f){
													if(v){
														...
														var url = "'.$set->SSLprefix.$userLevel .'/affiliates.php?act=activate&id='. $db['id'] .'&method=" +  type ;
			...
			if ($id) {
				...
				foreach ($merchants as $merchantww){
					...
					// nirs fix 06/1/2015
					$min_cpaAmount = getLatestDeal($merchantww['id'],$id,'min_cpa');

					$cpaAmount = getLatestDeal($merchantww['id'],$id,'cpa');

					$dcpaAmount = getLatestDeal($merchantww['id'],$id,'dcpa');
						
					$revenueAmount = getLatestDeal($merchantww['id'],$id,'revenue');
					
					$revenueSpreadAmount = getLatestDeal($merchantww['id'],$id,'revenue_spread');
					
					$lotsAmount = getLatestDeal($merchantww['id'],$id,'lots');
				
					$cplAmount = getLatestDeal($merchantww['id'],$id,'cpl');

					$pnlAmount = getLatestDeal($merchantww['id'],$id,'pnl');
				
	             	$cpcAmount = getLatestDeal($merchantww['id'],$id,'cpc');				

	             	$cpmAmount = getLatestDeal($merchantww['id'],$id,'cpm');		
					
					$positionsRevShareAmount = getLatestDeal($merchantww['id'],$id,'positions_rev_share');
					
					$cpiAmount = getLatestDeal($merchantww['id'],$id,'cpi');
					
					$listMerchantsDealTypeSection .= '<tr '.($l % 2 ? 'class="trLine"' : '').'>
							<td align="center" style="width:40px"><label class="switch"><input type="checkbox" name="activeMerchants[]" onchange="checkVal(this)" value="'.$merchantww['id'].'" '.(@in_array($merchantww['id'],$merchantList) ? 'checked' : '').' /><div class="slider round"></div></label></td>
							<td align="left"><input type="hidden" name="deal_main_merchant[]"  value="'.$merchantww['id'].'" /><b>'.$merchantww['name'].'</b></td>
					</tr>';
					...
					$listDealType .= '<tr '.($l % 2 ? 'class="trLine"' : '').'>
									...
									<td>'. $set->currency .' <input class="dealType1'.$l.' cpa'.$l.'" type="text" name="deal_cpa[]" id="deal_cpa_'.$l.'" data-value="'.$cpaAmount.'" value="'.$cpaAmount.'"  style="width: 80px; text-align: center;" /></td>
                                                                        <td align="center"><a href="#" class="cpacg_update" ref="'.$merchantww['id'].'"><span class="ui-icon ui-icon-document"></span></a></td>
									...
									($isOneOfMerchantsForex && $set->deal_revshare_spread==1 ? ( 'forex'  == strtolower($merchantww['producttype']) ? '<td>% <input class="dealType1'.$l.' rev_spread'.$l.'" type="text" name="deal_revenue_spread['.$l.']" data-value="'.$revenueSpreadAmount.'" value="'.$revenueSpreadAmount.'" id="fieldClear" style="width: 80px; text-align: center;" /></td>' : '<td></td>'):"").
								...
								$dealsFunction .= '<script type="text/javascript">
									$(document).ready(function(){
							...
							//advancedDeals
						$min_cpa_deals = getLatestAdvancedDeal($merchantww['id'],$id,'min_cpa');
						
						$cpa_deals = getLatestAdvancedDeal($merchantww['id'],$id,'cpa');
						
						$dcpa_deals = getLatestAdvancedDeal($merchantww['id'],$id,'dcpa');

						$cpl_deals = getLatestAdvancedDeal($merchantww['id'],$id,'cpl');
						
						$cpi_deals = getLatestAdvancedDeal($merchantww['id'],$id,'cpi');
						
						foreach($min_cpa_deals as $k=>$deal){
							$geo = $deal['geo']==''?'WW':$deal['geo'];
							$advancel_deals[$deal['merchant_id']][$geo]['merchant_id'] = $deal['merchant_id'];
							$advancel_deals[$deal['merchant_id']][$geo]['geo'] = $geo;
							$advancel_deals[$deal['merchant_id']][$geo]['min_cpa'] = $deal['amount'];
							$advancel_deals[$deal['merchant_id']][$geo]['deal_mincpa_id'] = $deal['id'];
						}
						foreach($cpa_deals as $k=>$deal){
							$geo = $deal['geo']==''?'WW':$deal['geo'];
							$advancel_deals[$deal['merchant_id']][$geo]['merchant_id'] = $deal['merchant_id'];
							$advancel_deals[$deal['merchant_id']][$geo]['geo'] = $geo;
							$advancel_deals[$deal['merchant_id']][$geo]['cpa'] = $deal['amount'];
							$advancel_deals[$deal['merchant_id']][$geo]['deal_cpa_id'] = $deal['id'];
						}
						foreach($dcpa_deals as $k=>$deal){
							$geo = $deal['geo']==''?'WW':$deal['geo'];
							$advancel_deals[$deal['merchant_id']][$geo]['merchant_id'] = $deal['merchant_id'];
							$advancel_deals[$deal['merchant_id']][$geo]['geo'] = $geo;
							$advancel_deals[$deal['merchant_id']][$geo]['dcpa'] = $deal['amount'];
							$advancel_deals[$deal['merchant_id']][$geo]['deal_dcpa_id'] = $deal['id'];
						}
						foreach($cpi_deals as $k=>$deal){
							$geo = $deal['geo']==''?'WW':$deal['geo'];
							$advancel_deals[$deal['merchant_id']][$geo]['merchant_id'] = $deal['merchant_id'];
							$advancel_deals[$deal['merchant_id']][$geo]['geo'] = $geo;
							$advancel_deals[$deal['merchant_id']][$geo]['cpi'] = $deal['amount'];
							$advancel_deals[$deal['merchant_id']][$geo]['deal_cpi_id'] = $deal['id'];
						}
						foreach($cpl_deals as $k=>$deal){
							$geo = $deal['geo']==''?'WW':$deal['geo'];
							$advancel_deals[$deal['merchant_id']][$geo]['merchant_id'] = $deal['merchant_id'];
							$advancel_deals[$deal['merchant_id']][$geo]['geo'] = $geo;
							$advancel_deals[$deal['merchant_id']][$geo]['cpl'] = $deal['amount'];
							$advancel_deals[$deal['merchant_id']][$geo]['deal_cpl_id'] = $deal['id'];
						}
					
				}
			
				if(!empty($advancel_deals)){
					$lll = 0;
						foreach($advancel_deals as $key=>$deals){
							foreach($deals as $b=>$row){
							$listAdvDeals .= '<tr '.($lll % 2 ? 'class="trLine"' : '').'>
															<td align="left"><!--input type="hidden" name="deal_merchant_geo[]" value="'.$merchantww['id'].'" /><b>'.$merchantww['name'].'</b-->
															<select name="deal_merchant_geo[]" style="width:150px">
															'. listMerchants($row['merchant_id']) .'
															</select>
															</td>
															<td><select style="width:150px;" name="country[]"><option value="worldwide" '.($row['geo']=='WW' ? 'selected' :'').'>'. lang('WORLDWIDE') .'</option>'. getCountries($row['geo']) .   '</select></td>
															<td>'. $set->currency .' <input class="dealType1_geo deal_min_cpa_geo minCpa_geo" data-amount_id="'.$row['deal_mincpa_id'].'" id="minCpa_geo" type="text" name="deal_min_cpa_geo[]" data-value="'.$row['min_cpa'].'" value="'.$row['min_cpa'].'"  style="width: 80px; text-align: center;" /></td>
															<td>'. $set->currency .' <input class="dealType1_geo cpa_geo" type="text" name="deal_cpa_geo[]" data-amount_id="'.$row['deal_cpa_id'].'" id="deal_cpa_geo" data-value="'.$row['cpa'].'" value="'.$row['cpa'].'"  style="width: 80px; text-align: center;" /></td>
															<td>% <input class="dealType1_geo  dcpa_geo" type="text" name="deal_dcpa_geo[]" data-value="'.$row['dcpa'].'" data-amount_id="'.$row['deal_dcpa_id'].'" value="'.$row['dcpa'].'" id="fieldClear" style="width: 80px; text-align: center;" /></td>'.
															($set->deal_cpi==1 ? '<td>'. $set->currency .' <input class="dealType1_geo cpi_geo" type="text" name="deal_cpi_geo[]" data-amount_id="'.$row['deal_cpi_id'].'" data-value="'.$row['cpi'].'" value="'.$row['cpi'].'" id="fieldClear" style="width: 80px; text-align: center;" /></td>' :'')
															.($set->deal_cpl ? '<td>'. $set->currency .'<input class="dealType2_geo cpl_geo" type="text" name="deal_cpl_geo[]" data-amount_id="'.$row['deal_cpl_id'].'" data-value = "'. $row['cpl'] . '" value="'.$row['cpl'].'" id="fieldClear" style="width: 80px; text-align: center;" /></td>' : '').'
															<!--td style="float:left !important;"><input type="submit" name="delete_new" value="'.lang('Delete').'" style="display:none"/></td-->
													</tr>';
                                            $lll++;
							}
						}
...
/* 								  
					$qry = "SELECT id,tier_amount,CONVERT(SUBSTRING_INDEX( tier_amount,  '-', 1 ),UNSIGNED INTEGER) as tierorder,tier_pcpa,amount,merchant_id , tier_type from (

SELECT deal1.*
FROM affiliates_deals deal1 LEFT JOIN affiliates_deals deal2
 ON (deal1.tier_amount = deal2.tier_amount AND deal1.rdate < deal2.rdate)
WHERE  deal1.valid=1  " .(isset($tier_merchant)&&$tier_merchant!=""?' and deal1.merchant_id= ' . $tier_merchant :' and deal1.merchant_id= ' . $defaultTierMer)  .$networkWheremid. " and deal2.id IS NULL and deal1.affiliate_id = '".$id."' and deal1.tier_type='ftd_amount' and deal1.dealType = 'tier'
...
die ($qry);
				...
				$testParam = 4;
				$tierDealActiveDeals = getCurrentActiveTierDeal($defaultTierMer, $id , $networkWheremid,$testParam);
					
					
					foreach  ($tierDealActiveDeals as $takeww) {


						if ($takeww['amount']==0 && $takeww['tier_pcpa']==0){
							continue;
						}
					...
					$(".allproductdeals").keyup(function(e){
						...
						.
																	...
																	submit: function(e,v,m,f){
																		...
																		if(type=="regulareDeals"){
																			if($("#min_cpa_global").val() == ""){
																				...
																				l = 1; 
																				$("input[name=\'deal_cpa_geo[]\']").each(function(){
																						if($("#deal_cpa_geo_"+l).val() !=""){
																							 if($("#minCpa_geo_"+l).val() == ""){
																								 $(".minCpa_geo"+l).addClass("requiredError");
																								 $(".displayErrs_geo").html("'.lang('Minimum Deposit is required in case of CPA.').'");
																								 tiererrors = true;
																							 }
																						  }
																						  l++;
							...
							<div id="deal_type" data-tab="deal_type" class="config_tabs" style="padding-bottom:20px;display:none;">
                             <div class="normalTableTitle" style="cursor: pointer;">'.lang('Merchants').'</div>
									<form action="'.$set->SSLprefix.$set->basepage.'" method="post" id="formDealType2">
										<table class="normal tblDetails" width="100%" border="0" cellpadding="3" cellspacing="0">
											<thead>
												<td style="text-align: left;width:40px">'.lang('Active').'</td>
												<td style="text-align: left;">'.lang('Merchant').'</td>
											</thead>
											<tbody>
													'.
													$listMerchantsDealTypeSection
													.'
											</tbody>
										</table>
									</form>
									<script>
										function checkVal(e){
												if($(e).is(":checked")){
													...
													var strAjaxAddr = "' . $_SERVER['SERVER_HOST'] . '/ajax/updateAffiliateMerchants.php";
													$.post(strAjaxAddr, { id : $(e).val(),checked:1,affiliate_id:'. $id .' }, function(res) {
													...
													var strAjaxAddr = "' . $_SERVER['SERVER_HOST'] . '/ajax/updateAffiliateMerchants.php";
													$.post(strAjaxAddr, { id : $(e).val(),checked:0,affiliate_id:'. $id .' }, function(res) {
										...
										/* $(document).ready(function(){
											$("input[name="activeMerchants[]"]").on("change",function(){
												$(this).is(":checked")){
                                                        ...
                                                        <script>
                                                            $("#show-modal-manager-notes").on("click",function(e){
                                                                e.preventDefault();
                                                                var modal_manager_notes = $("[data-remodal-id=modal-manager-notes]").remodal();
                                                                modal_manager_notes.open();
                                                            });
                                                        </script>

                                                         <script src="/admin/js/country_group.js?v=3"></script>
                                                         
                                                            <div id="modal-cpacg-list" data-remodal-id="modal" ref="'. $id .'">
                                                                <button data-remodal-action="close" class="remodal-close"></button>
                                                                <h1>'.lang('CPA by countries group').'</h1>
                                                                <div class="cpa-content"><p style="text-align center;"><img src="/images/ajax-loader_big.gif"/></p></div>
                                                                <button data-remodal-action="cancel" class="remodal-cancel">'.lang('Close').'</button>
                                                            </div>
							 ...
							 <div id="tab-container" class="tab-container">
								 <ul class="etabs">
								   ...
								   '.($set->deal_tier?'<li class="tab" id="tab-2"><a href="#tabs1-tierDeals">'. lang('Tier Deals') .'</a></li>':'').'
								    '.($set->deal_geoLocation==1?'<li class="tab" id="tab-1"><a href="#tabs3-advancedDeals">'. lang('Advanced Deals') .'</a></li>':'').'
								 </ul>
												...
												<table class="normal tblDetails" width="100%" border="0" cellpadding="3" cellspacing="0">
													...
													<tr>
														...
														<td>'.lang('CPA').' <span class="dcpa_max" style="display:none">/ ' . lang('DCPA - Max') .'</span></td>
                                                                                                                <td>'.lang('CPA by countries group').'</td>
														<td>'.lang('DCPA').'</td>' 	. 
													...
													</tr>'.$listDealType . $dealsFunction.'</tfoot>
												</table>'.
												($userLevel =='admin' || ($userLevel =='manager' && $set->AllowDealChangesByManager==1) ?
																			'
																				<div align="right" style="padding-top: 20px;">
																				'.($set->deal_tier?'<input type="button" value="'.lang('Save').'"  onclick="checkTierDeal(\'regulareDeals\');"/>':'<input type="submit" value="'.lang('Save').'""/>') :'').'
																						<br />
																				</div>
								   ...
								   '.($set->deal_tier?'<div id="tabs1-tierDeals">
															  ...
															  <div style="font-size: 10px; padding: 5px;float:left;">
                                                                ...
                                                                <script type="text/javascript">
																	...
																	$(document).ready(function(){
																		 $("#form_tier_deal_type").on("submit",function(){
																				matched = true;
																				$("input[name=\'deal_tier_amount[]\'],input[name=\'tier_amount[]\']").each(function(k,v){
																					tier_deal_range = $(v).val();
																					if(tier_deal_range!=""){
																						if(tier_deal_range.match(/^[0-9]+-[0-9]+$/)){
																							$(this).css("border","1px #CECECE solid");
																						}
																						else{
																							matched = false;
																				...
																				if(matched === false){
																					$.prompt("'.lang('Please fix the highlighted deposit range. <br/> <i>(e.g. 1-100)</i>').'", {
																							top:200,
																							title: "'. lang('Edit Affiliate') .'",
																							buttons: { "'.lang('Ok').'": true},
																							submit: function(e,v,m,f){
								  ...
								  '.($set->deal_geoLocation==1?'<div id="tabs3-advancedDeals">
											<div id="tab_3" style="width: 100%; background: #F8F8F8;">
												<form action="'.$set->SSLprefix.$set->basepage.'" method="post" id="formGeoDealType">
												<input type="hidden" name="act" value="' . ($userLevel =='admin' || ($userLevel =='manager' && $set->AllowDealChangesByManager==1) ? 'save_geo_deal' : 'disable'  ) .'" />
												<input type="hidden" name="affiliate_id" value="'.$id.'" />
												<input type="hidden" name="adv_deal_count" id="adv_deal_count" value="1" />
												<table class="normal tblDetails" width="100%" border="0" cellpadding="3" cellspacing="0">
													<thead>
													<tr><td align="left"colspan="10" class="displayErrs_geo"></td></tr>
													<tr>
														<td style="text-align: left;width:150px">'.lang('Merchant').'</td>
														<td style="text-align: center;width:150px">'.lang('Country').'</td>
														<td style="width:100px">'.lang(ptitle('Min. Deposit')).' <span class="dcpa_min" style="display:none"> / ' . lang('DCPA - Min') .'</span></td>
														<td style="width:100px">'.lang('CPA').' <span class="dcpa_max" style="display:none">/ ' . lang('DCPA - Max') .'</span></td>
														<td style="width:105px">'.lang('DCPA').'</td>' 	. 
														($set->deal_cpi==1 ? '<td style="width:100px">'.lang('CPI').'</td>' : '') . 
														($set->deal_cpl ? '<td style="width:100px">'.lang('CPL').'</td>' : '').'
														<td style="width:150px"></td>
													</tr></thead>
													<tfoot class="adv_alldeals">
													'.$listAdvDeals.'
													'.($listAdvDeals == "" ?'
													<tr>
															<td align="left"><!--input type="hidden" name="deal_merchant_geo[]" value="'.$merchantww['id'].'" /><b>'.$merchantww['name'].'</b-->
															<select name="deal_merchant_geo[]" style="width:150px">
															<option value="">'. lang('Choose Merchant') .'</option>
															'. listMerchants() .'
															</select>
															</td>
															<td><select style="width:150px;" name="country[]"><option value="worldwide" selected>'. lang('WORLDWIDE') .'</option>'. getCountries('') .   '</select></td>
															<td>'. $set->currency .' <input class="dealType1_geo deal_min_cpa_geo minCpa_geo" id="minCpa_geo" type="text" name="deal_min_cpa_geo[]" data-value="" value=""  style="width: 80px; text-align: center;" /></td>
															<td>'. $set->currency .' <input class="dealType1_geo cpa_geo" type="text" name="deal_cpa_geo[]" id="deal_cpa_geo" data-value="" value=""  style="width: 80px; text-align: center;" /></td>
															<td>% <input class="dealType1_geo  dcpa_geo" type="text" name="deal_dcpa_geo[]" data-value="" value="" id="fieldClear" style="width: 80px; text-align: center;" /></td>'.
															($set->deal_cpi==1 ? '<td>'. $set->currency .' <input class="dealType1_geo cpi_geo" type="text" name="deal_cpi_geo[]" data-value="" value="" id="fieldClear" style="width: 80px; text-align: center;" /></td>' :'')
															.($set->deal_cpl ? '<td>'. $set->currency .'<input class="dealType2_geo cpl_geo" type="text" name="deal_cpl_geo[]" data-value = "" value="" id="fieldClear" style="width: 80px; text-align: center;" /></td>' : '').'
															<!--td style="float:left !important;"><input type=\'button\' name=\'delete_new\' value=\''.lang('Delete').'\' style="display:none"></td-->
													</tr>':'').'
													</tfoot>
												</table>'.
												($userLevel =='admin' || ($userLevel =='manager' && $set->AllowDealChangesByManager==1) ?
																			'
																				<div align="right" style="padding-top: 20px;">
																				<input type="button" value="'.lang('Add New').'" onClick="addNewAdvancedDeal()"/>&nbsp;&nbsp;'.($set->deal_tier?'<input type="button" value="'.lang('Save').'"  onclick="checkTierDeal();"/>':'<input type="submit" value="'.lang('Save').'""/>') :'').'
																						<br />
																				</div>
								  ...
								  var row1 = 0;
										function addNewAdvancedDeal(){
											
											$("#adv_deal_count").val(parseInt($("#adv_deal_count").val())+1);
											
											var lastRow = $("tfoot.adv_alldeals tr:last").html();
											
											var brand = country = min_cap =  cpa = dcpa = cpl = cpi = "";
											
											  $("tfoot.adv_alldeals").append("<tr>" + lastRow + "</tr>");
											  
											  $("tfoot.adv_alldeals tr:last").find("select,input[type=\'text\']").each(function(k,val) {
													$(this).val("");
											  }); 
											  row1++;
										}										
									$(document).ready(function(){
										...
										$("tfoot.adv_alldeals tr td input").on("click",function(){
												if($(this).val() == ""){
													return;
												}
												$( "tfoot.adv_alldeals tr td a" ).remove(".showDelete");
										...
										/* $("tfoot.adv_alldeals").on( "mouseover","tr",function(){
																		 
											if($("tfoot.adv_alldeals input[name^=deal_ids]").length >= 1){
												$(this).find("input[name=delete]").show();
											}
											
											if($("tfoot.adv_alldeals select[name^=deal_merchant_geo]").length > 1){
													$(this).find("input[name=delete_new]").show();
											}
										...
										$("tfoot.adv_alldeals").on("click", \'input[name="delete_new"]\', function(e){
										   $(this).closest(\'tr\').remove();
										   $("#adv_deal_count").val(parseInt($("#adv_deal_count").val())-1);
										});
										
										$(".dcpa_geo").keyup(function(event){
											
											dcpa_arr = [];
											$("input[name=\'deal_dcpa_geo[]\']").each(function(k,v){
											...
											if(dcpa_arr.length == 0){
												$(".dcpa_min_geo, .dcpa_max_geo").hide();
											}
											else
												$(".dcpa_min_geo, .dcpa_max_geo").show();
										...
										($(".cpa_geo") && $(".cpa_geo").val().length>0) 	? 	$(".cpa_geo").trigger("keyup") 	: 		null;
										($(".dcpa_geo") && $(".dcpa_geo").val().length>0) 	? 	$(".dcpa_geo").trigger("keyup") 	:	 	null;
							...
							<td width="40%" align="left" valign="top">
								...
								<table border="0" cellpadding="0" cellspacing="5">
									...
									<tr  id="empn" style="margin-top:20px;">
										...
										<td align="left">
										'.getQualifications($db['qualify_type'],0) . '
										...
										'.lang('Limitation').': <input type="text" name="db[qualify_amount]" value="'.$db['qualify_amount'].'" style="width: 100px; text-align: center;" maxlength="5" /></td>
															...
															<td width="10%">&nbsp;</td>
															<td';
															if (!(($userLevel == 'admin') && (($set->userInfo['id'] == 1) || ($set->userInfo['id'] == 78)))) {
																$set->content .= ' style="display:none;" ';
															}
															$set->content .= '>
                                                                <br><br>
                                                                        '.lang('Max monthly amount PNL').': $ <input type="text" name="db[max_pnl_monthly_amount]" value="'.getTraderDealsValue($id,'max_pnl_monthly').'" style="width: 100px; text-align: center;" /><br><br>
                                                                        '.(($userLevel =='admin')?''.lang('Reduce PNL').': <input type="text" name="db[pnl_lower]" value="'.getTraderDealsValue($id,'pnl_lower').'" style="width: 100px; text-align: center;" />%':'').'<br>
															</td>';
															
						$set->content .= '</tr>
						</table>
                                                
                                                <div style="text-align: center; padding:20px;"><input type="submit" value="'.lang('Save').'" /></div>
...
div.floatFeatures div {
	...
	PADDING-BOTTOM: 10PX;
			...
			while ($ww=mysql_fetch_assoc($qq)) {
				...
				$set->content .= '<div  data-tab="api_access" id="api_access" class="config_tabs" style="padding-bottom:20px;display:none;"><div class="normalTableTitle" style="cursor: pointer;">'.lang('API Access').'</div>
						<div id="tab_8" style="width: 100%; background: #F8F8F8;">
							...
							<table class="normal tblDetails" width="100%" border="0" cellpadding="3" cellspacing="0">
								<thead><tr>
									...
									'.(empty($affiliateapiurl) || $db['apiAccessType']=='None' || $db['apiAccessType']==''  ?'' : '<td align="center">'.lang('FeedUrl').'</td>').'
								...
								<tr>
									...
									'.(empty($affiliateapiurl) || $db['apiAccessType']=='None' || $db['apiAccessType']==''  ?'' : '<td align="center"><textarea style="width:255px;height:50px;" type="text"  readonly >'.$affiliateapiurl .'</textarea></td>').'
									<td align="center">'.(empty($affiliateapiurl) || $db['apiAccessType']=='None' || $db['apiAccessType']=='' ?lang('Inactive') : lang('Active')).'</td>
									<td align="center"><input type="submit" value="'.(empty($affiliateapiurl) ? lang('Save') : lang('Update')).'" />'.(empty($affiliateapiurl) ?'':'&nbsp;&nbsp;<input type="button" id="deleteAPIAccessIP" value="'.lang('Delete').'" />').'</td>
								</tr>
								</tfoot>
							</table>
						...
						<!-- API ACCESS SCRIPT-->
						<script>
						$("#deleteAPIAccessIP").on("click",function(){
							
							$.prompt("'.lang('Are you sure you want to reset the API settings?').'", {
								top:200,
								title: "'. lang('API Access') .'",
								buttons: { "'.lang('Yes').'": true, "'.lang('Cancel').'": false },
								submit: function(e,v,m,f){
									if(v){
										$("input[name=\'apiStaticIP\']").val("");
										$.post("'.$set->SSLprefix.'ajax/resetAffiliateAPISettings.php", { affiliate_id : '. $id .' }, function(res) {
											if(res ==false)
											{
												alert("'.lang('ERROR').'")
											}
		...
		$set->content .='
			...
			$set->content .= '<div id="'. ($set->introducingBrokerInterface ? 'sub_introduce_broker':'sub_affiliates').'" data-tab="'. ($set->introducingBrokerInterface ? 'sub_introduce_broker':'sub_affiliates').'" class="config_tabs" style="padding-bottom:20px;display:none;"><div class="normalTableTitle" style="cursor: pointer;">'.($set->introducingBrokerInterface ? lang('Sub Introduce Broker') : lang('Sub Affiliates')).'</div>
						...
						';
                            ...
                            $qq=function_mysql_query("SELECT * FROM ReportReferral WHERE AffiliateID='".$db['id']."' ORDER BY Date DESC LIMIT 50",__FILE__,__FUNCTION__);
							$merchantsArray = array();
							$l = 0;
                            while ($ww=mysql_fetch_assoc($qq)) {
									if(empty($merchantsArray[$ww['MerchantID']])){
										$merchantName=dbGet($ww['MerchantID'],"merchants");
										$merchantsArray[$ww['MerchantID']] = $merchantName;
									}
									else{
										$merchantName = $merchantsArray[$ww['MerchantID']];
									}
                                    $listTraffic .= '<tr '.($l % 2 ? 'class="trLine"' : '').'>
                                                            <td align="left"><a href="'.htmlentities($ww['ReferUrl']).'" target="_blank">'.htmlentities($ww['ReferUrl']).'</a></td>
                                                            <td align="center">'.$ww['LastClickIp'].'</td>
                                                            <td align="center">'.date("d/m/y H:i", strtotime($ww['Date'])).'</td>
	...
	case "save_wizard_pixel":
			...
			if (strtolower($wizard_trigger)=='ftd')
				$wizard_trigger = 'sale';
	...
	case "save_geo_deal": 
		...
		$_POST['deal_merchant'] = $_POST['deal_merchant_geo'];
		 for ($i = 0; $i < count($deal_merchant_geo); $i++) {
	
                unset($min_cpa_db, $cpa_db, $dcpa_db, $revenue_db, $revenue_spread_db,$lots_db, $cpl_db, $cpc_db, $cpm_db);
				
				if($country[$i] == "worldwide") $country[$i] = "WW";
				
                $chkDealMinCPA = mysql_fetch_assoc(function_mysql_query("SELECT id, merchant_id,amount FROM ".$appDeals." WHERE affiliate_id='".$affiliate_id."' AND lower(geo) = '". strtolower($country[$i]) ."' AND merchant_id='".$deal_merchant_geo[$i]."' AND dealType='min_cpa' ORDER BY rdate DESC",__FILE__,__FUNCTION__));
				
                $chkDealCPA = mysql_fetch_assoc(function_mysql_query("SELECT id,merchant_id, amount FROM ".$appDeals." WHERE affiliate_id='".$affiliate_id."' AND lower(geo) = '". strtolower($country[$i]) ."' AND merchant_id='".$deal_merchant_geo[$i]."' AND dealType='cpa' ORDER BY rdate DESC",__FILE__,__FUNCTION__));
                $chkDealDCPA = mysql_fetch_assoc(function_mysql_query("SELECT id, merchant_id,amount FROM ".$appDeals." WHERE affiliate_id='".$affiliate_id."' AND lower(geo) = '". strtolower($country[$i]) ."' AND  merchant_id='".$deal_merchant_geo[$i]."' AND dealType='dcpa' ORDER BY rdate DESC",__FILE__,__FUNCTION__));
                $chkDealCPI = mysql_fetch_assoc(function_mysql_query("SELECT id,merchant_id, amount FROM ".$appDeals." WHERE affiliate_id='".$affiliate_id."' AND lower(geo) = '". strtolower($country[$i]) ."' AND merchant_id='".$deal_merchant_geo[$i]."' AND dealType='cpi' ORDER BY rdate DESC",__FILE__,__FUNCTION__));
                $chkDealCPL = mysql_fetch_assoc(function_mysql_query("SELECT id,merchant_id, amount FROM ".$appDeals." WHERE affiliate_id='".$affiliate_id."' AND lower(geo) = '". strtolower($country[$i]) ."' AND merchant_id='".$deal_merchant_geo[$i]."' AND dealType='cpl' ORDER BY rdate DESC",__FILE__,__FUNCTION__));
                
                // Min CPA.
                processDealType($appDeals, $chkDealMinCPA, $set, $affiliate_id, $deal_merchant_geo[$i], 'min_cpa', -1, $deal_min_cpa_geo[$i], 'deal_min_cpa',$country[$i]);

                // CPA.
                processDealType($appDeals, $chkDealCPA, $set, $affiliate_id, $deal_merchant_geo[$i], 'cpa', -1, $deal_cpa_geo[$i], 'deal_cpa',$country[$i]);

                // DCPA.
                processDealType($appDeals, $chkDealDCPA, $set, $affiliate_id, $deal_merchant_geo[$i], 'dcpa',-1, $deal_dcpa_geo[$i], 'deal_dcpa',$country[$i]);

                // CPL.
                processDealType($appDeals, $chkDealCPL, $set, $affiliate_id, $deal_merchant_geo[$i], 'cpl', -1, $deal_cpl_geo[$i], 'deal_cpl',$country[$i]);
			    
				// CPI.
                processDealType($appDeals, $chkDealCPI, $set, $affiliate_id, $deal_merchant_geo[$i], 'cpi', -1, $deal_cpi_geo[$i], 'deal_cpi',$country[$i]);
            }
	...
	case "save_deal": 
		...
		/* 	if ($activeMerchants || $activeMerchants=="") {
            ...
            for ($i = 0; $i < count($deal_merchant); $i++) {
                ...
                // Revenue Spread.
                processDealType($appDeals, $chkDealRevenueSpread, $set, $affiliate_id, $deal_merchant[$i], 'revenue_spread', $revenue_spread_global, $deal_revenue_spread[$deal_merchant[$i]], 'deal_revenue_spread');
	...
	for($a=0;$a<$deal_count;$a++){
		  ...
		  // if ($_SERVER['REMOTE_ADDR']=='176.231.97.133')
		// $_GET['dbg1']=1;  
	
		$enums = get_enum_values("affiliates_deals",'dealType');
		
		$currDate        = dbDate();
	$datetimeMinusSecond = date("Y-m-d H:i:s", strtotime("-1 second", strtotime($currDate)));
	// echo $currDate.'<Br>';
	// die ($datetimeMinusSecond);
		...
		foreach ($deals as $deal) {
				...
				if ($enum!='tier' && $deal==$enum && count($deals)>0 ){
					...
					$lastDealRow = mysql_fetch_assoc(function_mysql_query($sql));
 ...
 '".$datetimeMinusSecond."',  ".$set->userInfo['id'].",  '".$deal_merchant[$a]."',  '".$affiliate_id."',  '".$deal."',  '0',  '',  '0',  '');";
					...
					else if ($enum=='tier' && $deal==$enum  ){
						
						
						$testParam=6;
						$tierDealActiveDeals = getCurrentActiveTierDeal($deal_merchant[$a], $affiliate_id , "",$testParam);
					...
					/* 	$sql = "select * from affiliates_deals where affiliate_id = " . $affiliate_id . " and merchant_id = " . $deal_merchant[$a] . " and dealType = '" . $deal . "' order by id desc limit 1;";
						...
						if ($lastDealRow['amount']<>0) {
							$sql = 	
									...
									1, '".date('Y-m-d H:i:s')."',  ".$set->userInfo['id'].",  '".$deal_merchant[$a]."',  '".$affiliate_id."',  '".$deal."',  '0',  '".$lastDealRow['tier_amount']."',  '".$lastDealRow['tier_pcpa']."',  '".$lastDealRow['tier_type']."');";
							mysql_query($sql);
						} */
						
						$normalizedNewTiers = array();
			/* 		echo '<pre>';
						var_dump($tierDealActiveDeals);
					echo '</pre>';
					echo '<br><br>';
					echo '<pre>';
					var_dump($_POST);
					 ...
					 */
							...
							for ($i=0; $i<count($deal_ids); $i++) {
								...
								echo '</pre>';
								die();  */
								$tmpDealFromPost=array();
								// $tmpDealFromPost['id'] = $deal_ids[$i];
								$tmpDealFromPost['amount'] = $deal_cpa[$i];
								$tmpDealFromPost['tier_amount'] = $tier_amount[$i];
								$tmpDealFromPost['tier_pcpa'] = $deal_pcpa[$i];
								$tmpDealFromPost['merchant_id'] = $deal_merchant[$i];
								
								
								$savedRow = getExistingTierDealRowData($tmpDealFromPost,$tierDealActiveDeals);
								if (empty($savedRow)){
									
									
										$existingRow = getExistingTierDealRowDataByID($tmpDealFromPost['id'],$tierDealActiveDeals);
										if (!empty($existingRow['tier_amount'])){
										$sql = 	
									"INSERT INTO  `affiliates_deals` (`valid` ,`rdate` ,`admin_id` ,`merchant_id` ,`affiliate_id` ,`dealType` ,`amount` ,`tier_amount` ,`tier_pcpa` ,`tier_type`)VALUES (
									1, '".date('Y-m-d H:i:s',strtotime("-1 second"))."',  ".$existingRow['admin_id'].",  '".$existingRow['merchant_id']."',  '".$existingRow['affiliate_id']."',  '".$existingRow['dealType']."',  '0',  '".$existingRow['tier_amount']."',  '0',  '".$existingRow['tier_type']."');";
										// die ('f1: ' . $sql);
							function_mysql_query($sql);
										}
								...
								// handle change issue HERE	
									$normalizedNewTiers[]= $tmpDealFromPost;
								...
								if ($savedRow['id']){
									$normalizedNewTiers[]= $savedRow;
								}
								unset($row);
							}
								
							for ($i=0; $i<count($tier_amount); $i++) {	
								
								
								$row['affiliate_id'] = $_POST['affiliate_id'];
								$row['tier_type'] = $tier_deal_type;
								$row['geo'] = '';
								$row['dealType'] = 'tier';
								$row['tier_amount'] = $tier_amount[$i];
								$row['amount'] = $deal_cpa[$i];
								$row['tier_pcpa'] = $deal_pcpa[$i];
								
								$row['valid'] = 1;
								
								$row['admin_id'] = $set->userInfo['id'];
								$row['rdate'] = date('Y-m-d H:i:s');
								$row['merchant_id'] = $deal_merchant[$i]; 
								if (!isset($row['amount']) &&  $row['amount']!=null && $row['amount']!="NULL" )
								$normalizedNewTiers[] = $row;
								unset ($row);
						...
						foreach ($normalizedNewTiers as $NewTierRow){
								if ($NewTierRow['id']){
							/* 	 	var_dump($_POST);
									echo '<br><Br>'; 
									echo '<br><Br>'; 
								 	var_dump($normalizedNewTiers);
									echo '<br><Br>'; 
									echo '<br><Br>'; 
									var_dump($tierDealActiveDeals);
									die(); */
									
								$tierDealActiveDealRow = getExistingTierDealRowData($NewTierRow,$tierDealActiveDeals);
								
						/* 		if ($tierDealActiveDealRow['amount'] == $NewTierRow['amount'] && 
								$tierDealActiveDealRow['tier_amount'] == $NewTierRow['tier_amount'] && $tierDealActiveDealRow['tier_pcpa'] == $NewTierRow['tier_pcpa'] ){
						 */		
						 if ($tierDealActiveDealRow == $NewTierRow){
									continue;
									
								}else {
							/* 	echo '<Br>';
								echo '<Br>';
								var_dump($tierDealActiveDealRow);
								echo '<Br>';
								var_dump($NewTierRow);
								echo '<Br>';
								die('-0-0-0-0-0-'); */
									...
									if (!empty($NewTierRow['tier_amount'])){
										$sql = 	
												"INSERT INTO  `affiliates_deals` (`valid` ,`rdate` ,`admin_id` ,`merchant_id` ,`affiliate_id` ,`dealType` ,`amount` ,`tier_amount` ,`tier_pcpa` ,`tier_type`)VALUES (
												1, '".date('Y-m-d H:i:s',strtotime("-1 second"))."',  ".$set->userInfo['id'].",  '".$deal_merchant[$a]."',  '".$affiliate_id."',  '".$deal."',  '0',  '".$NewTierRow['tier_amount']."',  '".$NewTierRow['tier_pcpa']."',  '".$NewTierRow['tier_type']."');";
										function_mysql_query($sql);
									}
								}
								}
								...
								if ($NewTierRow['tier_amount'] &&  ( $NewTierRow['amount'] > 0 || $NewTierRow['tier_pcpa']>0)){
						...
						die();
						 */
						if (count($tierDealActiveDeals) == count($normalizedNewTiers)){
					...
					// die ('gergerge');
						...
						foreach ($normalizedNewTiers as $lastDealRow){
								
						if ($lastDealRow['amount']<>0) {
							
							$sql = 	
									"INSERT INTO  `affiliates_deals` (`valid` ,`rdate` ,`admin_id` ,`merchant_id` ,`affiliate_id` ,`dealType` ,`amount` ,`tier_amount` ,`tier_pcpa` ,`tier_type`)VALUES (
									1, '".date('Y-m-d H:i:s',strtotime("-1 second"))."',  ".$set->userInfo['id'].",  '".$deal_merchant[$a]."',  '".$affiliate_id."',  '".$deal."',  '0',  '".$lastDealRow['tier_amount']."',  '".$lastDealRow['tier_pcpa']."',  '".$lastDealRow['tier_type']."');";
							mysql_query($sql);
						}
		...
		$counter = !empty(count($deal_ids)) ? count($deal_ids) : $deal_count;
		...
		for ($i=0; $i<($counter); $i++) {
                  ...
                  /*    if (!$deal_tier_amount[$i] AND $deal_cpa[$i] <= 0) {
                    ...
                    $db['admin_id'] = $set->userInfo['id'];
                    $db['affiliate_id'] = $affiliate_id;
					$db['merchant_id'] = $deal_merchant[$i]; 
					if (empty($db['merchant_id']))
						$db['merchant_id'] = $deal_merchant[0];
					...
					if (empty($db['tier_amount']))
                    $db['tier_amount'] = str_replace(' ','',$tier_amount[$i]);
						
					
                    $db['tier_pcpa'] = str_replace(' ' ,'',$deal_pcpa[$i]);
					if (empty($db['tier_pcpa']))
                    $db['tier_pcpa'] = str_replace(' ','',$pcpa[$i]);
					if (empty($db['tier_pcpa']))
                    $db['tier_pcpa'] = 0;
				   ...
				   $db['amount'] = str_replace(' ','',$deal_cpa[$i]);
					if (empty($db['amount']))
                    $db['amount'] = str_replace(' ','',$cpa[$i]);
						
						
						
                    $db['tier_type'] = $tier_deal_type;
					if (empty($db['tier_type']))
						$db['tier_type']= 'ftd_amount';
				 ...
				 */
						...
						$existingRow = getExistingTierDealRowDataByID($db['id'],$tierDealActiveDeals);
					if (empty($db['merchant_id']) && !empty($existingRow['merchant_id'])){
						$db['merchant_id'] = $existingRow['merchant_id'];
					}
					 if ($existingRow['id'] && $existingRow['tier_amount']==$db['tier_amount'] &&
					 $existingRow['amount']==$db['amount'] &&
					 $existingRow['tier_pcpa']==$db['tier_pcpa'] 
					 ){ 
					 $db['rdate'] = $existingRow['rdate'];
					 } else {
						unset($db['id']);
					 }
					// }
 ...
 // ["affiliate_id"]=> string(4) "2147" ["deal_count"]=> string(1) "2" ["deal_merchant"]=> array(2) { [0]=> string(1) "1" [1]=> string(1) "1" } ["tier_deal_type_new"]=> array(2) { [0]=> string(10) "ftd_amount" [1]=> string(10) "ftd_amount" } ["tier_amount"]=> array(2) { [0]=> string(6) "10-249" [1]=> string(11) "250-1000000" } ["cpa"]=> array(2) { [0]=> string(2) "20" [1]=> string(3) "200" } ["pcpa"]=> array(2) { [0]=> string(0) "" [1]=> string(0) "" } } enum: 
				 ...
				 // last confirmation, make sure same row is not coming in again
				 $testParam= 7;
				  // var_dump($_REQUEST);
				 $tierDealActiveDeals = getCurrentActiveTierDeal($db['merchant_id'], $affiliate_id , "",$testParam);
				 $hasThisDeal=false;
				 /* echo '<Br>';
				 echo '<Br>';
				 var_dump($db);
				 ...
				 foreach ($tierDealActiveDeals as $activeDeal){
				 /* var_dump($activeDeal);
				 echo '<Br>';
				 echo '<Br>';
				 echo '<Br>'; */
				 if (
				 $activeDeal['tier_amount']==$db['tier_amount']
				 &&
				 $activeDeal['amount']==$db['amount']
				 &&
				 $activeDeal['tier_pcpa']==$db['tier_pcpa']
				 &&
				 $activeDeal['merchant_id']==$db['merchant_id']
				 &&
				 $activeDeal['affiliate_id']==$db['affiliate_id']
				 ...
				 echo 'activeDeal:<Br>';
				 var_dump($tierDealActiveDeals);
				 echo '<Br><Br>gasthisdeal<Br>';
				 var_dump($hasThisDeal);
				 echo '<Br><Br><Br>';
				 die(); */
				 if (!$hasThisDeal){
                    dbAdd($db, 'affiliates_deals');
		...
		unset($db);
			...
			//} temporary moved up above counter == ....
...
if ($userLevel=='admin' ||  ($set->allowDeleteCRMnoteForManager==1)) {
	...
	function getLatestDeal($merchant_id,$affiliate_id, $dealName){
			global $appDeals;
			$afDealsQuery = "SELECT id,dealType,amount FROM ".$appDeals." WHERE dealType='". $dealName ."' and merchant_id='".$merchant_id."' AND affiliate_id='".$affiliate_id."' order by rdate desc limit 0,1";
			$takeqq = function_mysql_query($afDealsQuery,__FILE__,__FUNCTION__);
			$takeww = mysql_fetch_assoc($takeqq) ;
			return $takeww['amount'];
	}
	
	function getLatestAdvancedDeal($merchant_id,$affiliate_id, $dealName){
			global $appDeals,$set;
			
			
			$afDealsQuery = "
			select id,merchant_id,affiliate_id,dealType,amount " . ($set->deal_geoLocation==1 ? " ,geo " : " " ) . " from affiliates_deals WHERE merchant_id = ". $merchant_id ." and affiliate_id = ". $affiliate_id ." and valid = 1 " . ($set->deal_geoLocation==1 ? " and (geo='' or lower(geo)='ww') " : " " ) . " and dealType = '". $dealName ."'
			and rdate = 
			(
			SELECT max(rdate) as rdate
			FROM affiliates_deals p
			WHERE merchant_id = ". $merchant_id ." and affiliate_id = ". $affiliate_id ." and valid = 1 " . ($set->deal_geoLocation==1 ? " and (geo='' or lower(geo)='ww') " : " " ) . " and dealType = '". $dealName ."')
			UNION
			SELECT id,merchant_id,affiliate_id,dealType,amount " . ($set->deal_geoLocation==1 ? " ,geo " : " " ) . "  FROM `affiliates_deals` where merchant_id= ". $merchant_id ." and affiliate_id = ". $affiliate_id ." and dealType='".$dealName."' and valid =1 " . ($set->deal_geoLocation==1 ? " and geo !='' and lower(geo)!='ww' group by geo" : "" );
			/* if($dealName == "cpl"){
				echo $afDealsQuery;die;
			} */
			$takeqq = function_mysql_query($afDealsQuery,__FILE__,__FUNCTION__);
			$deals = array(); 
			while($takeww = mysql_fetch_assoc($takeqq)){
